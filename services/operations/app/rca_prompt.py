...
13. recommendations마다 risk_level을 반드시 넣으세요 — 이건 장애의 심각도가 아니라
    ...
    비워도 됩니다.
14. summary·analysis 등 자연어에서 수치를 언급할 때는 Evidence에 있는 값(current,
    baseline.mean, baseline.stddev 등)을 그대로 인용하세요. "평상시보다 N배/N% 높음" 같은
    배율·증감률을 직접 계산해 만들어내지 마세요 — 특히 baseline이 0에 가까운 지표는 나눗셈
    결과가 실제 변화 폭과 무관하게 비정상적으로 커질 수 있습니다. 배율 표현이 꼭 필요하면
    "약 N배 이상"처럼 근사치임을 밝히거나, 원본 수치(예: current=0.00176 vs
    baseline.mean=0.00172)를 함께 제시해 엔지니어가 직접 판단하게 하세요.
"""
