...
**설계·스크립트는 확정이다. 그런데 입력값 일부가 아직 가정이다.** 안 정한 채로 돌려도 스크립트는 돌아가지만 **나온 숫자의 해석이 달라진다.** 아래 6개만 정하면 된다. 전체 목록·근거는 [§10](#10-미확정-입력값--임의-확정-금지), 산출 과정은 [§3.1](#31-가정-전부-가정이다--확정-아님).

| # | 정할 것 | 현재 값 | **안 정하면 무슨 일이 생기나** | 누가 |
|---|---|---|---|---|
| **1** | **A7 · 창 내부 집중계수 *k*** | **3.0** | 🔴 **가장 임의적이고, 영향이 가장 크다.** baseline 도착률 λ 가 이 값에 **정비례**한다 — `k=2 → λ=0.27` · `k=5 → λ=0.67`. λ 가 흔들리면 §3.4 의 **MULT ↔ DAU 등가표 전체가 같은 배율로 어긋나고**, 최종 산출물인 *"DAU 몇까지 안전"* 결론이 그만큼 틀어진다 | 팀(제품 감각) |
| **2** | A2 1.6 세션/일 · A4 60% 흡수 · A6 창당 30분 | 좌측 | λ 를 함께 결정하는 나머지 세 가정. 개별 영향은 *k* 보다 작지만 **곱해져서 누적**된다 | 팀(제품 감각) |
| **3** | **write SLO** | **p95 < 1s (잠정)** | 등록·publish 의 **합격/불합격 선이 없다.** 숫자는 나오는데 "이게 통과냐"를 판정할 수 없어 §6.2 H8 이 판정 불가가 된다 | 팀 합의 |
| **4** | `HOT_PEAK` 상한 | 400 rps | 바이럴 knee 를 **못 찾고 끝날 수 있다**(400 까지 안 꺾이면 "한계 미도달"이 결론). 그 경우 상향 재실행이 필요하니 **재실행 여유를 미리 잡아둘지**를 정해야 한다 | 실행자 |
| **5** | **목표 성장 규모** | **미정** | ✅ **의도된 미정** — 이 테스트의 *산출물*이다. knee 가 나오면 §3.4 표로 "DAU 몇까지 안전"을 역산해 그때 정한다. 미리 정하지 말 것 | 이 테스트 결과로 |
| **6** | A9 세션 체류 180s | 180s | 영향 국소적 — **동시 유저 환산(리틀의 법칙)에만** 쓰인다. 도착률·부하량 자체는 안 변한다 | 낮은 우선순위 |

**🔴 가정을 실측으로 갈아끼울 수 있는 길이 있다.** 클릭스트림 파이프라인이 라이브다(`mp-user-event-sink`, 2026-07-20 개통) → **A2·A4·A6·A7 을 실제 세션 로그에서 산출 가능**하다. 다만 DAU 500 미만 실서비스라 **표본이 충분한지는 미확인**. 확인해서 갈아끼우면 §3 전체가 가정 기반에서 실측 기반으로 바뀐다 — 이게 가장 값싼 개선이다.

**판단이 아니라 제약(정해줄 수 없는 것)**
- **알림 fan-out 시나리오는 재현 불가** — 스키마에 팔로우/구독이 없고 notify 는 조회 2개뿐이다([§2.3](#23-바이럴은-왜-핫키인가--스키마-제약)). 기능이 생기면 그때 별건.
- **유저 등록 레시피는 ES 에 안 들어간다** — "등록 → CDC → 검색 노출" 을 전제로 한 측정은 성립하지 않는다. 코드 근거 = [§4.3](#43--유저-등록-레시피는-es-검색에-안-나온다-코드-근거).

⚠️ 위 판단과 별개로 **실행 전 반드시 읽을 것** = [§7.11 알림 소음 경고](#711--알림-소음-경고)(Slack 이 운다) · [§8 실행 절차](#8-실행-절차)(런당 ~760건 `TEST-` 레시피가 쌓인다).
