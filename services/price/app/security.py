"""Security — JWT **검증** 딥 모듈 (account가 발급한 access 토큰을 price가 로컬 검증).

price는 토큰 **소비자**다 — 발급(issue)·비밀번호 해시 없음 → bcrypt 불요(A03 의존성 최소화).
검증 규약은 account/security.py._verify 와 **동일**(HS256 · typ=='access' · sub=user_id) 해야
account 발급 토큰과 호환된다. 순수(DB·전역·시계 무관) → 인터페이스만으로 검증 가능.

price의 조회 API(현재가·이력·핫딜)는 공개라 인증이 없다. 관심 등록/해제(#29·#30)만 유저
귀속 데이터를 다루므로 이 모듈을 쓴다 — 라우트 단위로 붙이고 기존 공개 경로는 그대로 둔다.
