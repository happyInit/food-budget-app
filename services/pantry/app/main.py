...
log = configure_service_logger(service="pantry")
...
async def lifespan(app: FastAPI):
    ...
    log.info("pantry service started", extra={"event": "service_started"})
    try:
        yield
    finally:
        await pool.close()
        log.info("pantry service stopped", extra={"event": "service_stopped"})
...
async def log_unhandled_request_error(request: Request, call_next):
    ...
    try:
        ...
        log.error("unhandled pantry request failure", extra=fields)
        raise
