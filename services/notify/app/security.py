"""Security — 비밀번호 해시 + JWT 발급/검증의 딥 모듈. (account 정본 그대로 복제.)

작은 인터페이스(hash/verify/issue/verify_*) 뒤에 bcrypt·PyJWT 구현을 잠근다.
**순수**(DB·전역·시계 목킹 무관) → 인터페이스만으로 완전 테스트됨(tests/test_security.py).
⚠️ notify는 토큰을 **발급하지 않고** verify_access(JWT 검증)만 쓴다. 그래도 딥 모듈은
account와 동일하게 복제해 두어(bcrypt 미사용) 서비스 간 구조를 통일한다.
"""
