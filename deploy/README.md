...
CI가 이미지를 Harbor에 올리고 → VM2가 pull해서 컨슈머 상주 + 폴러 cron으로 돌린다.
정본 스케줄 = `docs/design.md` §7.1·§3.4. 토폴로지 = §8.4.

## 좌표 (design §8.4)
| VM | 좌표 | 역할 |
|----|------|------|
| VM1·Data | `192.168.0.8` | PG·ES·Redis·**Kafka**(:9092) |
| VM2·App+AI | `192.168.0.9` | FastAPI·ML·**크롤러/컨슈머/폴러** ← 이 배포 대상 |
...
| `poller-deal` | 15:05, 17:05 | 오아시스 딜 | timeSale 15시 리셋·closeSale 17시 오픈 직후 |
...
- `run-poller.sh`가 flock으로 중첩 실행 방지 + `/var/log/fb-pollers/<svc>.log` 기록.
