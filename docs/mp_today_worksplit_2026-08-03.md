...
```
┌─ 봉수 레인 ────────────────────────┐  ┌─ 태현 레인 ───────────────────────┐
│ config: services/**  (14개 서비스) │  │ config: platform/pgsync/**        │
│ config: monitoring/rules-app-*.yaml│  │ config: monitoring/rules-data-*.yaml│
│ app:    frontend/**                │  │ app:    infra/ansible/**           │
│ GitHub: 오너 권한(토큰·시크릿)     │  │ app:    scripts/index_recipes_es.py│
│ 클러스터: app ns 워크로드           │  │ 클러스터: argocd · data ns · ES     │
└────────────────────────────────────┘  └───────────────────────────────────┘
        겹치는 파일 0개 · 겹치는 ns 0개
```
...
### B-3. services 일괄 PR · 70분 · [필수 #13·#24 + 권장 #1 + #8 선행]
...
| **#1** | `services/*/base/deployment.yaml` × 12 | `priorityClassName: app-normal` |
| **#8선행** | `services/recipe/base/deployment.yaml` | `ES_INDEX` → `recipes_live` (alias명) |
...
`recipe(2)` `price` `pantry` `notify` `operations` `recipebook` `video` `frontend(2)` `cloudflared` `gateway(2)` + `mp-ocr-config-canary` CronJob
쿼터 여유 1,792Mi > 배치2 서지 1,344Mi → **수용 가능**(태현 실측).

**#8선행 상세** — 🔴 **태현이 alias 를 먼저 만들어야 한다**(§5-①, 2분). 만들어졌다는 확인을 받고 이 줄을 바꾼다.
...
# #8선행 확인
kubectl -n app exec deploy/mp-recipe -- env | grep ES_INDEX
```
...
### T-0. ES alias 선생성 · 2분 · [봉수 B-3 해제]

**가장 먼저.** 이게 있어야 봉수가 `ES_INDEX` 를 바꿀 수 있다.
...
→ 끝나면 봉수에게 **"alias 생성됨"** 한마디. 이후 서로 안 기다린다.

---

### T-1. ArgoCD sourceRepos 복구 · 40분 · [필수 #2]

**왜**: `descheduler`·`rollouts` Application 이 **지금 죽어 있다**(`InvalidSpecError`, 08-03 00:13·00:33~).
...
| # | 내용 | 방향 | 타이밍 |
|---|---|---|---|
| **①** | ES alias `recipes_live` 생성 | **태현 → 봉수** | 태현 **T-0**(오전 첫 2분). 완료하면 한마디 |
| **②** | `monitoring/kustomization.yaml` 에 규칙 파일 2개 등록 | **Claude → 둘 다** | **지금** Claude PR 로 선처리 |

②를 미리 안 하면 봉수(`rules-app-symptom.yaml`)와 태현(`rules-data-tier.yaml`)이
**둘 다 `kustomization.yaml` 을 고쳐서 충돌**한다. 그래서 제가 먼저 넣는다.
...
| 우려 | 실제 |
|---|---|
| 둘 다 config 레포에 push | **디렉토리가 다르다.** 봉수 `services/**` · 태현 `platform/pgsync/**`. git 이 자동 병합 |
| 둘 다 `monitoring/` | **파일을 갈랐다**(§5-②). `rules.yaml` 원본은 아무도 안 건드림 |
| 둘 다 클러스터 조작 | **ns 가 다르다.** 봉수 app ns 워크로드 · 태현 argocd·data ns·ES |
| 봉수 recipe 파드 재시작이 태현 재색인을 깨나 | **안 깬다.** 파드는 alias 를 읽을 뿐, 재색인은 ES 안에서 돈다 |
...
**봉수**
- [ ] `github_runner_pat` revoke · `DEPLOY_SSH_KEY` 삭제 (`argocd_repo_ssh_key` 는 유지)
- [ ] `queries.ts` `catch {}` 제거 + `npm run build` 통과
- [ ] services PR 머지 — #13 · #24 · #1 · ES_INDEX
- [ ] `mp-mealplan` restart 완료 · `PGSSLMODE` 주입 확인 · priorityClass 빈 파드 0
- [ ] (여력) 유저경로 알람 3종 — `for:` 15~30분

**태현**
- [ ] ES alias `recipes_live` 생성 → 봉수에게 통보
- [ ] `--check --diff` 확인 후 playbook → `descheduler`·`rollouts` **Healthy**
- [ ] PGSync 프로브 + 슬롯 알람 적용 · 파드 정상 기동
- [ ] (착수 시 잔여 3h 확보) 재색인 → **검증 후** alias 스왑 → 앱 검색 확인 → **구 인덱스 보존**
