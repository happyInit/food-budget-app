...
| ~~**fb-ci-harbor**~~ | 203 | ~~`.10`~~ | `10.10.10.10` | 3 | 5GB | on(≥3G) | 150G | 70G | ⚠️ **은퇴 수순**(2026-07-27) — Harbor·CI 가 신규 물리 호스트 C 로 이관(`.10`·인증서 승계). 구 이미지(`food-budget/*`)는 VM 파괴와 함께 소멸(백필 안 함). 파괴 후 5GB 회수 → worker-a1 재원 |
