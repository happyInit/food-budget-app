...
> **✅ 갱신(2026-07-15) — "스냅샷 export"로 확정.** 실시간 반복 쿼리가 아니라 **1회 벌크 스냅샷(CSV/JSONL) → 오프라인 학습**. OLTP에 라이브 부하를 안 주고(태현 지적 대응), 라벨·모델은 AI-side 아티팩트(git→S3, 모델 X). 상세: `ner-requirements-to-data.md` §2.1 #4 · §2.3. 아래는 원 논의.

- **실시간 쿼리 vs 스냅샷 export**: 제가 필요할 때마다 PG를 직접 쿼리하면 되는지, 아니면 특정 시점에 CSV/JSONL로 떨궈주시는 게 편한지 — 태현 쪽 파이프라인 운영 부담을 고려해서 편하신 방식으로. → **스냅샷 확정.**
