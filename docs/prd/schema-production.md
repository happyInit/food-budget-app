...
**⚠️ 동의 게이팅(§4):** 미동의 유저 이벤트는 **앱/컨슈머가 produce 안 함**. `session_id`로 노출↔클릭 조인.
**파이프라인:** 앱 → Kafka `events.user.activity` → 컨슈머 → PG(append-only). 노출 로그는 **mealplan 랭커가 emit**. (컨슈머·보존배치 설계 = `docs/proposals/clickstream-consumer-retention-design.md`)
...
CREATE TABLE activity.user_event (            -- VIEW 약긍정 · ADD_CART 주 라벨 · NOTIF_CLICK
  ...
  user_id bigint NOT NULL, session_id uuid,                  -- user_id=논리값·동의 유저만
  ...
  occurred_at timestamptz NOT NULL, context jsonb, created_at timestamptz NOT NULL DEFAULT now()
...
**메모**
- **참조 전부 논리값(FK X)** — 이벤트 로그를 카탈로그(recipe·item_master) lifecycle과 분리: 삭제돼도 이력 보존, write 성능, 삭제권(§4)=앱. (문서 §3.2 real FK에서 이벤트 특성상 조정.)
- **`event_id`·`impression_id`(멱등키):** producer 발급 UUID + `UNIQUE` → 컨슈머 at-least-once 재전달 시 `ON CONFLICT DO NOTHING`으로 중복 방지(ADD_CART 라벨 편향 회피). 설계서에서 발견한 갭 반영.
- **append-only**: UPDATE/DELETE 금지, 정정=새 이벤트. 학습=PG 1회 스냅샷 오프라인.
- **남음(별건):** Kafka 토픽·컨슈머·랭커 노출 produce 배선 — 동의 승인 후(설계서 §8 순서).
