...
## 7. ES 시방서 (오브젝트 9종 · 매니페스트 수준)

### 7.1 «CRD» Elasticsearch (ECK가 설치 — 문서화만)
`elasticsearches.elasticsearch.k8s.elastic.co` (group `elasticsearch.k8s.elastic.co`, kind `Elasticsearch`, Namespaced). ECK 번들 설치 시 등록되며 손으로 작성하지 않는다. 이게 있어야 아래 «CR»을 API가 인식한다.

### 7.2 «CR» Elasticsearch `recipes-es` (설계 본체)
```yaml
...
spec:
  version: 8.15.3
  image: harbor.local/library/elasticsearch:8.15.3-nori   # nori bake
  secureSettings:
    - secretName: es-s3-creds        # S3 키 → ES keystore(스냅샷)
  nodeSets:
    - name: zone-a
      count: 1
      config:
      ...
      podTemplate: &pod
        spec:
          ...
          affinity: { nodeAffinity: "host=A 노드에만" }
    ...
    - name: zone-b
      ...
      config:
        ...
        node.attr.host: b
        cluster.routing.allocation.awareness.attributes: host
      podTemplate: *pod                # affinity만 host=B
      volumeClaimTemplates: *vct
```

### 7.3 «STS» (ECK가 «CR»에서 생성 — 문서화만)
`recipes-es-es-zone-a`(1) · `recipes-es-es-zone-b`(2) → 합계 파드 3. **StatefulSet**(Deploy 아님 — 안정 identity·PVC·quorum). ECK가 «PDB» `maxUnavailable:1` 자동 생성. **HPA 없음**.

### 7.4 «PVC» (volumeClaimTemplate → 파드당 1)
`elasticsearch-data-recipes-es-es-zone-{a-0,b-0,b-1}`. accessModes **RWO** / storageClassName **openebs-lvm**(EKS=gp3, 오버레이 변수) / storage **5Gi** / **WaitForFirstConsumer**.

### 7.5 «SVC» (ECK 자동 2종)
`recipes-es-es-http`(ClusterIP, 9200 HTTPS — 앱 질의) · `recipes-es-es-transport`(Headless, 9300 — 노드간 discovery).

### 7.6 «Secret»
| 이름 | 생성 | 내용 |
|---|---|---|
| `recipes-es-es-elastic-user` | ECK 자동 | elastic 계정 비번 |
| `recipes-es-es-http-certs-public` | ECK 자동 | HTTP CA 인증서(앱 TLS 검증) |
| `recipes-es-es-transport-certs` | ECK 자동 | 노드간 mTLS |
| `es-s3-creds` | ESO 주입 | s3.client.default.access_key/secret_key(→CR secureSettings) |
| `es-app-creds` | ESO(ExternalSecret) | ES 유저/비번 → 챗·레시피·인덱서 ns |

### 7.7 «NP» NetworkPolicy `es-ingress-9200` (표준·이식용)
```yaml
...
### 7.8 «CNP» CiliumNetworkPolicy `es-cilium` (실제 강제)
```yaml
...
spec:
  ...
  ingress:
    - fromEndpoints: [{ app: chat }, { app: recipe }, { app: es-indexer }, { app: pgsync }]
      toPorts: [{ ports: [{ port: "9200", protocol: TCP }] }]
  egress:
    - toEndpoints: [{ "k8s-app": kube-dns }]            # CoreDNS(53) — 누락 시 클러스터 마비
      toPorts: [{ ports: [{ port: "53", protocol: UDP }] }]
    - toFQDNs: [{ matchName: "s3.ap-northeast-2.amazonaws.com" }]   # 스냅샷 S3
      toPorts: [{ ports: [{ port: "443", protocol: TCP }] }]
```
NP=이식용 표준자산, CNP=Cilium 실제 강제(+FQDN egress). 둘 병행(정본 패턴).

### 7.9 «Job» (컷오버 1회성 2종)
- **`es-index-template`**(ArgoCD PostSync): `curl -XPUT _index_template/recipes` 로 템플릿 등록. 내용 = `index_patterns:[recipes,recipes_v*]` + settings(`shards:1`/`replicas:1`/`korean` 분석기) + 현행 recipes 매핑 1:1(name·ingredient_names=text/korean, keyword들, *_g/kcal=float, servable=boolean, image_url=keyword index:false). **재색인 前** 실행.
- **`es-reindex-recipes`**: `index_recipes_es.py` 실행(품질게이트 그대로, **SETTINGS 제거** — 설정은 템플릿 소유). 컷오버 1회성 → CronJob 아님. env: `ESHOST=recipes-es-es-http:9200`, `ES_PW`(secretKeyRef), `PGHOST`.
