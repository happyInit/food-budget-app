...
| — | **대화형 어시스턴트 / 배치 플랜** | MealPlan reactive 어시스턴트(NER 재사용 + RAG, **생성=Gemini prod 활성**(2026-07-18, cost-break 가드) · **멀티턴·계정연동(제외재료 양방향) 활성**(2026-07-19)) · "며칠치 플랜" 배치 확장 |
...
| **ML Serving** | NER + 이상탐지(최저가) + 개인화 랭킹 재랭킹 서빙(`ranking-serving`) — ~~신선도~~ 드롭 |
...
- **품목 입도(granularity) 결정 (2026-07-20 확정)**: item_master 는 **평면**(parent 계층 없음). 부위·종 세분화(소고기→차돌박이·등심, 수산→종×부위 등)는 **의도적 보류** — species-level 버킷 유지, cut-level 아님.
  - **근거**: 예산앱은 '대략 얼마'가 목적이라 부위별 정가는 과잉. 한 카테고리를 쪼개면 전 카테고리(소고기·돼지·수산…) 분할 의무로 번져 **무한 큐레이션·유지비**(과복잡성). 얻는 건 예산추정 정밀도뿐.
  - **완화(분할 대신)**: `retail_item_price_compare` 100g 단가 = **트림드-min**(품목 median 25% 밑 저가 아웃라이어 제거, PR #226) — 버킷에 뼈·타종·가공식품이 섞여도 min 이 쓰레기값을 안 집음. + 앱 `min(usage, 1팩)` 상한이 상단 방어 → **양끝 바운드**. 그래서 coarse 버킷도 자릿수 맞는 가격. (계기: 차돌박이 200g 이 사골 잡뼈값 1,180원 → 트림드-min 후 2,948원.)
  - **분할 예외 기준(체계화 금지, ad-hoc만)**: ①레시피 고빈도 ②generic 버킷값과 **3배+** 어긋남 ③일회성 — 3조건 동시 충족 시에만 기존 갈비·안심처럼 **평면 품목**으로 추가(계층 X). 예: 차돌박이(현 generic 2,948 vs 실 ~5,560 = **1.9배 → 문턱 미달, 미실행**). 등심·양지·우삼겹도 동일.
  - **후보(문턱 넘으면)**: 차돌박이·등심·양지·우삼겹(SKU 충분·현재 소고기 alias). 가격은 이미 트림드-min 이 방어하므로 **저순위**.
...
- **영수증 OCR 방식 — 검토중(PoC 대기)**: Tesseract → Gemini Vision-first(저빈도·감열지 약함) 방향으로 기울어 있으나, 실물 영수증 PoC(20장 비교) 전까지 미확정. 공개 벤치마크상 Gemini가 저품질/감열지 문서에서 우세하나 한국어 감열지 특화 데이터는 부재 — PoC로 최종 확인 필요. 상세 `ai-spec.md` §7. **→ 2026-07-18**: PoC 확정 전 prod 배선 선반영 — compose `OCR_BACKEND=vision`(기본) + `GEMINI_API_KEY` 주입(fb-app `.env`, 챗 키와 분리=비용격리). 단 `ocr-service` 이미지 미빌드(Harbor 부재)로 컨테이너 미기동 → 실호출 없음(키만 스테이징). **→ 2026-07-16 방식 확정 = Gemini Vision 단독** · **🔴 2026-07-29 호스팅 = 개인 Google AI API 키 → GCP Vertex AI 이전 결정**(`gcp-migration-plan.md`·PR #387, 모델은 Gemini 유지).
...
- **챗봇/대화형 어시스턴트 생성 백엔드 — 확정(2026-07-13, 팀 최종)**: RAG 아키텍처 채택 + 1차 생성은 **TemplateGenerator(무료·승인 불필요)**. 실사용 품질 실측 후 부족하면 유료 전환(Bedrock/Gemini)을 그때 재검토 — 지금은 AGENTS.md 재승인 불필요. 상세 `ai-spec.md` §5·§8, `chat-assistant-ai.md` §3·§9. **→ 2026-07-18 유료전환 실행(prod)**: `GENERATOR_BACKEND=gemini` 활성 + `CHAT_GEMINI_API_KEY` 주입(fb-app `.env`), chat 컨테이너 재생성. 비용 가드 = 팀 cost-break(#155 — 월 예산 상한 초과 시 template 자동 강등). ⚠️ design 예상('실측 후 재검토')보다 앞선 활성 — AGENTS.md 절대제약3 예외확대 인지 필요. **→ 🔴 2026-07-28 실측 확정(최종): 생성 백엔드 = AWS Bedrock `apac.amazon.nova-micro-v1:0`(서울) 이전** — 프로덕션 경로 20/20 = Gemini 동률 · 40% 저렴 · 2배 빠름 · 국내처리. 정본 `ai-features-roadmap.md`, 근거 `ai-model-selection-final.md`. 카나리 전환.
