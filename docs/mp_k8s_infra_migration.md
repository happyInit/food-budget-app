...
nGrinder 실측 **~200 VUser 부근 포화**(응답 8~20s). VM CPU 는 여유(18%)였고 병목은 코드·설정 레벨 → 물질화 뷰 전환 + Redis 캐시로 1차 해소. (`design.md §8.5` · `mp_k8s_loadtest_design.md`)

다만 **잔여 포화의 근본 해법은 K8s 수평 확장**이라고 복수 문서가 명시한다:

- 컨테이너 `cpus` · 워커 수 · PG `max_connections` 상한이 포화의 1차 원인 → 근본 확장 = K8s 수평 (`design.md §8.5`)
- 수직 확장 여지는 작다. "진짜 스케일 = 수평 확장(K8s 이전 계획)" (`mp_k8s_loadtest_design.md §9`)
