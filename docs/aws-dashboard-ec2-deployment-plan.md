...
>
> 🔴 **정본은 `docs/operations-ai-aws-migration-plan.md` 다**(사용자 확정 · C-84 에서 지목).
> 새로 시작하는 판단은 전부 그 문서에서 출발한다.
>
> 🔴 **그래도 이 문서를 지우면 안 된다** — 아래 **확정 결정 5건이 여기에만 있다.**
> 새 정본의 §0-B 가 같은 목록을 갖고 있으며, **거기로 옮겨진 뒤에** 이 문서를 지운다.
>
> | # | 결정 | 이 문서의 위치 |
> |---|---|---|
> | ① | 학원 PG allowlist 는 **교체가 아니라 추가** (온프렘 IP 를 빼면 지금 도는 `mp-operations` 가 죽는다) + 차단 층 미확인(`pg_hba` 면 에러 / 앞단 방화벽이면 타임아웃) | §6.1 |
> | ② | **MongoDB 범위 밖** — 담당자 개인 로컬 설치물 · 라이브에 워크로드 0 | §6.2 |
> | ③ | **Cognito 기각 근거**와 확정된 로그인 흐름 | §3.1 · §3.2 |
> | ④ | **회색(DNS 전용)을 고른 근거 3가지** | §4 |
> | ⑤ | **NodePort 선택 근거·가역성** + 상시 `port-forward` 금지(감사로그 30일→52.62h 붕괴) | §5.1 |
>
> ⚠️ 아래 본문은 2026-08-13 시점이다. **라이브와 어긋난 곳이 있다** — 특히
> *"C-64 가 Prometheus 를 MNG 고정 노드에 묶었다"* 는 EKS 에서 성립하지 않고(오버레이가
> `nodeSelector` 를 제거했다), Prometheus 는 **NodePort 가 아직 없다**. 새 정본 §0-A 참조.
