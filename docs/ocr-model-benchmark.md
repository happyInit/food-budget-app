...
> **작성:** 건우 (AI 담당) · 2026-07-16 · **통합 2026-07-27** (`ocr-method-comparison.md` 방식 선택 근거 흡수)
> **목적:** OCR 방식·모델 선택을 **자체 실측 데이터**로 검증(팀 요구). §A=방식 선택 근거(왜 Vision), 상단=모델 요약·결론, 하단=전체 기록.
> **지표:** 성공률(합계정합=품목가격 합≈영수증 합계) · 소요시간 · 비용/장. 유료 티어.
> **관련:** `ocr-service-design.md`(설계·구현 정본)

---

# ■ A. 방식 선택 근거 — 왜 Gemini Vision인가 (구 `ocr-method-comparison.md`)

OCR은 **F5 냉장고 재고 등록**·**F17 식비 기록**의 공통 입구(`service-spec-handoff.md`). 출력은 어느 방식이든 **재료 NER → item_master 표준품목코드**로 넘어가 **뒷단 파이프라인은 방식 무관**(`ai-spec.md §7`) → 방식 선택은 **입구(이미지→구조화 품목)** 구간에 국한. 도메인 = **한국어 감열지(thermal)**(저대비·퇴색·구겨짐 = OCR 최악 구간).

## A.1 기준별 비교 매트릭스 (◎우수 ○양호 △약함 ✕부적합)
| 기준 | Tesseract 단독 | **Gemini Vision** | CRNN(CNN+RNN+LSTM) | Tess+Vision 폴백 |
|---|:---:|:---:|:---:|:---:|
| 정확도(감열지) | △ 급락 | **◎ 저품질 최강** | △ 데이터 없이 낮음 | ◎ |
| 구현 공수 | △ 파서 1.5~2주 | **◎ 며칠(프롬프트+JSON)** | ✕ 수주~수개월 | ✕ 복잡도 최대 |
| 런타임 비용 | **◎ 0원** | ○ 장당<1원 | ○ 추론 저렴(학습비 별도) | ◎ 폴백건만 |
| 오프라인 | **◎ 완전** | ✕ API 의존 | ◎ 로컬 | ○ 부분 |
| 유료승인 | ◎ 불필요 | △ 필요 | ◎ 불필요 | △ 필요 |
| 학습데이터 | ◎ 불필요 | **◎ 불필요(zero-shot)** | ✕ **대량 필요→치명** | ◎ 불필요 |
| 구조 출력 | △ raw→파서 | **◎ JSON 직출력** | ✕ 인식만 | ○ |
| 프라이버시/moat | ◎ 로컬 | △ 외부전송 | ◎ 로컬·자체모델 | ○ |

## A.2 결론
결정적 기준 = **공수·감열지 정확도·데이터기아**.
- **CRNN 탈락**(데이터 수만 라벨 필요, 20장뿐 + 인식만이라 검출·레이아웃 대공사). **moat는 NER이 이미 담당** → OCR엔 자리 아님.
- **④ 폴백 단독 채택 보류**(공수 최대). **CLOVA OCR 기각**(유료 → 무료 API 절대제약 충돌, `data-validation.md §132`).
- **Vision vs Tesseract** = "정확도·공수(Vision)" vs "비용·오프라인·승인(Tesseract)". 도메인이 감열지라 Vision 우세 + 뒤에 HITL 확정이 정확도 미달분 보정.
- **→ 채택: 방식무관 백엔드 골격 + 1순위 = Vision, 폴백 = Tesseract(인터페이스만), CRNN 제외.** 실측 근거는 아래 ■.
