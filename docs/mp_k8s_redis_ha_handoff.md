...
| | 후보 | 근거·상태 |
|---|---|---|
| **ⓑ** | 오퍼레이터 이미지 **v0.26.0** 오버라이드 | **최악인 시나리오 2(#1711)에 상류 수정 주장이 있는 유일한 선택지.** 🔴 차트는 0.26.0 이 **없다**(최신 차트 = 0.25.0) — 이미지만 올리는 것이라 **0.25.0 CRD 위에서 도는 조합**이 된다(§3 이 경고한 그 형태). → **먼저 시험할 것** |
| ⓒ | Sentinel 제거, 오퍼레이터 단일 제어면 | 제어면 충돌(1·4)은 사라지지만 **시나리오 2 는 그대로일 공산이 크다**(오퍼레이터가 승격을 안 했다). ⓑ 결과 뒤에 판단 |
