...
| R1 경계정책 | `classify.py:_EDGE_POLICY`(코드 상수) | **[해소 #243] `edge:3`** (생수·얼음·홍삼정) — 정책을 코드 상수화(파일→코드) | ✅ |
| R2 gazetteer | `classify.py:_load_gazetteer_db` | **`gaz_source:db, gaz:1079`** (§2-7) | ✅ 일치 |
| R3 보관법 | `classify.py:_load_shelf_db`(DB) | **[해소 #243] `shelf:121`** — `shelf_life_ref`(CURATED) DB 로드(gazetteer 패턴) | ✅ |
...
| 🚧 미배포 | 2 (→ **해소**) | **OCR R1(경계정책), R3(보관법)** — 이전 CSV 미배포(edge:0/shelf:0). **PR #243로 해소**: edge=코드 상수(_EDGE_POLICY), shelf=DB 로드(shelf_life_ref). 재확인 2026-07-20 배포본 `edge:3·shelf:121` |
...
2. ~~🚧 OCR 경계정책·보관법 시드 상시 미작동(edge:0/shelf:0)~~ → **[해소 PR #243]** "데이터는 DB, 정책은 코드" 원칙으로 수정: **shelf=`shelf_life_ref` DB 로드**(gazetteer 패턴), **edge=`_EDGE_POLICY` 코드 상수**(정책=로직). CSV는 문서로 유지, Dockerfile·compose 무변경. 배포본 재확인 `shelf_source=db shelf:121·edge:3`.
...
**[후속 2026-07-20] 클릭스트림 개통**: `.8` data-pipeline 1.1.4 재배포(Issue #220 close)로 Kafka 토픽 `events.user.activity` 생성·`user-event-sink` 컨슈머 기동·`activity.user_event` 싱크 확인(ADD_CART 11·impression 80). 데이터 흐름 개통, 현재 학습행<200 콜드스타트(축적 중; 2026-07-20 학습행 120·그룹 6). **OCR 시드 CSV 미배포(🚧)도 PR #243로 해소** — shelf=DB 로드(shelf_life_ref)·edge=코드 상수, 배포본 `shelf:121·edge:3` 재확인.
