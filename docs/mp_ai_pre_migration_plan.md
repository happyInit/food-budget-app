...
> 관련 정본 = `docs/mp_aws_prep_checklist.md` (**C-88 신설** · C-14 · C-20 · C-30 · C-44 · C-68 · C-72 · C-77 · `1-14` · `1-15` · `1-21`)
...
**결정: C-20(이미지에 굽기) 유지.** S3 + initContainer 로 바꾸지 않는다. **랭킹 서빙은 유지한다**(정지하지 않는다 — 근거는 §4.4).
...
### 4.4 랭킹 서빙 정지(Retire) — ❌ 하지 않는다

**정지 제안이 있었다.** 출처는 `docs/serverless-redesign/01_이전대상_7R.md` §2 의 Retire 검토 칸이다:

> *"**부의** — 서빙 Deployment 정지(사실상 Retire)를 팀 결정에 올림 (**호출 0 인데 상시 가동·PVC 점유**)"*

기능이 잘못됐다는 지적이 아니라 **"안 쓰이는데 왜 띄워두냐"** 는 자원 관점의 지적이다.

**실측 — 절약분이 거의 없다**

```
requests   CPU 100m · 메모리 192Mi
실사용     CPU   3m · 메모리 145Mi        ← CPU 는 요청의 3%
PVC        1Gi        (실사용 0.95 MiB — 702KB 모델 하나)
```

노드 2대 재집계 **8.19 vCPU** 대비 **100m = 1.2%**. PVC 1Gi 도 C-16 총량 116 GiB 대비 미미하다.

**정지하지 않는 이유 4가지**

1. **절약분이 CPU 1.2% · PVC 1Gi 로 미미하다** — 되살릴 때의 재배포·재검증 비용이 더 크다
2. 근거인 *"호출 0"* 은 **`RANKING_ML_ENABLED=false` 로 의도적으로 꺼둔 결과**이지 기능이 죽은 게 아니다. 클릭스트림을 켜서 학습 데이터를 모으는 목적이 결국 **그 플래그를 켜는 것**이므로, **정지 근거는 소멸할 예정**이다
3. 지금도 **모델을 들고 정상 대기 중**이다 — `GET /health` → `{"status":"ok","model_loaded":true}`
4. `CLAUDE.md` 의 자체 CPU 모델 3종(NER · 최저가 · **랭킹**) 중 하나다. 정지하면 **AI moat 가 2개로 준다**

🔴 **제안의 지위 정정** — 그 문서는 `.git/info/exclude` 로 **로컬 제외**돼 있어 레포에 없다. 즉 **팀은 이 정지 제안을 본 적이 없다.** *"팀 판단 대기"* 가 아니라 **로컬 문서에 적힌 미결**이며, 위 실측으로 **근거가 성립하지 않는 것으로 판단**한다.

⇒ **랭킹 서빙 유지 · #616 은 조건부가 아니라 확정.**
