...
| 🟢 **온프렘 필수 · 무해** | **2** | **`0-8b`**(가장 먼저 · `patch pv` = 재시작 0) · `0-22`(호스트 C · 🟢 여유 **36G** · 디스크 우려 해소) | 라이브 · Ansible 신규 롤 |
| 🔴 **온프렘 필수 · 조심** | **1** | `1-1` + `1-18` + `1-16` — **분리 불가**(WAL 이 지금 차고 있다) | 라이브 `data` ns |
...
| ⚪ 무관 | **1** | `0-21`(문서) | — |
| 🆕 신규 | **4** | **`A-43`** Jenkins CD eks 핀 · **`A-44`** ENI SG 단위 · **`A-45`** 파이프라인·pgsync 핀 구조 · **`A-46`** ECR 네이밍 통일 | 앱·config 레포 |

🔴 **순서 하나만 강제** — `0-8b`(PV → Retain)를 `0-3`(소유권 이전) **앞에** 둔다.
