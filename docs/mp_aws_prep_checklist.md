...
════ AWS Organizations ══════════════════  │  ═══════════════════════════════
 ...
 │  🔴 선행 = multi-arch 빌드(A-19) — 지금 buildx 0건 · 노드 5/5 amd64   │
 │                                                                       │
...
- [ ] **A-19 🔴 multi-arch(arm64) 빌드 — C-2(GitLab CI) 의 요구사항으로 실을 것** (2026-08-09 신설)
      실측: `buildx`·`--platform` **0건** · `docker build` 3회 = amd64 전용 · 노드 **5/5 amd64**.
      Graviton(`m7g`, ~20% 저렴)을 쓰려면 선행이다. 🔴 **GitLab CI 를 새로 쓰는 김에 넣지 않으면, 나중엔 파이프라인을 두 번 고치게 된다.**
...
- [ ] **A-20 노드그룹 2층 구성** — 층1 MNG(`m7g.xlarge` × 3 · AZ 당 1대 · **consolidation 없음**) / 층2 Karpenter NodePool(온디맨드 · 평시 0대).
      🔴 **Karpenter 자신이 층1 위에 떠야 한다** → MNG 를 먼저 만든다.
      🔴 **데이터 티어를 Karpenter 에 맡기지 말 것** — consolidation 이 EBS 붙은 PG·ES·Kafka 를 재시작시킨다.
      A-12(Karpenter 채택 확정)와 한 묶음
...
AWS 착수  20건   ← 온프렘 선행이 아니다
                   A-1~A-11 (S4 산출물) · A-12~A-18 (C-27 배포전략 전환)
                   · A-19~A-20 (C-29 컴퓨트 형상)
──────────────
합계     122건
```
