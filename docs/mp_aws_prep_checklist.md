...
> | | |
> |---|---|
> | **확정 결정** | **C-1 ~ C-59** (§0.1). ⚠️ CLAUDE.md 는 아직 "C-1~C-29" 로 적혀 있다 — **stale** |
...
| C-45 | 🔴 **워커 노드 = `m7g.xlarge` × 2 로 시작 + Karpenter 로 확장.** C-43 의 "3대" 를 **정정**한다. 실측 재집계 **6.22 vCPU / 18.86 GiB**(0-27 적용 후) vs 2대 allocatable **7.84 vCPU / 29.1 GiB** → CPU **79%** · 메모리 **63%**. 🔴 **`0-27` 이 전제** — 미적용이면 **104%** 로 안 뜬다. 월 **$292.88**(3대 대비 **−$146.44**) | 2026-08-10 | 사용자 확정 + 실측 |
...
| **확정 소계** | | **$722.80** | 83.4% |
| GitLab·kubecost EC2 · LCU · AZ간 전송 | 수량 가정 | $143.46 | 16.6% |
| **합계** | | **$866.26** | 100% |
...
| kubecost 3종 (`finopsagent` 0.5/0.5 는 잔류) | 0.160 | 3.30 | C-19 |
...
| kubecost aggregator·store·frontend | −160m | −3,383 MiB | 클러스터 밖 EC2 (C-19) |
...
```
   m7g.xlarge 1대 실사용 가능(allocatable)   약 3,920m CPU / 약 14,900 MiB 메모리
                    ...
                    합계                CPU              메모리
     2대   7,840m / 29,800 MiB     6,240 → **80%**    18,810 → **63%**   ✅
     3대  11,760m / 44,700 MiB     6,240 →   53%      18,810 →   42%     여유
```
...
```
   2대 (AZ 당 1대)
     노드 1대 사망 = **AZ 1개 사망**과 같다
       → 남은 1대 = 3,920m < 필요 6,240m  🔴 절반 이상 Pending
...
> 반영 범위 = **C-1 ~ C-58** (2026-08-11 — **NACL 미채택**(C-58) · 🔴 **계정 3개 → 1개**(C-57) · **Interface 엔드포인트 3종 채택**(C-56) · PG 복제 하이브리드(C-55) · DR 전환 2단(C-54)).
...
════ AWS Organizations (남의 조직 o-8gviph817d · mgmt = aws_3@etechcloud.net) ═══════
 ...
 │ ┃   🟡 arm64 지원 확인 중 — 미지원이면 m7i.xlarge(x86, +$59)                 ┃ │
 │ ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛ │
 │ ┏━ VPC-C "관측·FinOps" 10.12.0.0/16 (C-34) 🟡 용도 제안 ━━━━━━━━━━━━━━━━━━┓ │
 │ ┃   kubecost EC2 (C-19) · FinOps 대시보드 (선생님: AWS Budgets 대체)        ┃ │
 │ ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛ │
 │                                                                              │
 │  [VPC 밖 · 리전 관리형]                                                       │
 ...
 └──────────────────────────────────────────────────────────────────────────────┘
  ...
  EC2 4대 = 워커 2 + GitLab 1 + kubecost 1   (부하·배포 중 Karpenter +0~2)
```
...
| ⑧ | **GitLab CE 의 arm64 지원 여부** | 미지원이면 CI 를 x86(`m7i.xlarge` +$59)로 | 🟡 조사 중(1-6 레인) |
...
| 🆕⑲ | 🔴 **EC2 부트스트랩 전송로** — C-37(키페어 금지)로 **Ansible 의 SSH 경로가 없다** | **가** Terraform `user_data`(🔴 재실행 불가) / **나** Ansible over SSM(`aws_ssm` 커넥션 — 🟢 롤 자산 유지·멱등) / **다** Packer AMI. 🔴 **C-59(Omnibus)가 `나` 를 밀어준다** — `gitlab-ctl reconfigure` 를 다시 돌려야 하는 구조라 **재실행 가능한 도구**가 필요하다 | 🔴 미결 (2026-08-11) · A-28 의 선행 |
| 🆕⑳ | **VPC-B 미결 묶음** — 러너 위치 · 러너 아키텍처 · 유입 경로 · NAT | 러너(EC2 privileged DinD ↔ EKS 파드+IRSA+rootless)가 나머지 셋을 연쇄로 정한다. 🔴 **결정 0건** · `sonar-scanner-cli` 는 **amd64 단일**이라 arm64 러너와 충돌 | 🔴 사용자 파악 중 (2026-08-11) |
...
- [ ] **1-44 S3 라이프사이클 규칙 Terraform 신설 (C-49)** — 🔴 **지금 규칙이 없다.**
      CloudTrail 90일(Object Lock GOVERNANCE) · PG 백업 30일 · 크롤 30일 · **만료 후 즉시 삭제**
...
- [ ] 🆕 **A-28 CI 서버 프로비저닝 — 🔴 "수정"이 아니라 대부분 "신설"이다** (2026-08-11 신설, C-59)
      ...
      🟡 **동거 검증** = SonarQube 실측 1.65 GiB + GitLab 권장치가 `t4g.xlarge`(16GiB)에 함께 들어가는지 (C-38 은 스왑 4~8GiB 전제)
