...
> | | |
> |---|---|
> | **확정 결정** | **C-1 ~ C-65** (§0.1). ⚠️ CLAUDE.md 는 아직 "C-1~C-29" 로 적혀 있다 — **stale** |
...
| C-18 | 🔄 **보강·개명(2026-08-12, C-65)** — "온사이트 덤프" 는 **위치가 아니라 방법(논리 백업)** 이 본질이고, 목적지 버킷 이름은 **`mp-pg-dump-ap2`** 로 확정. **MinIO 삭제 → S3** · 🔴 **덤프 목적지는 barman 과 다른 버킷**(선행) | 2026-08-09 | 아래 (D5 ①) |
...
##### 🔴 C-65 의 근거 — 두 백업을 가르는 축은 **위치가 아니라 방법**이다 (2026-08-12)
...
> 반영 범위 = **C-1 ~ C-65** (2026-08-12 — **PG 백업 2트랙·`mp-pg-dump-ap2` 개명**(C-65) · 2026-08-11 — **CI/CD 형상 확정**(C-61·C-62·C-63) · **NACL 미채택**(C-58) · 🔴 **계정 3개 → 1개**(C-57) · **Interface 엔드포인트 3종 채택**(C-56) · PG 복제 하이브리드(C-55) · DR 전환 2단(C-54)).
...
- [ ] **1-20 `data/mp-pg-onsite` egress 를 S3(443)로 여는 CNP 추가** 〔목적지 = **`mp-pg-dump-ap2`** · C-65〕 — 현재 DNS·5432·**9000(MinIO)만** 허용.
...
- [ ] **2-8 `mp-pg-onsite-dump` 프리픽스 DR 분기** (AWS 목적지 = **`mp-pg-dump-ap2`** · 프리픽스 `onprem/`·`aws/` · C-65) — 같은 경로면 상호 덮어쓰기 + 백업 신선도 판정 오염. **0-23 과 한 작업**
