...
| C-29 | 🔄 **보강(2026-08-10, C-43)** — 3대 유지 · 2+1 배치. `m7g` 는 메모리 특화가 아니라 **범용** 계열이다(표현 정정). **컴퓨트 형상 = `m7g.xlarge`(Graviton) × 3 · AZ 당 1대 · MNG 고정**(consolidation 없음) **+ Karpenter NodePool 1개**(온디맨드 · 평시 0대 · BG 버스트 전용). 🔴 **Spot 미채택** · 선행 2건 = **CPU 요청 재조정** · ~~**multi-arch 빌드**~~ ⟳ **C-89 로 소멸**(사이트별 단일 아키 — EKS 는 arm64 만 받으면 된다) | 2026-08-09 | 아래 (D10 수량 절반 해소) |
...
A1  데이터 티어    CNPG **빈 클러스터**(인스턴스 2 · AZ 당 1) · ECK 3노드 · ElastiCache Valkey
                  ...
                  🔴 **DDL 은 `SET ROLE fbapp` 뒤에 돌린다** — 2026-08-14 실측 회고: A1 이 이걸
                     `postgres` 로 돌려 **DB 의 모든 오브젝트 소유자가 postgres** 가 됐다(온프렘은 fbapp).
                     소유자가 갈리면 ① PGSync bootstrap 이 `must be owner of relation recipe` 로 죽고
                     ② 이후 멱등 DDL 을 앱 롤로 못 돌리고 ③ 양 사이트 DR 절차가 달라진다.
                     복구 = `ops/eks-owner-realign`(config #184 · 완료). 🔴 **A3 의 `pg_restore` 는
                     덤프의 `ALTER ... OWNER TO fbapp` 을 그대로 실어오므로 같은 함정이 아니다** —
                     함정은 *덤프가 아니라 DDL 을 직접 돌리는 경로*다
...
| 날짜 | 내용 |
|---|---|
| 2026-08-14 | 🟢 **A1 회고 — EKS 오브젝트 소유자 정합화 완료(postgres → fbapp) + ArgoCD Degraded 3건 중 2건 해소.** **① 발견** — ArgoCD `pgsync` 가 Degraded 라 들어갔더니 `mp-pgsync` 가 `Replication slot "foodbudget_recipes_live" does not exist` 로 CrashLoop 이고, 그 원인인 bootstrap 이 `must be owner of relation recipe`(`DROP TRIGGER`)로 죽고 있었다. 🔴 **증상은 pgsync 하나로 보였지만 범위는 DB 전체였다** — 실측: 스키마 8 · 테이블 41 · 시퀀스 26 · 뷰 2 · matview 1 이 **전부 `postgres` 소유**(온프렘은 전부 `fbapp`). 원인 = **A1 이 `schema-production.sql` 을 postgres 로 돌린 것**. **② 왜 GRANT 로 못 고치나** — PG 에서 **트리거 삭제는 소유자 전용**이고 `GRANT TRIGGER` 는 *생성*만 준다. ⇒ `platform/pg/base/bootstrap-role.yaml` 이 설계한 경로(`inRoles: [fbapp, pgsync]` = **owner 권한 상속**)는 **fbapp 이 소유자일 때만** 성립한다. 🔴 그 파일이 *"table owner 변경과 superuser 사용은 금지한다"* 로 못박고 있어, 소유권을 **새 롤로 옮기거나 슈퍼유저로 실행하는 안은 둘 다 기각**된다. **③ 조치** — 소유자를 **온프렘 값(fbapp)으로 되돌리는 정합화**(설계 변경이 아니라 적재 사고 복구). config **#184** `ops/eks-owner-realign/`(README + `reown.sql`) · 단일 트랜잭션 · `lock_timeout 10s` · 검산 실패 시 전체 롤백 · 필터 `relowner='postgres'` 로 **pgsync 소유물 구조적 제외**. 실행 결과 **70 오브젝트 이전**, 온프렘과 표 일치. 🟢 **무해성 실측** — GRANT 는 소유자 변경에 영향받지 않는다(`svc_account` 유지 / `svc_price` 는 여전히 차단 = 격리 보존) · pgsync 파드 **재시작 0** · 슬롯 3개 유지 · CDC 계속 동작. **④ 🔴 Job 이 아니라 런북인 이유** — `ALTER ... OWNER TO` 는 현재 소유자/superuser 전용인데 클러스터가 **`enableSuperuserAccess: false`** 라 superuser TCP 자격증명이 **없다**. Job 화하면 그것을 새로 만들어야 하고(레포 금지사항), `kubectl exec` 로컬 소켓 **peer 인증**은 자격증명을 만들지도 저장하지도 않는다 ⇒ **SQL 은 레포에, 실행은 사람이**(`ops/pgsync-stable-alias` 선례 · ArgoCD 미참조). **⑤ 곁다리 — 죽은 ExternalSecret 2종 제거**(config **#182**): `mp-harbor-pull`(EKS 는 kubelet 이 노드 롤로 ECR 을 당긴다 ⇒ dockerconfigjson 불요 · `1-31` 누락분 정렬) · `mp-pg-onsite-minio`(C-18 로 MinIO 없음 + A4 목적지는 S3+IRSA 라 정적 키 자체가 불요 = **A-49 ③**). 둘 다 영구 `SecretSyncedError` 라 **ns 의 ESO 가 상시 빨강 → 진짜 실패가 묻히는** 상태였다. ⇒ ArgoCD **38 Healthy / 1 Degraded**(`pipelines` 만 잔존 — `argocd/overlays/eks` 에 path 패치가 없어 EKS 가 `pipelines/overlays/onprem` 을 보고 Harbor `192.168.0.10` 을 4.5시간째 당기는 중 · **사용자 판단으로 유예**). 🟢 **비용 영향 0 실측** — Karpenter NodeClaim 0 · NAT egress 19MiB/6h(월 ~$0.14, 클러스터 전체) · 실패 풀은 SYN 뿐이라 그 안에서도 반올림 오차. **⑥ 남은 것** = `ALTER ROLE fbapp NOLOGIN`(`schema-roles.sql` 5단계 · EKS fbapp 은 아직 login=true) · 오늘 슈퍼유저로 만든 트리거 2종은 **다음 bootstrap 때** 정상 경로로 갈린다(🔴 그것 때문에 지금 bootstrap 을 돌리지 말 것 — stock bootstrap 은 슬롯·트리거를 drop = 전체 재동기화) |
