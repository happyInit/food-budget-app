...
> | | |
> |---|---|
> | **확정 결정** | **C-1 ~ C-89** (§0.1). ⚠️ **CLAUDE.md 재동기화 필요**(2026-08-14, C-89 — 대조표 `CI` 줄이 여전히 "Jenkins → GitLab" 이다) · 종전 동기화 범위 `C-1~C-87` · 선행 작업 실측(체크박스 182 / 고유 번호 163) · 대조표 `외부 LB` 를 **ALB(C-60)** 로 정정 |
...
> | **당장 답이 필요한 것** | **#585**(AI·알림 담당 · 08-12 기한) · ~~A-24 조직 SCP~~ → ✅ **승인 받음**(2026-08-13) · 🔴 **남은 것 = A0 용 AWS 자격증명**(계정 ID + VPC/EKS/IAM 권한 프로필 — `mp-backup` 은 백업 전용) |
>
...
| C-2 | **CI = EC2 1대에 GitLab** (Jenkins 은퇴) ⟳ 🔴 **정정(2026-08-14, C-89) — 은퇴 철회 · 쌍방 영구 운영.** 온프렘 amd64 이미지 공급원이 계속 필요하다 | 2026-08-07 | 사용자 |
...
| C-7 | 🔄 **정정(2026-08-13, C-82)** — ~~**Cilium IPAM = `cluster-pool`**(오버레이 유지, ENI 모드 미채택)~~ → **ENI 모드 채택**. 🔴 근거 4개 중 3개가 무너졌다 — ①동형성은 C-70(온프렘 앱 소멸)이 무력화 ③파드 밀도는 `m7g.xlarge` max-pods **58 > 실측 36** 이라 인스턴스 타입 강제가 **없다** ④파드별 보안그룹은 오버레이의 *손실*이었으므로 ENI 면 **되찾는다**. 살아남은 근거는 ②(ENI 는 cilium-operator 가 EC2 권한 IRSA 를 요구)뿐이고, 그 대가보다 **오버레이의 대가가 크다**(EKS API 서버가 오버레이 파드에 라우팅 불가 → 웹훅 5종을 `hostNetwork` 로 영구 패치) | 2026-08-07 → 정정 2026-08-13 | 아래 (C-82) |
| C-8 | 🔄 **부분 정정(2026-08-10 · 2026-08-11)** — ④AZ 3개→**2개**(C-32) · ⑤서브넷→**4티어×2AZ=8개**(C-33) · ②계정 3개→🔴 **1개**(C-57 · 조직 멤버라 계정 생성 불가) · ⑥엔드포인트→**Interface 3종 채택**(C-56) · IdC·SCP 미채택(C-35·C-50) · ③**파드 CIDR `10.20.0.0/16` 소멸**(C-82 = ENI 모드라 파드가 노드 서브넷 VPC IP 를 받는다) · **VPC 3개→2개**(C-73). **VPC / Landing Zone** — 리전·계정·CIDR·AZ·서브넷·NAT | 2026-08-07 | 아래 |
| C-9 | 🟡 **재검토(2026-08-10, C-41)** — Tailscale 이 없어져 전제가 바뀜. **진입점 = 공개 ALB 1개만. 내부 도구 6종은 ALB 없이 Tailscale 로만**. ⟳ **해소(2026-08-13, C-85)** — C-53 이 Tailscale 을 중단시켜 *"Tailscale 로만"* 이 공백이 됐고, **내부 도구 6종 = `kubectl port-forward`**(C-80 public 엔드포인트 + IAM · **$0 · LB 불요**)로 메웠다. 🟢 **"공개 ALB 1개만" 은 그대로 지켜진다** | 2026-08-07 | 아래 |
...
| C-18 | 🔄 **정정·개명(2026-08-12, C-57·C-68·C-69)** — **MinIO 삭제 → S3** 는 유지. 🔴 ~~*"온사이트 덤프 목적지는 barman 과 다른 버킷/**계정**"*~~ 에서 **계정 분리는 불가능하다**(C-57 = 조직 멤버라 계정 생성 권한이 없다). 대체재도 없다 — **Object Lock 은 barman 의 보존 정책(30d)과 충돌**하고(락을 걸면 barman 이 옛 백업을 못 지워 무한 증식) **SCP 는 C-50 미채택**이다. ⇒ **버킷 분리까지만 한다**(`mp-pg-dump-ap2` ≠ `mp-backup-ap2` · C-68·C-69). 🔄 **버킷 이름은 C-69 에서 `mp-pg-onsite-ap2` → `mp-pg-dump-ap2` 로 개명**했다 — "온사이트" 는 *위치*를 말하는데 실제 근거는 *방법(논리 백업)* 이고 🔴 **AWS 에는 "사이트" 가 없다**(C-57 로 계정 1개 · 두 버킷이 같은 리전). 🟢 **그래도 대부분이 막힌다** — 잘못된 prefix 삭제 · lifecycle 오설정 · 워크로드 키 유출은 실무에서 일어나는 거의 전부이고 버킷 분리로 차단된다. 🔴 **못 막는 것 = 계정 admin 이 백업을 통째로 지우는 것**이고 이건 **받아들인다** — 그 시점이면 클러스터·데이터가 이미 넘어간 상태라 백업만 지키는 의미가 작다. 🟢 진짜 오프사이트 사본은 **온프렘 PG replica cluster**(C-55 Step 1)다 | 2026-08-09 · 정정 2026-08-12 | 아래 (D5 ①) |
...
| C-26 | 🔄 **정정(2026-08-11, C-60)** — 진입 형상이 **ALB 로 바뀐다**. ~~Cloudflare(주황) → NLB(TCP:443 패스스루) → Istio Gateway · TLS 종단 = Istio Gateway 유지(LE) · **ALB 미채택**~~ → **Cloudflare(회색) → ALB(ACM 종단 · AWS WAF) → Istio Gateway**. ⟳ **재정정(2026-08-13, C-82)** — ~~`target-type: instance` 만은 그대로 유효(오버레이 CNI 제약)~~ → 🟢 **`target-type: ip` 가 가능해진다.** 그 제약의 전제가 *파드 IP 가 VPC 에 없다*는 것이었고 **ENI 모드가 그 전제를 없앤다**. 부수로 노드 홉·`externalTrafficPolicy` 함정·헬스체크 포트 문제(`1-34`)가 함께 소멸한다 | 2026-08-09 | 아래 (C-60 으로 대체) |
| C-27 | 🔄 **정정(2026-08-13, C-76)** — ~~이관은 **현행 그대로**(canary 2 + 롤링 7) 넘어간다~~ → **카나리를 유지하지 않고 버린다**. 방향(**전 서비스 Blue-Green** · Argo Rollouts + prePromotionAnalysis)과 **파일럿 → 판정 → 전면 전환** 은 그대로. **ADR-0002** 로 기록 | 2026-08-09 → 정정 2026-08-13 | 아래 (D6 해소 · C-76) |
...
| C-29 | 🔄 **보강(2026-08-10, C-43)** — 3대 유지 · 2+1 배치. 🔴 **이 "3대"는 C-45(2026-08-10)가 곧바로 "2대 + Karpenter"로 정정했다 — 이 머리말이 따라오지 않았다.** 2026-08-16 감사 실측 = 노드 **2대**(`m7g.xlarge` · 2a·2b) + Karpenter NodePool `mp-burst` **0대**. 즉 **라이브는 C-45·C-87 과 일치**하고 이 줄만 옛 숫자다. 아래 본문의 "× 3"도 같다 — **정정된 결정행이 머리말을 이긴다.** `m7g` 는 메모리 특화가 아니라 **범용** 계열이다(표현 정정). **컴퓨트 형상 = `m7g.xlarge`(Graviton) × 3 · AZ 당 1대 · MNG 고정**(consolidation 없음) **+ Karpenter NodePool 1개**(온디맨드 · 평시 0대 · BG 버스트 전용). 🔴 **Spot 미채택** · 선행 2건 = **CPU 요청 재조정** · ~~**multi-arch 빌드**~~ ⟳ **C-89 로 소멸**(사이트별 단일 아키 — EKS 는 arm64 만 받으면 된다) | 2026-08-09 | 아래 (D10 수량 절반 해소) |
...
| C-33 | 🔄 **정정(2026-08-13, C-74)** — ~~4티어 × 2AZ = **8개**(공개·노드·데이터·**도구**)~~ → **3티어 × 2AZ = 6개**(공개·노드·데이터). **도구 티어 삭제** — 입주 예정이던 Tailscale subnet router 가 C-55 로 **미채택**(AWS 는 K8s Operator 만 = 파드라 서브넷 불요)이고 C-53 으로 착수도 중단됐다. CIDR 배분 확정 · **EKS 노드는 전용 서브넷** 은 그대로 | 2026-08-10 → 정정 2026-08-13 | 아래 · 선생님 지시("EKS 노드는 별도 서브넷") |
| C-34 | 🔄 **정정(2026-08-13, C-73)** — ~~**VPC = 3개**(③ 관측·FinOps `10.12.0.0/16`)~~ → **2개**. ① 서비스 `10.10.0.0/16` ② CI `10.11.0.0/16`. 🔴 **VPC-C 폐기** — C-64 로 kubecost 가 인클러스터로 들어간 뒤 **입주자가 0** 이 됐고, 빈 VPC 는 라우트 테이블·IGW·엔드포인트만 복제한다. 이상징후·FinOps 대시보드는 **VPC-A 공개 서브넷**에 둔다 | 2026-08-10 → 정정 2026-08-13 | **선생님 지시**(3개) → 사용자 확정(2개) |
...
| C-38 | **CI 서버 = GitLab `t4g.xlarge`(4vCPU/16GiB, 월 $121.47) + 스왑 4~8GiB** · **SonarQube 동거**. ⟳ **정정(2026-08-14 · 사용자 확정)**: 동거 근거로 적었던 *"차단 게이트 목적"* 은 **현행과 다르다** — 온프렘 Jenkins 도, GitLab 이식본도 **측정만(비차단)** 이다. 🔴 **당분간 비차단 유지**하고 차단 게이트는 **나중에 시간이 되면** 해본다. 동거 근거는 *"차단 게이트"* 가 아니라 **① 분석이 CI 워크스페이스를 봐야 하고 ② 기계를 하나 더 세울 이유가 없다**(실측 1.65 GiB) 로 대체한다 | 2026-08-10 | 아래 (실측) · ⟳ 2026-08-14 |
...
| C-53 | 🕐 **잠정 중단(2026-08-13, 사용자 지시)** — **결정을 뒤집는 게 아니라 착수를 멈춘다.** 중단 대상 = `1-45`·`1-46`·`1-47`·`A-23` + app#592(닫음 · 브랜치 존치) · config#150·#155(보류). 🟢 **지금 개발은 안 막힌다** — C-53 이 대체하려던 `ssh -L` + 원격 `kubectl port-forward` 2중 터널이 그대로 유효하다. 🔴 **EKS 가 서면 다시 올라온다** — C-41 원안(EKS 공개 엔드포인트+IAM+IP제한 / cloudflared+Access / SSM)에는 **파드에 닿는 수단이 없고**, C-53 의 존재 이유가 정확히 그것이었다(PG·ES 가 파드). 재개 조건 = 사용자 재개 지시 **또는** 미결 ⑮(EKS 엔드포인트 IP 허용목록)의 답이 exit node 로 날 때. ~~🔴 **Tailscale 채택 — 사람 평면 전용.**~~ C-41 의 "미채택" 을 **범위 정정**한다(기각은 *데이터 복제 경로로서* 만 유효). 배치 = ① AWS **도구 서브넷** subnet router `t4g.nano` ② EKS **Tailscale K8s Operator** ③ 온프렘 subnet router 파드. 🔴 **②가 없으면 PG 에 못 붙는다** — PG·ES 는 파드(10.20/16 오버레이)라 subnet router 가 광고하는 VPC CIDR 안에 **없다**. 비용 = 월 **$1.90**(Tailscale Personal **6인 무료** 실측 · subnet router·operator 전 티어 포함) | 2026-08-10 | 사용자 확정 |
...
| C-58 | **NACL 미채택 — VPC-A 한정.** 근거는 비용($0)이나 복잡도가 아니라 🔴 **사려는 것을 살 수 없다**는 것: NACL 이 SG 보다 나은 점은 `DENY` 하나뿐인데, ① ⟳ **재작성(2026-08-13, C-82)** — 종전 근거 *"파드 IP `10.20/16` 은 VPC 에 없는 주소라 NACL 이 애초에 도달 불가"* 는 **ENI 모드로 소멸했다**(파드가 VPC IP 를 받으므로 NACL 이 적용 가능해진다). 🔴 **그래도 미채택은 유지**한다 — 근거가 *불가능*에서 *무용*으로 바뀔 뿐이다: **파드 ↔ 파드는 같은 서브넷 안**이고 NACL 은 **서브넷 경계에서만** 평가되므로 같은 서브넷 내부 통신에는 **규칙이 적용되지 않는다**. → Cilium netpol 전담(#532 라이브) ② **데이터 서브넷 → 인터넷**은 **라우팅**이 이미 강제(NAT·IGW 경로 자체가 없다) ③ **공격 IP 차단**은 **Cloudflare 엣지**(C-4·C-46). ⇒ 남는 용도 0. 🔴 부수 근거 = **스테이트리스라 응답 수신용 임시포트 1024-65535 를 열게 되어 실질 통제가 소멸**하고, 오설정 시 **조용히 끊긴다**(타임아웃으로만 보임). 🟡 **VPC-B 는 별건** — 공개 서브넷 안(공인 IP · SG 가 유일한 벽)을 고르면 `DENY` 가 값어치를 갖는다 | 2026-08-11 | 사용자 확정 |
...
| C-72 | 🔴 **이관 원칙 = 온프렘 동결 · 이관은 덧셈만.** (사용자 지시 2026-08-12) **① 온프렘의 기존 코드·오브젝트는 이관이 끝나고 AWS 가 안정적으로 돌 때까지 손대지 않는다.** 되돌릴 원본이 없으면 컷오버 실패가 곧 복구 불가다. **② 검증은 기존 것을 고쳐서 하지 않고 사본을 새로 만들어서 한다** — 기존 파일·CronJob 은 🔒 불변, 새 경로는 **별도 파일·별도 오브젝트**로 짓고 **`suspend: true` + 수동 트리거**(`kubectl create job --from=cronjob/...`)로 1회씩 검증한다. 🔴 **상시 병행(dual-write)도 미채택** — 크롤 대상(마켓컬리·오아시스) 요청이 2배가 되어 **차단 위험**이 는다. **③ AWS 는 처음부터 Kafka·MinIO 없이 짓는다** — 그건 *"온프렘에서 먼저 걷어낸다"* 와 다른 말이다. **재분류** = `0-29`(stream 15모듈) · `0-26`+C-18(MinIO) · `0-31`(Strimzi) · `0-28`(크롤 `--kafka`) → 🕐 **전부 이관 후**. 🔴 **`0-29` 는 "처분 판정"이 아니라 "삭제 금지·방치"** 로 성격이 바뀐다(#585 무응답 상태에서 지우면 Kafka 로 되돌아갈 경로가 사라진다). ⟳ **적용 사례로 들었던 PR #593 판정은 오류였다(2026-08-13 정정).** diff stat 의 `22-` 를 삭제로 읽은 것이고, **22줄 전부 제자리 수정**이라 원래 경로가 보존된다 — `produce_record()` 는 **`emit_record()` 로 개명**되고 Kafka produce 블록은 `if _producer is not None:` 안에 **원문 그대로** · Dockerfile 은 `confluent-kafka` **유지** + `boto3` 추가 · `json.dump` 는 `else` 로 보존 · `run()`·`--out`·종료코드 조건은 **인자·항 추가**. `--kafka` 만 주면 동작이 main 과 동일하다. 🔴 **그리고 분리안(#612)은 성립하지 않았다** — `--s3` 플래그와 `boto3` 가 #593 에만 있어 config#153 이 호출할 대상이 없었다. ⇒ **#612 닫고 #593 채택**(둘 다 2026-08-13). 🟢 **원칙 자체는 유효하다** — 판정 기준은 여전히 *"기존 파일에서 코드를 지우는가"* 이고, 그 판정을 **diff stat 이 아니라 diff 본문으로** 해야 한다는 교훈이 추가됐다 | 2026-08-12 | 사용자 지시 |
| C-73 | 🔴 **VPC = 2개.** C-34 의 "3개"를 **정정**한다 — **VPC-C(관측·FinOps `10.12.0.0/16`) 폐기.** 근거 = C-64 로 kubecost 가 **인클러스터로 복귀**한 뒤 그 VPC 의 입주자가 **0** 이 됐다(정본에도 `🟡 용도는 제안` 으로만 남아 있었다). 빈 VPC 는 라우트 테이블·IGW·엔드포인트를 복제하면서 **얻는 게 없다**. 🔴 **이상징후 대시보드 + FinOps 대시보드 = VPC-A 공개 서브넷**(조건부가 아니라 확정 · 사용자). ⚠️ **선생님 지시(3개)를 뒤집는 것**이므로 근거를 남긴다 — 지시 당시엔 kubecost 가 클러스터 밖 EC2 였다(C-19), 그 전제가 **비지원으로 확인돼 폐기**됐다 | 2026-08-13 | 사용자 확정 |
| C-74 | 🔴 **서브넷 = 3티어 × 2AZ = 6개** (공개 · 노드 · 데이터). C-33 의 "4티어 8개"를 **정정**한다 — **도구 티어 삭제.** 근거 = 그 티어의 유일한 입주 예정자가 **Tailscale subnet router** 였는데 **C-55 가 AWS 쪽 subnet router 를 미채택**했고(AWS 는 K8s Operator 만 = **파드라 서브넷이 필요 없다**) **C-53 으로 착수까지 중단**됐다. 정본 다이어그램에도 이미 `④ 도구 … 🔴 비어있다` 로 적혀 있었다. 🔵 대역 `10.10.16.0/20` 은 **예비로 남긴다**(반납할 이유가 없다) | 2026-08-13 | 사용자 확정 |
| C-75 | **CloudFront 미채택 — 확정.** C-60 의 `🟡 2단계 옵션`(ALB 앞에 CloudFront 를 얹어 CF 무료 L7 DDoS·CDN 절감을 되살린다)을 **명시적 기각**으로 바꾼다. 🔴 **그러면 포기가 영구히 확정된다** — ① **L7 DDoS 방어는 AWS WAF 레이트룰이 유일**해진다(Shield Standard 는 L3/L4 만 · Advanced 월 $3,000 은 논외) → 룰 설계 비중이 커진다(`1-49`) ② **CDN 대역폭 절감 상실**(콜드 방문자당 **443KB** 실측)을 ALB 전송료로 그대로 낸다. 🟢 규모(0.959 req/s · DAU ~500)에서 금액은 작다 | 2026-08-13 | 사용자 확정 |
| C-76 | 🔴 **배포전략 = Blue-Green 을 골자로 이관한다 · 카나리는 버린다.** C-27 의 *"이관은 현행 그대로(canary 2 + 롤링) 넘어간다"* 를 **정정**한다. **범위를 나눈다** — ① `mp-account`·`mp-recipe` = **이관 시점에 canary → blueGreen 으로 spec 교체**(이미 Rollout 이고 컨트롤러·Gateway API 플러그인이 라이브 실증됨: 0→20→50→100 자동 promote) ② **나머지 11종 = Deployment 롤링 그대로 넘긴 뒤 안정화 후 순차 전환**. 🔴 **13종 동시 BG 는 노드 용량이 안 받는다** — 2대가 CPU 75.9% / 메모리 71.7%(C-45·C-64)이고 BG 는 promote 창에 파드가 **2배**다 ⇒ Karpenter 버스트 상한 **실측이 선행**(A-35). 🟢 **eks 오버레이는 처음부터 blueGreen 으로 쓴다**(두 번 쓰지 않는다). 🟡 replica 1 서비스는 BG 창에도 HA 가 없다 — *BG 부착* 과 *replica 2 승격* 은 별개 판단이고 후자는 6Gi 예산과 묶인다. ❌ **전부 Deployment 로 내려서 이관 후 일괄 전환** 은 기각(라이브에서 도는 Rollout 2개를 지우는 것 = 되돌릴 원본 폐기 · C-72 정신 위반) | 2026-08-13 | 사용자 확정 |
| C-77 | 🔴 **AWS 용 IaC 는 기존 파일을 수정하지 않고 전부 새로 작성한다** (C-72 의 IaC 판 · 사용자 지시). **Terraform** — 기존 `infra/terraform/*.tf`(Proxmox)와 `infra/terraform/aws/`(#593 크롤분: S3·SQS·IAM)는 🔒 **손대지 않고**, **`infra/terraform/aws-platform/` 신설**(VPC·EKS·노드그룹·ALB·ACM·WAF·KMS·IRSA·CloudTrail·GuardDuty·EventBridge·Lambda). 🔴 **backend key 를 따로**(`tfstate/aws-platform.tfstate`) — 이 스택의 apply 가 크롤 스택이나 Proxmox VM 을 건드릴 수 없어야 한다. **Ansible** — `site.yml`·`k8s.yml`·`hypervisor.yml` 과 기존 롤 전부 🔒 불변, **`eks.yml` + `roles/eks_*` 신설**(`connection: local` · `KUBECONFIG`=EKS). 🔴 **기존 롤을 재사용하지 않고 복사해서 만든다** — 기존 롤이 이미 `kubectl --kubeconfig {{ }}` 라 *돌기는* 하지만, 공유하면 **AWS 쪽 수정이 온프렘으로 번진다**. 대가(중복)가 정확히 사려는 것(격리)이다. **config 레포** — `overlays/onprem/` 🔒 불변(렌더 불변 8/8 실증), `overlays/eks/` 만 쓴다 | 2026-08-13 | 사용자 지시 |
| C-78 | 🔴 **이관 순서 = A0~A6 · DB 는 앱과 한 창 · 파이프라인은 맨 뒤.** K8s 이전(P0~P4)의 순서를 따르되 🔴 **P1(앱)/P2(데이터) 분리를 하지 않는다** — 그때는 앱이 옮겨간 뒤에도 구 데이터 티어를 **같은 LAN 에서** 봤지만, AWS 는 **C-41(사이트 간 상시 연결 없음)** 이라 그 중간 상태가 존재할 수 없다. **A0** 기반(네트워크·EKS·CNI·CSI·오퍼레이터·Istio·ArgoCD·관측 — 트래픽 0·데이터 0) → 🆕 **A0.5 CI(GitLab)**(2026-08-13 신설·사용자 확정 — VPC-B·EC2 1대·러너·`.gitlab-ci.yml` ⇒ **ECR 18리포에 arm64 이미지 적재**. 🔴 **A1 이 이것을 요구한다** — ES 파드가 우리 이미지 `mp-elasticsearch-nori` 를 쓰고 `mp-pgsync` 도 우리 것이라 CI 없이 A1 을 시작하면 데이터 티어가 `ImagePullBackOff` 로 선다. 🔴 `0-22`(Jenkins 백업)는 **선행에서 뺐다** — 사용자 판단 = 호스트 C 로컬 소재라 감수) → **A1** 데이터 티어 기동(CNPG **빈 클러스터**·ECK·ElastiCache·스키마 DDL · 🟢 **여기서 페일오버 promote 실증** = `1-9` 리허설 부재가 자동 해소) → **A2** 앱 **12종**(🔄 **정정 2026-08-13 · C-84** — `operations` 가 대시보드 EC2 의 compose 로 나간다) + `aws.mealbong.cloud`(회색→ALB) 내부 검증 → **A3** 🔴 **데이터 이전 + 컷오버를 한 창에**(온프렘 쓰기 중단 → `pg_dump -Fc -Z6` 318MiB → `pg_restore` → **ES 는 PG 에서 재파생** → Redis 빈 채로 시작 → 스모크 → DNS 전환 · **다운타임 5~10분** · 롤백 = 레코드 1개) → **A4** 안정화(관측·알림·**barman → S3**·논리 덤프·HPA/KEDA) → **A5** 파이프라인(크롤 S3/SQS + **온프렘 PG → AWS PG 증분 적재**) → **A6** 온프렘 축소(C-70). 🟢 **백업(A4)을 컷오버 뒤로 미뤄도 되는 근거** = A3 직후 AWS PG 를 통째로 잃어도 **온프렘 PG 가 읽기 전용으로 그대로 살아 있다**(C-72) ⇒ 원본이 있으니 다시 덤프하면 된다. 🔴 **그래서 A6 를 A4 완료 없이 하면 안 된다** — 그때가 원본이 사라지는 시점이다 | 2026-08-13 | 사용자 확정 |
| C-79 | 🔴 **S3 보관 = 계층 전환(IA·Glacier) + 만료 삭제까지의 라이프사이클을 전 버킷에 넣는다.** C-49 의 "Standard 단일"을 **정정**한다. 🔴 **판단 축이 절감이 아니다**(사용자 지시) — *"백업을 어떻게 했고, 어떻게 보관하고, 보관정책이 무엇이고, 언제 삭제되는지"* 라는 **스토리가 산출물**이다. C-49 의 실측(Glacier IR 절감 **$0.26**)은 여전히 사실이고 근거란에 남긴다. 🔴 **그래서 보관창을 먼저 늘려야 규칙이 발동한다** — Standard-IA 는 최소 저장 **30일**, Glacier IR·Flexible 은 **90일**, Deep Archive 는 **180일**이라 현 보관창(90/30/30일)으로는 계층에 **도달하기 전에 만료**된다. 확정 배치 = `mp-cloudtrail-ap2` Std 0-30d → IA 30-90d → **Glacier Flexible 90d-1y** → 만료 **400일** / `mp-pg-dump-ap2` Std → IA 30-90d → **Glacier IR 90-180d** → 만료 **190일** / `mp-source-backup-ap2` Std 0-90d → IA 90-180d → **Deep Archive 180d+** → 만료 `source/` **400일**·`gitlab/` **14일** / `mp-crawl-ap2` Std → IA 30-90d → 만료 `incoming/` **90일**·`failed/` **30일** / `mp-observability-ap2`〔🔴 **실물 이름은 `mp-observability-eks` 다 — 2026-08-16 감사 실측.** 이 행이 쓰인 뒤 실제 생성에서 이름이 갈렸고 아무도 정정하지 않았다. **라이프사이클을 이 이름으로 걸면 존재하지 않는 버킷에 거는 것**이라 조용히 무효가 된다〕 Std → 만료 **30일**. 🔴 **`mp-backup-ap2`(barman WAL·base)만 계층 전환 없이 Std + 만료 35일** — ⓐ 16MB 객체가 수천 개라 **요청료가 저장료를 넘고** ⓑ **PITR 이 필요한 순간에 복구가 몇 시간 걸리면 백업이 아니다**. 🔴 IA·Glacier 는 **최소 과금 크기 128KB** 이므로 작은 객체는 오히려 손해 — 위 배치는 전부 대용량 단일 객체 버킷이다. **설계도(`docs/design/` xlsx)도 수정 대상** | 2026-08-13 | 사용자 확정 |
| C-80 | 🔴 **운영·개발 접근 = 비대화형 경로가 필수다.** 근거 = 지금 개발이 `ssh -o BatchMode=yes … 'kubectl …'` **무암호·비대화형**을 전제로 돌아가고, AWS 에서도 **같은 환경이 유지돼야** 한다(사용자: *"필수적으로 필요"*). **(A) EKS** = `aws eks update-kubeconfig` → `kubectl`. 🟢 **C-35(IAM Identity Center 미채택)가 여기서 결정적으로 유리** — IdC/SSO 는 **브라우저 로그인**이라 비대화형이 불가한데, IAM 사용자 + **장기 access key** 는 `~/.aws/credentials` 로 무암호가 된다. 필요 = 전용 IAM 사용자 · **EKS Access Entry 등록**(C-24 `kubernetesGroups`) · public 엔드포인트 · IP allowlist 없음(C-41). **(B) EC2** = 🔴 **`aws ssm send-command`(Run Command)**. `start-session` 은 **대화형 셸이라 쓸 수 없다** — Run Command 가 `ssh '<명령>'` 과 정확히 동형이고, **Ansible 의 `community.aws.aws_ssm` 도 같은 경로**를 쓴다(C-61⑥). **비용 = $0**(IAM·access key·Run Command·public 엔드포인트 전부 무료). 🔴 **위험 3개** = ⓐ 장기 access key 는 **만료가 없다** → **90일 로테이션**(A-36) ⓑ IP allowlist 없음이라 키만 있으면 어디서든 붙는다 ⓒ 완화 = **`apply`·`delete` 는 소유자 프로필에만**, 에이전트 프로필은 **읽기 + `plan`**. 🔴 **적용 범위 = 지금은 봉수 1인** — 나머지 4인 IAM 은 **이관 후 차차**(이관이 우선) | 2026-08-13 | 사용자 확정 |
| C-81 | **감사로그 · RBAC Phase2 보고서를 정본에 절로 신설한다.** C-66(EKS `audit` → CloudWatch Logs)의 파생이고, 지금은 인과가 C-66 행 안에 한 줄로만 있어 **보고서로 못 쓴다**(사용자: 보안 세팅 때 함께 잡을 것). 담을 것 = ① **왜 지금 안 되는가** — 온프렘 마스터는 우리 것인데 audit 를 안 켰고, 보존창이 문서상 30일 vs **실측 52.62시간**(`maxage=30` 이 `maxsize 100MB × maxbackup 10` 에 가려 무효 · `1-25`) ② **왜 CloudTrail·Loki 로 대체 불가인가**(CloudTrail = AWS API 감사라 `kubectl get secret` 이 안 남는다 / 컨트롤플레인 파일은 EKS 에서 **AWS 소유**) ③ **EKS 에서 어떻게 해결되는가** ④ **그 결과 풀리는 것** = `1-26`(읽기의 84% 미기록) 해소 → **RBAC Phase2**(실사용 기반 최소권한) ⑤ **대가** = 커스텀 audit policy 불가로 **볼륨 조절 불가**(51.42M events/월) · 월 ≈ $59 **추정** → 1개월 실측 후 재판단(미결 ㉒) | 2026-08-13 | 사용자 확정 |
| C-82 | 🔴 **Cilium IPAM = ENI 모드.** C-7 의 `cluster-pool`(오버레이)를 **정정**한다. **발단 = 미검토 사항 1건** — Cilium 공식 문서가 오버레이에 대해 *"the EKS API Server is unable to route packets to the overlay network"* 이고 웹훅은 *"host networked or exposed through a service or ingress"* 여야 한다고 명시하는데, **C-7 의 근거 4개에 이 항목이 없었다**. 우리 웹훅 사용자 = **cert-manager · CNPG · ECK · Istio · Argo Rollouts** + metrics-server ⇒ 오버레이면 **오퍼레이터 5개 차트를 `hostNetwork` 로 영구 패치**(포트 충돌 + PSA privileged 예외 동반). **C-7 근거 재평가** = ①동형성 → **C-70 이 무력화**(온프렘 앱 13종 소멸 · C-60 이 이미 형상 분기) ②AWS API 의존 0 → 🟡 **유일하게 살아남음**(cilium-operator 가 `CreateNetworkInterface` 등 EC2 권한 IRSA 필요 · 다만 EBS CSI·Karpenter·ESO 도 이미 IRSA 의존이라 "의존 0"은 이미 성립하지 않았다) ③파드 밀도 → 🔴 **틀렸다**: `m7g.xlarge` max-pods **58**(4 ENI × 15 IPv4) vs 실측 **36/노드** ⇒ **인스턴스 타입 강제가 없다**(prefix delegation 이면 더 늘어난다) ④파드별 보안그룹 → 🟢 **오버레이의 손실이었으므로 ENI 면 되찾는다**. 🟢 **부수로 풀리는 것** = `target-type: ip` 가능(C-26) · 웹훅 패치 5개 불요 · metrics-server·`kubectl exec/logs` 정상 · **파드 CIDR `10.20.0.0/16` 자체가 소멸**(C-8③). 🔴 **대가** = EC2 권한 IRSA · 파드가 **노드 서브넷 VPC IP 를 소비**(`/20` = 4,094개에 파드 60여 + 노드 + ENI 예비 ⇒ 여유 충분하나 사이징 명시 필요) · ⚠️ **WireGuard × ENI 지원 미검증**(A-37). **연쇄 정정** = C-7 · C-8③ · C-26 · C-58① · C-74 | 2026-08-13 | 사용자 확정 + Cilium·AWS 공식문서 |
| C-83 | 🔴 **잔업 처리 원칙 = 온프렘 형상 동결 · AWS 는 신설 파일/오버레이로만.** C-77(AWS IaC 전량 신규)을 **Terraform·Ansible 밖까지 일반화**한다 — C-77 은 `aws-platform/`·`eks.yml` 만 다뤘고 **config 레포 매니페스트와 남은 온프렘 잔업**은 다루지 않았다. 🔴 **선을 정확히 그을 것** — 동결 대상은 **형상**(`overlays/onprem` 의 내용 · Ansible 기존 롤·플레이 · 온프렘 오브젝트의 구조·권한·스토리지·네트워크)이고, **앱 코드는 동결 대상이 아니다**(이미지 태그 범프 = Jenkins CD 가 하는 일 · 버그 고침은 양 사이트에 다 가야 한다 — 컬리 크롤은 C-3 상 온프렘 **상시 프로덕션**이다). 🟢 **이미 이 레포의 설계 원칙이다** — config 레포 `SITES.md`(2026-08-09 · 0-1 파생) 가 *"왜 지우기가 아니라 가르기인가"* 로 시작하고, 33트랙 전부 `overlays/{onprem,eks}` 가 이미 있다(0-1·0-4 반영 완료). **새 원칙이 아니라 잔업 전체에 일관 적용하는 것.** 🔴 **이득이 구체적이다 — 위험 4건 중 3건이 소멸한다**: ①`0-3`(kube-prometheus-stack 소유권 이전) → `overlays/eks` 에 신설 ⇒ **Prometheus 30GiB PV 를 prune 할 국면 자체가 없어진다** ②`0-23`/`0-30`(barman 경로) → 온프렘 `pg/pg/wals` 를 **건드리지 않고** AWS 쪽만 다른 경로로 짓는다 ⇒ 옛 WAL 체인 무손상 · 고아화 없음 · base backup 강제 불필요 (**즉 선행이 아니게 된다**) ③`0-14c`(워크로드별 SA) → `overlays/eks` 에만 ⇒ **`imagePullSecrets` 함정 무관**(Harbor 워크로드 41개 중 40개가 podspec 미기재). 🔴 **분리가 원리적으로 불가한 3건** = `0-8b`(지키려는 대상이 온프렘 PV 자체 · 단 `patch pv` 라 **파드 재시작 0**) · ~~`1-1`+`1-18`+`1-16`~~ ⟳ 🔴 **정정(2026-08-13 실측) — 세 건 다 위험이 아니었다.** 종전 서술 *"온프렘 PG 의 WAL 이 지금 차고 있다 = 유일한 진짜 위험"* 은 **내 오진이다**: `active=f`·`wal_status=reserved` 를 고장으로 읽었는데 **둘 다 정상값**이고, 보유 WAL 은 **16MB(1세그먼트) 고정**이었다(8/6부터 정지였다면 213MiB/h × 7일 ≈ 35GB 로 9.8GiB PVC 를 이미 터뜨렸어야 한다 — 실측 **12%**). ⇒ `1-1` **해소** · `1-18` **이미 라이브** · `1-16` **의도된 선택**. 🔴 ~~분리 불가 항목으로 남는 것은 `0-8b`(완료) 와 `0-22` 둘뿐이다~~ ⟳ **정정(2026-08-13)** — `0-22` 가 **유예 확정**(사용자 판단 = 호스트 C 로컬 소재라 감수)되어 🟢 **온프렘을 건드릴 잔업이 0건**이다. ⇒ C-83 의 동결이 예외 없이 성립한다. 🟢 **검증 게이트 = "온프렘 렌더 diff 0"** — `kustomize build <트랙>/overlays/onprem` 전후 비교. **선례 있음**: `platform/es` 오버레이 이사 때 *"onprem 렌더 33개가 바이트 단위 동일"* 을 확인했다(SITES.md). 원칙을 말로 두면 새므로 `scripts/validate.py`·PR 템플릿으로 **기계 강제**한다. 🔴 **부수 발견 = CI 가 원칙과 반대로 배선돼 있다** — `Jenkinsfile` CD 가 핀하는 경로는 `services/${name}/overlays/onprem` **하나뿐**이라 main 머지마다 온프렘 태그만 오르고 **eks 태그는 영원히 낡는다** → **A-43 신설**. 🔴 **유효기간 = 이관 전까지**(사용자 확정) — 이관 후에는 AWS 위에 쌓는 것이므로 이 제약이 사라진다. C-72(온프렘 동결)의 *"AWS 안정화까지"* 와 같은 창이다 | 2026-08-13 | 사용자 확정 |
| C-84 | 🔴 **FinOps·Operations 대시보드 = VPC-A 공개 서브넷 EC2 1대 · Docker Compose.** 배치는 C-73(VPC-C 폐기)에서 이미 정해져 있었고, 이 행이 **그 안의 형상**을 확정한다. 세부 계획서 = ⟳ 🔴 **`docs/operations-ai-aws-migration-plan.md`(2026-08-14 · 사용자 확정 · 정현·정은 작성)**. ~~`docs/aws-dashboard-ec2-deployment-plan.md`(#621)~~ 는 **superseded** — 다만 그 문서에만 있는 **확정 결정 5건이 아직 승계되지 않았다**(그 문서 머리말 참조) 이므로 **지우지 말 것.** 🔴 그리고 새 정본은 **감사 지적 미반영 상태**다(같은 머리말의 수정 목록 — 특히 학원 PG 포트 `5432`→**`15432`** · SG 인바운드 *"Cloudflare IP"*→**전체 개방** · Bedrock *"NAT 경유"*→**IGW** · *"알람 구성 안 함"*→**502 알림은 C-85 의 상쇄 조건이라 뺄 수 없다**). **형상** = `t3.medium` **x86_64** + Elastic IP + gp3 · `compose-edge`(Nginx 단일 443) / `compose-finops` / `compose-operations` + 공용 `dashboard-net` · 도메인 `finops.` `ops.mealbong.cloud`. 🔴 **x86_64 는 의도된 선택이다** — C-63(CI 서버 = `t4g.xlarge` Graviton 확정 · x86 미채택)과 **반대 방향**이고 `t4g.medium` 대비 월 $5~8 비싸지만, 사용자 판단 = *"ARM 이미지를 구울 수 있을지 불확실하니 무난하게"*. FinOps 이미지의 arm64 가용성이 미확인이라 **리스크를 돈으로 산 것**이다. 🔴 **진입 = Cloudflare 회색(DNS 전용)** — C-60 과 같은 형태다. **CF Access·Cognito 둘 다 미채택**(사용자 확정) ⇒ **인증은 오리진에서 한다: EC2 위 `oauth2-proxy` → Google + 이메일 5개 allowlist**(`--authenticated-emails-file`, 코드 0). 🟢 **자체 인증이라 오리진 우회 문제가 원리적으로 없다**(C-60 2265행 *"회색이면 우회라는 개념이 없다"*) ⇒ **SG 를 CF IP 범위로 못박을 필요가 없다**. 🟢 팀의 운영자 신원은 이미 Google 이다(현행 CF Access 구글 SSO) ⇒ 신원을 바꾸는 게 아니라 **같은 신원을 다른 문에서 쓴다**. 🔴 **`mp-account` JWT 재사용은 금지** — 대시보드 로그인이 EKS 를 지나면 **EKS 가 죽었을 때 그걸 알려주는 대시보드에 로그인할 수 없다**(C-65~C-68 이 *"보안 알림 Alertmanager 경유 금지 = 순환의존"* 으로 기각한 것과 같은 오류). 게다가 `mp-account` 는 최종사용자용이라 **앱 가입자 누구나 JWT 를 받는다**. 🔴 **대가 = 회색이라 CF WAF·DDoS 가 앞에 없다** — 5인용 관제 화면이므로 `nginx limit_req` 로 받고, 필요하면 나중에 SG 를 좁힌다(가역). 🔴 **`operations-api` 가 EKS 에서 나온다** — 라이브에 `app/mp-operations`(Deployment) + `app/operations`(ClusterIP 8011) 이 이미 돌고 있고 config 에 `services/operations/overlays/{onprem,eks}` 도 있다. compose 로 옮기므로 **C-78 A2 의 앱이 13종 → 12종**이 되고 `operations` 는 arm64 트랙(17개)에서 빠진다. 🔴 **온프렘 `services/operations` 는 그대로 둔다**(C-83). **데이터 = 제3자(학원) PG 단일** — 🔴 **포트 `15432`**(라이브 netpol 실측 `ipBlock 211.46.52.152/32 :15432`. 계획서의 *"5432 또는 제공 포트"* 는 오류) · MongoDB 는 **범위 밖**(담당자 개인 로컬 설치물 — 라이브 클러스터에 워크로드·PVC·ns 전무. 계획서 §6.1 의 K8s 리소스 retire 절차는 **우리 대상이 아니다** ⇒ C-83 우려 소멸). **추가 채택** = **GCP Workload Identity Federation**(EC2 Instance Profile → GCP SA · 장기 SA 키 미사용 · BigQuery·Monitoring·Asset·Recommender 읽기) · **Bedrock `apac.amazon.nova-micro-v1:0`**(Operations RCA · `apac.` = 교차리전 추론 프로파일) — 둘 다 **현재 사용 중/예정**이라 정본에 편입한다 | 2026-08-13 | 사용자 확정 |
| C-85 | 🔴 **내부 접근 = 로드밸런서 0개.** C-9(*"진입점 = 공개 ALB 1개만 · 내부 도구 6종은 Tailscale 로만"*)가 C-53(Tailscale 잠정 중단)으로 남긴 **공백을 메운다.** 🔴 **발단 = 사용자 지적** — *"도메인 안 붙이고 그냥 VPC IP 로 접속하면 되지 않나"* 가 맞는 방향이었고, 검토 결과 **LB 를 사지 않아도 된다.** 🔴 **먼저 오해 정정: 벽은 공개/사설 서브넷이 아니다.** 같은 VPC 안 서브넷은 로컬 라우트로 서로 통한다. 진짜 벽 = ① **ClusterIP `10.30.0.0/16` 이 VPC 에 없는 주소**(노드 eBPF 안의 가상 주소) ② **클러스터 DNS**(`*.svc.cluster.local` = CoreDNS, 클러스터 밖은 못 씀) ③ SG·netpol(정책 벽, 열기 쉽다). 🟢 **C-82(ENI)가 파드 IP 를 진짜 VPC 주소로 만들어 ①의 라우팅은 풀렸다** — 남은 건 *도달성*이 아니라 **주소 지정**(파드 IP 가 재스케줄마다 바뀐다)이다. **① 내부 도구 6종(Grafana·ArgoCD 등) = `kubectl port-forward`** — C-80 이 EKS 엔드포인트를 **public + IAM** 으로 확정했으므로 노트북에서 바로 된다. **$0 · LB 불요 · 도메인 불요.** 🟢 **지금보다 낫다** — 온프렘은 `ssh -L` + 원격 port-forward **2겹**(1913행: *"CLAUDE.md 에 절차가 적혀 있을 만큼 아프다"*)인데 AWS 는 **1겹**이 된다. **② 대시보드 EC2 → 클러스터(Prometheus·kubecost) = NodePort + 노드 사설 IP** · 노드 SG 에 **대시보드 EC2 SG 만** 허용(인터넷 노출 0). **$0.** 🟢 근거 = ⓐ **이미 쓰는 패턴**(라이브 `cost/kubecost-frontend 9090:30090`) ⓑ C-64 가 kubecost 를, 스토리지 사유로 Prometheus 도 **MNG 고정 노드**에 묶어 Karpenter 노드가 아니다 ⓒ nginx `upstream` 에 노드 3대를 두고 헬스체크하면 1대 교체는 견딘다 ⓓ **가역** — 아프면 NLB 로 올린다(반대는 불가). 🔴 **대가** = MNG 롤링 업그레이드로 노드가 **동시에** 교체되면 끊긴다 ⇒ **nginx 502 를 알림에 걸어** 조용한 실패를 막는다(관측 도구의 조용한 실패가 최악의 실패 모드다). 🔴 **`kubectl port-forward` 를 대시보드 상시 쿼리에 쓰지 말 것** — 온프렘에서 감사로그 보존창이 문서상 30일인데 **실측 52.62시간**으로 붕괴했고(`1-25`), 조사에서 **port-forward 트래픽이 바이트의 대부분**이었다. AWS 는 audit → CloudWatch Logs 가 **이미 월 ~$59 추정**(C-66)이라 *"무료"* 경로가 **$16 NLB 보다 비싸질 수 있다**. 🟢 **승급 경로는 C-82 로 깨끗해졌다** — NLB 로 올릴 때 `target-type: ip`(파드 직등록)라 노드 홉·`externalTrafficPolicy` 함정·헬스체크 포트 문제(`1-34`)가 **전부 소멸**한다. ❌ **공개 ALB 겸용은 금지** — 감사 #58 경고(*"내부 게이트웨이 netpol 이 전면 개방 + 와일드카드 SNI → scheme 을 틀리면 그대로 인터넷 노출"*). ❌ Route 53 Resolver 인바운드 엔드포인트 = **~$180/월**(NLB 의 10배)이라 검토만 하고 기각 ⟳ 🔴 **예외 1건 신설(2026-08-14 · 사용자 확정) — ArgoCD 만 UI 를 연다.** ①의 *"내부 도구 6종 = `kubectl port-forward`"* 에서 **ArgoCD 를 뺀다**(나머지 5종은 그대로). 목적 = **sync·rollback 을 브라우저에서** 하는 것. 형상은 **GitLab 과 같다**(`gitlab_cloudflared` · 2026-08-14) — 인클러스터 cloudflared 가 `argocd-aws.mealbong.cloud` 를 나르고 **차단은 Cloudflare WAF**(`argocd-aws-ip-allowlist` · 학원 IP 만)가 한다. CF Access 는 여전히 없다(ZT 결제 · C-61④ 유예). 🟢 **이 결정의 두 축은 안 깨진다** — cloudflared 는 **아웃바운드 QUIC** 이라 인바운드 포트도 LB 도 만들지 않는다 ⇒ **LB 0개 그대로** · 공개 ALB 도 1개 그대로(C-9) · **$0**. 🔴 **온프렘 `argocd-webhook-tunnel` 은 그대로 둔다**(C-83) — 그쪽은 `/api/webhook` 만 열고 UI 를 안 여는 별개 터널이고, 그 매니페스트 주석의 *"ArgoCD UI/API 는 인터넷에 노출되지 않는다(안전 설계)"* 는 **온프렘 한정**으로 읽어야 한다(안 그러면 이 예외가 그 주석을 어긴 것으로 읽힌다). 🔴 **Google SSO(dex)는 미채택** — 검토 이유가 인증 강도가 아니라 **감사**(admin 공용 계정이면 누가 sync 했는지 안 남는다)였는데 **쓰는 사람이 1명**이라 그 이유가 성립하지 않는다 ⇒ 계정 인증은 **admin 한 겹**, 바깥문이 WAF. 🟢 나중에 켜는 것은 **덧셈**이다(`configs.cm.url` 은 미리 넣어 뒀다). 🔴 **온프렘을 베끼면 깨지는 것 3** = ⓐ EKS `argocd-server` 는 **TLS 모드**(`eks_argocd` 인라인 values 에 `--insecure` 가 없다) ⇒ `https://…:443` + `noTLSVerify` ⓑ 호스트네임은 **`argocd-aws.`** — `argocd.` 는 온프렘 웹훅 터널이 소유 중이고 `argocd.aws.` 는 **Universal SSL 이 2단계 서브도메인을 안 덮는다** ⓒ 자격증명은 **ESO** 경유(GitLab 은 EC2 라 기계가 직접 읽는다). 구현 = app **MR !6**(`infra/ansible/roles/eks_argocd/`) · 라이브 검증 = 학원 회선 200 / 5G 차단 | 2026-08-13 | 사용자 확정 |
...
| C-88 | 🔴 **AWS 자생 이벤트 2종의 운반 = 앱·배치가 PG 에 직접 쓴다(안 "가").** **열린항목 ④ 를 닫고**, 종전 기본값 *"무응답 시 SQS 경유(보수적)"* 를 **폐기**한다. 대상 = `events.user.activity`(mealplan ADD_CART) · `price.anomaly.detected`(탐지 배치). 🔴 **종전 기본값의 근거가 무효로 확인됐다** — *"노출 로그를 켜면 볼륨이 수십~수백 배"* 였는데 **노출 로그(`impression_log`)는 애초에 Kafka 를 타지 않는다**(`services/mealplan/app/queries.py:186` 주석 그대로 *"mealplan 직접 write"*). 그 볼륨은 이 두 토픽과 무관하다. **실측 근거 4가지** = ① **소비자가 양쪽 다 1개** — `price.anomaly` 의 fan-out 은 구독자 팬아웃이 아니라 `INSERT ... SELECT` 한 문장 안의 **데이터 팬아웃**이라 EventBridge 규칙을 걸 대상이 없다 ② **유실 ≠ 미발송** — `price_anomaly` 행이 발행 **전에** 영속되고 `mark_published()` 가 전달 확인 후에만 `published_at` 을 찍는다(`pipelines/ingest/detect_price_anomaly.py:250` — *"미발행분은 published_at IS NULL 로 남아 재시도 대상"*) ③ **리플레이 = 배치 재실행** — `price_alert_sent` PK + 7일 쿨다운 **이중 멱등**(`consume_price_anomaly.py:65-69`) ④ 🔴 **볼륨 위계** — impression 이 추천당 **20행**을 이미 PG 직접 write 하는데(`routers.py:208·218`) ADD_CART(담기당 1건)를 큐로 빼는 것은 앞뒤가 안 맞는다. 🟢 **GRANT 무쟁점** — 실측상 로그인 롤이 `fbapp` 단일이고 그 롤이 **이미 `activity.user_event` 에 쓰고 있다**(서비스별 격리 롤은 아직 없다 — 별건). 🔴 **구현 = 덧셈** — `EVENT_SINK`(`kafka` 또는 `pg`, **기본 `kafka`**) 선택자 신설 · **온프렘 ConfigMap 변경 0** · `overlays/eks/` 에서만 `pg` (C-72·C-77 준수) · 배치는 `--emit-direct` 별도 인자. **선택자가 하나라 dual-write 가 구조적으로 불가능**하다(C-72 의 *상시 병행 미채택* 을 코드 구조로 보장). 🟢 **인프라 결정 변경 0** — Terraform·IRSA·SQS 엔드포인트·노드 사이징 전부 불변. 오히려 워크로드 2종(**−100m / −256Mi** 실측) + KEDA ScaledObject 1개 소거. 🔴 **선행 = 프론트 `session_id` 전송** — 실측 결과 `user_event.session_id` 44건이 **전부 NULL**(프론트 미전송)이라 노출↔행동 조인이 **영원히 실패**하고 양성 라벨이 **0건**이다. 컬럼이 `uuid` 타입이라 **유효 UUID 필수**. 🔴 **함께 닫히는 것** = 열린항목 ④ · 이슈 #608 | 2026-08-13 | 사용자 확정(AI 담당) |
| C-89 | 🔴 **이미지 빌드 = 사이트별 단일 아키 · 멀티아치(manifest list) 미채택 · Jenkins 은퇴 철회.** C-63 의 *"멀티아치가 강제"* 를 정정한다 — 그 근거는 *"온프렘 DR 이 x86(C-3)이라 앱 13종을 amd64 로 띄운다"* 였는데, 🔴 **하루 뒤 C-70(2026-08-12)이 DR 대기를 최후방으로 유예하고 앱 13종을 "없어지는 것" 목록에 넣었다.** 전제가 사라진 것이다. 🔴 **실측(2026-08-14)** — 온프렘 CronJob 이 쓰는 이미지는 **2개뿐**(`mp-data-pipeline` 13잡 · `mp-crawler-kurly` 1잡)이고, 배치 요약은 *온프렘 7(크롤만) / AWS 16(리파이너·컨슈머 6 + 내부배치 10)* 이다 ⇒ 양쪽이 진짜로 필요한 이미지는 **`mp-data-pipeline` 하나**이고 **`mp-crawler-kurly` 는 amd64 전용**(AWS 에선 돌지 않는다). **형상** = GitLab(arm64·EC2) → ECR → EKS ~~17종~~ ⟳ **15종**(2026-08-14 · 사용자 확정) / Jenkins(amd64·호스트 C) → Harbor → 온프렘 **크롤 2종**(+ 이관 창 동안 앱 13종 = 롤백 대상. A6 이후 축소). `mp-data-pipeline` 만 양쪽에서 굽지만 **레지스트리·태그가 갈려 충돌이 없다**(config 가 이미 `overlays/{onprem,eks}` 로 갈라져 있다). 🟢 **A-29 에서 통째로 소멸하는 것** = buildx · QEMU binfmt(러너 롤 수정 불요) · 스테이징 레지스트리+crane · 플랫폼별 2회 스캔 · 에뮬레이션 빌드시간. ⇒ `.gitlab-ci.yml` 은 현행 `Jenkinsfile` 구조의 **1:1 이식**이 된다. 🔴 **이건 편의가 아니라 게이트 문제였다** — Trivy 가 push **앞**에 설 수 있는 이유는 `docker build` 가 이미지를 로컬 데몬에 남기기 때문이고, `buildx --push` 는 build 와 push 가 한 몸이라 그 성질이 깨진다(우회하려면 스테이징 레지스트리 경유 + 다이제스트 복사가 필요하다). **즉 멀티아치를 포기하는 것과 차단 게이트를 지키는 것이 같은 결정이다.** 🔴 **C-2 정정 = Jenkins 은퇴 철회 · 쌍방 영구 운영.** 근거 = 온프렘이 amd64 로 **상시** 남고(C-3·C-70) 크롤이 프로덕션이므로 amd64 공급원이 계속 필요하다 + **학습 목적**(사용자 확정 — *"실제 운영이면 문제지만 뭐가 문제인지 파악하고 있다"*). 🔴 **C-70 정정 = "없어지는 것"에서 Harbor 제외.** ⇒ 🟢 **C-70 의 파생 숙제(온프렘이 ECR 에서 직접 당기기 위한 ECR 읽기 IAM 키)가 소멸**하고 **A-31(ECR→Harbor 미러)·열린 항목 ㉑ 이 불필요**해진다 — 온프렘은 지금처럼 Harbor 에서 당긴다. 🕐 **유예 = config 핀 분담**(Jenkins=`onprem` / GitLab=`eks` · A-43 이 붙인 eks 핀 되돌리기) — **이관 후**(사용자 판단 = *"지금 급한 건 마이그레이션"*). 🔴 **감수하는 것 3** = ① 파이프라인 정의가 영구히 2벌(`Jenkinsfile` + `.gitlab-ci.yml` 에 같은 카탈로그) — 🟢 **서비스 정본은 AWS**(사용자 확정)라 온프렘 쪽이 뒤처져도 정본이 흔들리지 않는다 ② 두 CI 가 같은 config 레포에 push(위 유예) ③ **호스트 C 가 영구 의존물**(디스크 27.9GB 단일 FS · 백업 없음 · 사용자 로컬 머신 — `0-22` 유예와 같은 감수). 🔴 **범위 밖** = 대시보드 EC2(C-84)의 amd64 이미지 공급원은 **이 결정이 다루지 않는다**(사용자 확정 = 새 안건으로 열지 않음 · 온프렘은 대시보드가 필요 없다)  ⟳ 🔴 **정정 2건(2026-08-14 · A-29 실행 중 실측)**: ① ~~17종~~ → **15종**. `mp-data-pipeline` 은 **EKS 에서 안 쓴다**(C-20 = 파이프라인 2종 온프렘 잔류 · A5 가 맨 뒤) ⇒ GitLab 이 구울 이유가 없다. ② **`operations` 제외**(사용자 확정) — C-84 대로 **VPC-A 공개 서브넷 EC2 의 docker compose** 로 나간다(미구현). 🔴 **파생 미결 = 그 이미지의 amd64 공급원이 없다.** 그 EC2 는 x86_64(C-84, 의도된 선택)인데 GitLab 은 arm64 만 굽고 Jenkins 는 Harbor 로만 올리며 공개 서브넷 EC2 에서 LAN Harbor(`192.168.0.10`)는 닿지 않는다. C-84 구현 때 ⓐ GitLab 에 amd64 전용 잡(QEMU) / ⓑ 그 EC2 가 소스에서 직접 빌드 / ⓒ Jenkins 가 ECR 에도 push(온프렘에 AWS 자격증명 발생 — 가장 비싸다) 중 하나를 고를 것. 🟢 **A0.5 완료 판정 실증(2026-08-14 08:21 KST)** — 파이프라인 19(main·`SERVICES=all`) **26/26 성공** ⇒ **ECR 16/18 리포에 `linux/arm64` 적재**(전수 확인). 빈 2개 = `mp-crawler-kurly`·`mp-data-pipeline` = 온프렘 잔류. **18/18 이 목표가 아니다.** | 2026-08-14 | 사용자 확정 |
...
```
   ...
   ⇒ C-9 를 "공개 진입점은 **1개만**"으로 읽고, 그 1개를 ~~NLB~~ **ALB** 로 구현한다.
   ⛔ **C-60(2026-08-11)으로 대체** — 당시 결론은 NLB 였다. "진입점 1개" 라는 읽기 자체는 유효하다.
```
...
| **A''. NLB TCP:443 패스스루** | ①⑤ | ⛔ **C-60 으로 대체(2026-08-11)** — 당시엔 채택이었다 |
...
```
   ...
   ⇒ ~~NLB = 이 형상 **그대로 유지**~~
   ⛔ **C-60(2026-08-11)으로 대체** — 현행 진입은 **ALB**(ACM 종단 · AWS WAF 부착)다.
...
✅ **해소(2026-08-13)** — `0-14c`·`0-16` 의 절차 서술을 IRSA 로 고쳤다. 🔴 **나머지 15곳은 손대지 않았다** — 전부 *"왜 Pod Identity 를 기각했나"* 의 비교표·근거·이력이라 치환하면 결정 근거가 소멸한다. 종전 부채 서술 = **C-30 으로 IRSA 가 확정**됐으므로 실제 스텝은 *association 생성* 이 아니라 *어노테이션 + 롤 신뢰정책* 이다. 순서(①SA→②시크릿분리→③신원부착→④env제거)는 그대로 유효하다.
...
| **확정 소계** | | **$722.80** | 83.4% |
| 🆕 **대시보드 EC2** (C-84) | `t3.medium` x86 1대 + EIP + gp3 | **~$45** ⚠️ 추정 | — |
...
⟳ **2026-08-13 C-84·C-85 반영** — 🆕 **대시보드 EC2 ~$45**(`t3.medium` $38 + EIP + gp3, 추정) 를 행으로 추가했다. 🔴 **확정 소계에는 넣지 않았다** — 단가 미검증이고 `t4g.medium` 로 가면 $5~8 내려간다(C-84 는 x86 확정).
🟢 **내부 LB 는 넣지 않았다 — C-85 가 로드밸런서를 0개로 확정**했다. 종전 초안의 *"내부 NLB ~$16/월"*(1457행 예상치)이 **집행되지 않는다.**
...
##### 🔴 C-69 의 근거 — 두 백업을 가르는 축은 **위치가 아니라 방법**이다 (2026-08-12)

**발단 = 사용자 질문** — *"AWS 에서도 온사이트 대비해서 백업이 필요한 거야? 오프사이트는 물리(Base+WAL)고 온사이트는 논리라 용도가 다르다는 거지?"* 🟢 **그 판단이 맞고, 이름이 그걸 가리고 있었다.**

⟳ **이 절이 정정하는 것** — 2026-08-12 세션 초반에 *"온사이트 백업은 지금 규모(893 MB)에 불필요하니 버킷을 빼자"* 는 권고가 있었다. **틀렸다.** ⓐ 이미 존재하는 트랙이고(아래 실측) ⓑ **C-18 이 이미 "다른 버킷" 을 결정**해 두었으며 ⓒ 판단축이 규모가 아니라 **방법**이다.

```
  data/mp-pg-onsite-dump   CronJob  0 4 * * *  (Asia/Seoul)
    initContainer :  pg_dump -Fc -Z6 -f $OUT        ← 논리 덤프
    container     :  mc cp → mp-pg-onsite/foodbudget/ · mc rm --older-than 7d
  실측 크기 318 MiB (MinIO 내용물의 35%)  /  PG 실사용 893 MB (PVC 20Gi 의 5%)
```

| | **barman-cloud (물리)** | **`pg_dump` (논리)** |
|---|---|---|
| 담는 것 | 데이터 파일 바이트 + WAL | SQL 문장 |
| 복구 단위 | 🔴 클러스터 통째로 | 🟢 **테이블 하나만도** |
| 시점 | 🟢 **PITR — 임의 시각** | 🔴 덤프 시각 하나 |
| PG 메이저 버전 | 🔴 **같은 메이저에만** | 🟢 **버전을 넘는다** |
| 손상 | 🔴 **손상도 충실히 복사** | 🟢 읽다가 드러난다 |
| RPO | **5분** (C-40) | 24시간 |

🔴 **그래서 AWS 에서도 유지한다 — 위치 때문이 아니라 물리 백업이 원리적으로 못 하는 일이 있어서다.** 특히 ②(메이저 업그레이드)는 **C-15(PG 자체운영)의 직접적 대가**이고, 그 절차 자체가 아직 미완이다(§상시 *"PG·ES 마이너 버전 패치 프로세스 신설"*).

🟢 **이관이 이 트랙을 개선한다** — §상시의 경고 *"온사이트 덤프 저장소는 DR 이 아니다 · MinIO 단일 replica · worker-b2 고정 · **b2 디스크 사망 = 7일치 소멸** · 백업 2중화로 계산하지 말 것"* 은 **노드 로컬 스토리지 때문**이었다. S3 로 가면 이 전제가 사라져 **비로소 백업 2트랙으로 셀 수 있다.**

🔴 **개명 시점이 지금인 이유** — 이 버킷은 **아직 없다**(현재 MinIO). Object Lock 을 걸고 나면 **객체 만료 전까지 버킷을 못 지운다**(C-57·A-5). 이름을 고치는 값이 지금은 $0, 생성 후엔 편도다.

**포기하는 것** = C-18 원문·`2-8`·`1-20`·`0-23` 에 남은 `onsite` 표기를 함께 손봐야 한다(이 커밋에서 처리). 리네임 대상은 **AWS 버킷 이름 하나**이고, 🔵 **온프렘 실물(`mp-pg-onsite-dump` CronJob · MinIO 버킷)은 건드리지 않는다** — §명명 규칙의 *"기존 실물 이름은 그대로 참조"* 예외.
...
| 3 | **스토리지 자동 확장** | WAL PVC **10Gi** · ⟳ 🔴 **정정(2026-08-13)** — 종전 *"361 MB/일 → 약 27일치"* 는 **S3 업로드량(압축 후)을 PVC 사이징에 쓴 오류**다. PVC 에 쌓이는 것은 **압축 전 세그먼트**라 실측 **4.99 GiB/일** ⇒ 아카이빙이 막히면 **약 2일**(13배 낙관이었다). 차면 **PG 가 선다** | 🟢 **알림 3중 라이브**(`1-18`) |
...
```
   ...
   ⇒ 우리 5인 = **Tailscale 요금 $0** · ~~EC2 t4g.nano $1.90~~ → 🔴 **합계 월 $0**
     (C-55 가 AWS subnet router 를 미채택 ⇒ 그 EC2 가 없다. 게다가 C-53 으로 🕐 착수 중단)
```
...
| ~~D-ing~~ | ~~AWS 유입~~ | → C-26 → 🔴 **C-60 으로 정정**(2026-08-11 · ALB). 아래는 당시 표기 — Cloudflare(주황) → **NLB TCP:443 패스스루** → Istio GW · ALB 미채택 | ✅ |
...
| ~~**D8-r**~~ | ~~Prometheus replicas (AWS 쪽)~~ | → **C-86 으로 확정** (2026-08-13) — **`1` 유지.** 근거·대가(🔴 C-67 논거 한 다리 소멸 · `gp3-retain` 이 유일 방어선)는 C-86 행 | ✅ |
...
✅ **해소(2026-08-13 · 사용자 확정) = `1`** → **C-86**. 위 표의 *"2배가 되는 셋"* 을 **A0 노드 사이징에 넣지 않는다**는 뜻이다.
🔴 **그 대가로 C-67(AWS Backup 미채택)의 논거 한 다리가 없어진다** — 그 행은 *"Prometheus 는 `replicas 2` 가 더 낫다"* 를 근거로 썼다.
⇒ Prometheus 시계열의 방어선은 **`gp3-retain` 하나**(0-8e · config #163)로 좁아진다. **그것은 즉사 방지선이고 백업이 아니다.**
🟢 다만 **가역**이다 — `replicas` 는 CR 필드이고, 함께 필요한 것(nodeSelector 제거·zone TSC)은 이미 eks 오버레이에 있다.
...
⚠️ **대역폭은 논점이 아니다** — 터널을 건널 양은 **4.7 MB/일**인데, 온프렘은 이미 barman WAL 로 **361 MB/일**(77배)을 인터넷으로 보내고 있다(🟢 **여기선 361 이 맞는 값이다** — 전송량은 압축 후다). 위험은 속도가 아니라 **단절 내성**이다.
...
> 반영 범위 = **C-1 ~ C-89** (2026-08-14 — 🔴 **이미지 = 사이트별 단일 아키 · Jenkins 은퇴 철회**(C-89) · 2026-08-13 — 🔴 **Cilium ENI 모드**(C-82 → 파드 CIDR 소멸·`target-type: ip`) ·
> **VPC 2개**(C-73 · VPC-C 폐기) · **서브넷 3티어**(C-74 · 도구 티어 삭제) · **CloudFront 미채택 확정**(C-75) ·
> **Blue-Green 골자**(C-76) · **AWS IaC 전량 신규**(C-77) · **이관 순서 A0~A6**(C-78) · **S3 계층 전환**(C-79) ·
> **비대화형 접근**(C-80) · 감사로그 보고서(C-81) · 🔴 **잔업 처리 원칙 = 온프렘 형상 동결**(C-83). 이전 = 2026-08-12 — 🔴 **이관 원칙 = 온프렘 동결·덧셈만**(C-72) · 🔴 **온프렘 목표 = 크롤 단독**(C-70) · **CI 공개 서브넷·NAT 미채택**(C-71) · **PG 백업 2트랙·`mp-pg-dump-ap2` 개명**(C-69) · 2026-08-11 — **CI/CD 형상 확정**(C-61·C-62·C-63) · **NACL 미채택**(C-58) · 🔴 **계정 3개 → 1개**(C-57) · **Interface 엔드포인트 3종 채택**(C-56) · PG 복제 하이브리드(C-55) · DR 전환 2단(C-54)).
...
════ AWS Organizations (남의 조직 o-8gviph817d · mgmt = aws_3@etechcloud.net) ═══════
 ...
 │ ┃  │   파드 = **노드 서브넷 VPC IP** (C-82 ENI) — 별도 파드 CIDR 없음      ┃ │
 │ ┃  │   max-pods 58/노드 (4 ENI × 15 IPv4) vs 실측 36 → 타입 강제 없음      ┃ │
 ...
 │ ┃  │  ❌ 밖으로 나가는 경로 없음 │  │  ❌ 동일                  │              ┃ │
 │ ┃  ├───────────────────────────┤  ├───────────────────────────┤              ┃ │
 │ ┃  │ ❌ 도구 티어 삭제 (C-74)  │  │ ❌ 동일 — 대역만 예비     │  Tailscale   ┃ │
 │ ┃  │   `10.10.16.0/20` 예비    │  │                           │  미채택 C-55 ┃ │
 ...
 │       → pytest → sonar(**aarch64 zip**) → build(**arm64 단일** C-89) → Trivy → ECR │
 ...
 │ ❌ **VPC-C 폐기 (C-73)** — 입주자 0 (C-64 로 kubecost 인클러스터 복귀)          │
 │ ┏━ 이상징후·FinOps 대시보드 → **VPC-A 공개 서브넷** (C-73) ━━━━━━━━━━━━━━━┓ │
 │ ┃   FinOps 대시보드 (선생님: AWS Budgets 대체) · 🔴 kubecost 는 C-64        ┃ │
 │ ┃   **EC2 `t3.medium` x86 + EIP** · Docker Compose 3파일 (C-84)             ┃ │
 │ ┃   Nginx 443 단일 ← **CF 회색** · 인증 = oauth2-proxy→Google (오리진)      ┃ │
 │ ┃   `operations-api` 도 여기 (EKS 에서 나옴 ⇒ A2 앱 12종)                   ┃ │
 │ ┃   → 클러스터 조회 = **NodePort + 노드 사설 IP** (C-85 · LB 0개)           ┃ │
 │ ┃   → 제3자(학원) PG `211.46.52.152:15432` · GCP WIF · Bedrock nova-micro   ┃ │
 │ ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛ │
 │                                                                              │
 │  [VPC 밖 · 리전 관리형]                                                       │
 ...
 └──────────────────────────────────────────────────────────────────────────────┘
  ...
  CIDR 비충돌 (C-8③ · C-33 · C-34)
    AWS    VPC-A 10.10/16 · VPC-B 10.11/16   (VPC-C 폐기 · C-73)
           🔴 파드 = VPC IP (C-82 ENI · 별도 파드 CIDR 없음) · svc 10.30/16
...
| ~~②~~ | ~~**VPC-C 용도 확정**~~ | → **C-73 으로 해소**(2026-08-13) — **VPC-C 폐기**. 대시보드는 VPC-A 공개 서브넷 | ✅ |
| ③ | 🔴 **온프렘의 S3 쓰기 권한** — 온프렘은 EKS 밖이라 **IRSA 불가** | 정적 IAM 키가 생기면 **이 설계의 유일한 보안 후퇴** | 🔴 미착수 (C-44 로 **더 중요해졌다** — 이제 크롤러가 직접 쓴다) |
| ~~④~~ | ✅ **해소(2026-08-13, C-88)** — **안 "가"(PG 직접) 확정.** 종전 기본값 *"무응답 시 SQS 경유(보수적)"* 는 **근거 무효로 폐기**(노출 로그는 Kafka 를 타지 않아 볼륨 논거가 이 두 토픽과 무관). ~~PG 직접 쓰기 vs SQS 경유~~ | 🟢 어느 쪽이든 Kafka 는 제거된다 | ✅ **닫힘** (구 #585·#608) |
...
| ~~🆕⑱~~ | ~~조직이 우리 계정에 건 SCP~~ | 막혀 있으면 무너지는 것 = `m7g` 계열 · EKS · `ap-northeast-2` | ✅ **해소(2026-08-13)** — 사용자가 **승인 받음** ⇒ 전제 유지 · A-24 닫음 |
...
| ~~🆕㉑~~ | ✅ **해소(2026-08-14, C-89)** — Harbor 잔류 + Jenkins 유지로 **미러링이 필요 없다**(온프렘이 Harbor 에서 그대로 당긴다). 종전: **ECR → Harbor 미러링 경로** — §1 은 *"Harbor = ECR 미러(DR 이미지 공급)"* 라는데 **계획 0건** | 온프렘 DR 이 뜨려면 앱 13종 이미지가 **온프렘에 amd64 로** 있어야 한다. 후보 = skopeo CronJob / CI 양쪽 push / Harbor proxy-cache | 🔴 A-31 (2026-08-11 신설) |
| ~~🆕㉕~~ | ~~**`mp-pg-dump-ap2` 보존 정본이 갈렸다**~~ → 🟢 **해소(2026-08-13, 사용자 확정) = C-79 의 190일 · 라이프사이클이 지운다.** 🔴 **CronJob 의 `mc rm --older-than 7d` 를 eks 오버레이에서 걷는다**(안 걷으면 7일에 지워져 라이프사이클이 **발동조차 못 한다**) · 🔵 **온프렘은 7일 유지**(C-83 동결). 근거 = C-79 의 판단축이 *"라이프사이클을 설계했다는 증명"* 이고 **논리 덤프가 그 스토리의 주 전시물**인데, 7일이면 **Standard-IA 최소 30일에 도달조차 못 한다.** ⇒ **A-49** · 종전 근거는 아래 | 보존표(§lifecycle 행) = **7일**(CronJob 이 `mc rm --older-than 7d` 로 지운다) ↔ **C-79**(2026-08-13) = Std → IA 30-90d → Glacier IR 90-180d → **만료 190일**(라이프사이클이 지운다). 🔴 **숫자만 다른 게 아니라 주체가 다르다.** 정하기 전에 업로드 컨테이너를 재작성하면 **둘 다 지우거나 아무도 안 지운다.** 🔴 이게 정해져야 `2-8`·`1-20`(A4)이 시작된다. 판단축 = *"라이프사이클을 설계했다는 증명"*(C-79)이면 190일이 맞고, 비용·단순성이면 7일이다 | 🔴 **미결 (2026-08-13 신설)** |
| ~~🆕㉖~~ | ~~**`mp-pg-onsite-minio` + `onsite-backup.yaml` 을 eks 에서 걷을지 / S3 로 살릴지**~~ → 🟢 **해소(2026-08-13, 사용자 확정) = eks 오버레이에서 `suspend: true`.** 걷어내면 **A4 에 다시 만들어야** 하고, 두면 **매일 실패 알림**이다. suspend 는 둘 다 피하면서 **A4 에 한 줄로 켠다.** ⇒ **A-49** · 종전 근거는 아래 | C-69 로 *"2트랙 유지"* 는 정해졌으나 **재작성 시점이 A4** 다. 그때까지 eks 렌더에 **존재하지 않는 MinIO 를 가리키는 CronJob** 이 남아 **매일 실패 + 알림**이 된다(C-18 로 MinIO 는 삭제된다). ⇒ ⓐ eks 오버레이에서 잠시 걷기 ⓑ `suspend: true` ⓒ A4 까지 방치(알림 감수) 중 택일 | 🟡 **미결 (2026-08-13 신설)** |
...
```
   ...
   │      VPC 2개(C-73) · 3티어 서브넷 × 2AZ(C-74) · 보안그룹            │
...
| **0-22** | Jenkins 백업 | ⏸ **유예 확정**(2026-08-13 · 사용자 판단 = 호스트 C 로컬 소재라 감수) — **A0.5 의 선행이 아니다.** 기록: #565 머지 · S3 버킷 미생성 |
| **0-11c** | 죽은 키 3개 삭제 | #570 검출 자동화 완료 · 🔴 **사람이 `kubectl delete`** (0-11 백업이 끝나 선행조건은 해소) |
| **1-6** | ⟳ 이미지 아키 (**멀티아치 아님** — C-89) | #577(하드코딩 2곳 제거) + #584(Jenkins buildx + 호스트 QEMU, IaC) · **1-E 전수 실측으로 "스택 전부 ARM 지원" 검증 완료** → ⟳ 🔴 **재정의(2026-08-14, C-89)** — manifest list 를 만들지 않는다. GitLab CI 가 **arm64 단일**로 17종을 ECR 에 올리면 끝이고, 온프렘 amd64 는 **Jenkins 가 계속 공급**한다(은퇴 철회). ⇒ 이 항목은 **A-29 의 일부**로 흡수된다 |
...
```
   ...
   0-1 · 0-4 · 0-2 · 0-3c · 0-5 · 0-6 · 0-7 · 0-8 · 0-8c · 0-9 · 0-10 · 0-16 · 0-17 · 0-18 · 0-27 · 1-31
                                          🟢 **config 반영 완료** (2026-08-13 실측 정정 · 16건)
     ⟳ 종전 표기 *"브랜치 8개 존재 · 미머지"* 는 **틀렸다** — 그 계획(#578)은 이미 실행됐다.
       잎 6개(L1·L2·L3·L4·L5·L7) 전부 `ahead=0` = config main 에 있고, `services/*/{base,overlays}` ·
       `bootstrap/{argocd,eso}` 가 실재한다. **0-4 컷오버 3단계 전부 완료** — 1단계 config#146 ·
       **2단계 app#582** · 3단계 config#147. 라이브 = `mealplanning-root` → `argocd/overlays/onprem` Synced.
     🔴 **`- [x]` 로 닫지는 않는다** — 매니페스트가 준비된 것이고 **EKS 실적용은 컷오버(C-78 A0~A2)** 때다.
       `0-16` 은 특히 *선언*만 들어갔다(정적 키 제거는 AWS 오버레이에서만 · C-30).
     🔴 **판정은 브랜치 존재가 아니라** `gh api compare/main...<브랜치> -q .ahead_by` **로 한다** —
       남아 있는 잔재 브랜치 4개를 "미머지"로 읽은 것이 이 오류의 원인이었다(삭제 권고).
                                          → ArgoCD 뿌리 IaC화는 #581·#582 로 2단계까지 진행
   0-19                                   AWS 쿼터 증액 — 🟢 **게이트 해제**(2026-08-13 · 사용자: 쓰려는
                                          사양이 조직 한도 내로 확인됨). 대상 = vCPU · EIP · **ALB**(C-60)
   #585                                   AI·알림 담당 답변 (08-12 기한)
```

### ⟳ 🔴 아직 아무도 손 안 댄 것 — ~~차단 8건이 먼저다~~ **이 블록은 stale 이다 (2026-08-13)**

> 🔴 **아래 5줄을 "지금도 막혀 있다" 로 읽지 말 것.** 실측(2026-08-13):
> ① `0-1`~`0-4` = **config 반영 완료**(위 🟠 절의 16건에 포함 · eks 오버레이 **37/38 트랙**. 빠진 1개는 `services/cloudflared` = **설계상 정상**[C-5 = 온프렘 DR 전용])
> ② `0-6` · ④ `0-27` = 같은 16건에 포함 · ③ `0-19` = **게이트 해제** · 🆕 **A-24(SCP)도 해제** ⇒ **조직 측 조건은 없다**
> ⑤ `0-28` = 매니페스트만 · app **#593** 머지됨
> 🔴 **그래서 지금 남은 가장 큰 덩어리는 이 목록에 없다 — Terraform `aws-platform/`(AWS 리소스 **0건**) + Ansible `eks.yml`(**파일 부재**)** 이다(A-7 · C-77).
> 아래는 2026-08-10 시점의 기록으로 남긴다.
...
## 🔴 이관 순서 — A0 ~ A6 (C-78 · 2026-08-13 확정)

> **이 절이 실행 순서의 정본이다.** Phase 0/1/2 는 *"무엇이 준비돼야 하는가"* 의 목록이고,
> 여기는 *"무엇을 어떤 순서로 세우는가"* 다. 둘은 축이 다르다.

### 🔴 K8s 이전(P0~P4)과 결정적으로 다른 한 가지

K8s 이전은 **P1(앱) → P2(데이터)** 로 나눌 수 있었다. 앱이 클러스터로 옮겨간 뒤에도
구 데이터 티어(`fb-data` VM `192.168.0.8`)를 **같은 LAN 에서 계속 봤기** 때문이다.

🔴 **AWS 는 그게 불가능하다** — **C-41(사이트 간 상시 연결 없음 · 양쪽이 S3 만 본다)** 이라
EKS 앱이 온프렘 PG 를 보는 중간 상태가 **존재할 수 없다.** ⇒ **앱과 DB 를 한 창에서 옮긴다.**

```
A0  기반          네트워크(VPC·서브넷·SG·엔드포인트) · EKS · 노드그룹
                  🔴 CNI 교체(vpc-cni 미설치 → Cilium ENI · C-82) · kube-proxy 애드온 제거
                  EBS CSI · 오퍼레이터(CNPG·ECK·cert-manager·ESO·Argo Rollouts)
                  Istio + Gateway API · kube-prometheus-stack + LGTM · ArgoCD · Karpenter
                  ← 트래픽 0 · 데이터 0.  **실패해도 잃을 것이 없는 유일한 구간**
                  대응: K8s P0        IaC: aws-platform/ (Terraform) + eks.yml (Ansible)

A0.5 CI (GitLab)  🆕 **2026-08-13 신설 · 사용자 확정.** VPC-B · EC2 1대(GitLab Omnibus + SonarQube
                  docker 동거 · C-38·C-59) · 러너 = 같은 기계 privileged DinD(C-61①)
                  부트스트랩 = **Ansible over SSM**(C-61⑥ · 🔴 키페어 없음 = C-37)
                  🔴 **여기서 ECR 18리포에 arm64 이미지를 채운다**(`1-6` · C-29 Graviton)
                  ← 🔴 **A1 이 이것을 요구한다**: ES 파드가 우리 이미지
                     `mealplanning/mp-elasticsearch-nori` 를 쓰고 `mp-pgsync` 도 우리 것이다
                     ⇒ CI 없이 A1 을 시작하면 데이터 티어가 `ImagePullBackOff` 로 선다
                  대응: (K8s 이전에 대응물 없음 — 온프렘 CI 는 호스트 C 에 이미 있었다)
                  IaC: aws-platform/ 신규 `.tf`(VPC-B·EC2) + Ansible 신규 플레이(`gitlab.yml`)
                  🔴 **`0-22`(Jenkins 백업)는 선행에서 뺀다** — 정본은 *"GitLab 이관 전 반드시"* 라고
                     적었으나 **사용자 판단으로 유예**(2026-08-13): 호스트 C 가 로컬 소재라 소실
                     위험을 감수한다. 🔴 **감수한 것을 명시한다** — 호스트 C 가 죽으면 Jenkins
                     형상(잡·자격증명·웹훅)이 소실되고, **GitLab 이 다 서기 전까지 Jenkins 가
                     유일한 CI** 다. 항목은 지우지 않고 남긴다(A0.5 완료 후 무의미해진다).

A1  데이터 티어    CNPG **빈 클러스터**(인스턴스 2 · AZ 당 1) · ECK 3노드 · ElastiCache Valkey
                  스키마 DDL 적용(`docs/prd/schema-production.sql` — 멱등)
                  🟢 **여기서 페일오버 promote 를 실증한다** — 데이터가 없어 망가뜨려도 되고,
                     이 클러스터 자체가 리허설이라 **`1-9`(리허설 클러스터 부재)가 자동 해소**된다
                  🔴 barman·논리덤프는 여기서 하지 않는다 (A4)

A2  앱 + 검증      앱 13종 배포 · `aws.mealbong.cloud`(회색 → ALB)로 **내부 검증**
                  🔴 여기서 잡아야 하는 것 = FQDN netpol 9개(A-39) · OAuth redirect URI(A-40)
                     · 이미지 arm64 전량 · ES 초기 색인 · Rollout blueGreen 2종(C-76)
                  🟢 온프렘은 `app.mealbong.cloud` 로 계속 서비스 중 — **무영향**
                  🔴 **도메인을 새로 파지 않는다** — 서브도메인 레코드 1개 추가일 뿐이고,
                     ACM 은 DNS 검증이라 온프렘이 그 호스트를 서비스하는 중에도 발급된다

A3  🔴 데이터 이전 + 컷오버   ← **한 창에서 끝낸다 · 다운타임 5~10분**
                  온프렘 쓰기 중단 → `pg_dump -Fc -Z6`(893MB → **318MiB** 실측) → S3 → `pg_restore`
                  → **ES 는 PG 에서 재파생**(8,963 docs · 13.9MB · 분 단위) → Redis 는 **빈 채로 시작**
                  → 스모크 테스트 → DNS: `app` 을 회색 + ALB 로 전환
                  🟢 **롤백 = 레코드 하나 되돌리기**(주황 + 터널 · TTL 60초)
                  🔴 barman `bootstrap.recovery` 를 쓰지 않는다 — *생성 시점 1회만 유효*라
                     틀리면 Cluster 재생성인데, 빈 클러스터 + `pg_restore` 는 그 경로를 안 밟는다
                  대응: K8s P2 + 컷오버

A4  안정화         관측·알림 9종 확인 · 🔴 **barman → S3** · 논리 덤프 CronJob(C-69 2트랙)
                  · HPA/KEDA · S3 라이프사이클(C-79) · kubecost(C-64)
                  대응: K8s P3

A5  파이프라인      크롤 S3 업로드(#593) · SQS/리파이너 · **온프렘 PG → AWS PG 증분 적재**
                  🔴 **맨 뒤다**(사용자 지시) — 서비스 전체가 안정적으로 도는 것을 확인한 뒤
                  🟢 미뤄도 안전한 근거 = 온프렘 크롤이 계속 돌아 **온프렘 PG 에 쌓이므로 원본이 남는다**
                     ⇒ 나중에 증분 적재로 회수 가능. *"재생산 불가"* 는 **아무도 안 받는 경우**에만 성립한다

A6  온프렘 축소     C-70(크롤 파이프라인 단독) — 🔴 **A4 완료가 절대 선행**
                  ← 그때가 **원본이 사라지는 시점**이다
                  대응: K8s P4
```

### 🟢 백업(A4)을 컷오버 뒤로 미뤄도 되는 근거

```
A3 직후 AWS PG 를 통째로 잃어도  →  온프렘 PG 가 읽기 전용으로 그대로 살아 있다 (C-72)
                                 →  원본이 있으니 pg_dump 를 다시 뜨면 된다
```
즉 **백업의 목적(원본 상실 대비)이 온프렘으로 이미 충족**돼 있다.
🔴 **그래서 A6 를 A4 없이 하면 안 된다** — 순서가 안전장치다.

### 각 단계의 IaC 소관 (C-77)

| 단계 | Terraform `aws-platform/` | Ansible `eks.yml` | config `overlays/eks/` |
|---|---|---|---|
| A0 | VPC·서브넷·SG·엔드포인트·EKS·노드그룹·ALB·ACM·WAF·IRSA | CNI·CSI·오퍼레이터·Istio·ArgoCD·Karpenter | — |
| **A0.5** | **VPC-B(서브넷·IGW·RT·SG) · EC2 1대 · EBS · 인스턴스 프로필(SSM+ECR push)** | 🔴 **`eks.yml` 이 아니다 — 신규 `gitlab.yml`**(`connection: aws_ssm`) | — |
| A1 | ElastiCache · KMS | — | CNPG·ECK CR |
| A2 | — | — | 앱 13종 · netpol · Rollout |
| A3 | — | — | — (운영 작업) |
| A4 | S3 버킷·라이프사이클(C-79) | — | barman ObjectStore · 덤프 CronJob |
| A5 | SQS·DLQ·IAM(#593 스택 재사용) | — | 리파이너·KEDA |
| A6 | — | — | 온프렘 오버레이 축소 |

🔴 **A3 는 IaC 가 없다** — 사람이 순서대로 실행하는 운영 작업이므로 **런북이 산출물**이다(`1-9` 파생).

---

## Phase 0 — 이게 끝나야 AWS 착수

### 🔴 0-0. 처리 원칙 — 어디서 고치는가 (C-83 · 2026-08-13 확정)

> **이 절을 먼저 읽는다.** 아래 모든 항목의 *무엇을* 이 아니라 **어디서** 를 이것이 정한다.
> 🔴 **"온프렘에서 고치면 되겠지" 로 읽으면 안 된다** — 대부분은 `overlays/eks` 에만 넣는다.
> 근거·예외·검증 게이트 = **§0.1 C-83**. 구조 정본 = config 레포 `SITES.md`.
...
| 🆕 **AWS 전용 신설 — PR 대기** | **1** | `0-8e`(config **#163** 미머지 — gp3 SC 2종 신설 + 재파생 불가 데이터를 `gp3-retain` 에 배선 · `validate.py` 검사 추가) | `overlays/eks` |
...
| 🟢 **온프렘 필수 · 무해** | ~~1~~ **0** | ~~`0-22`~~ → ⏸ **유예**(2026-08-13 사용자 판단) ⇒ 🔴 **온프렘을 건드릴 잔업이 0건이 됐다** — C-83(동결)이 완전해진다 | — |
...
| ⚪ 무관 | **1** | `0-21`(문서) | — |
| 🆕 신규 | **8** | **`A-43`** Jenkins CD eks 핀 · **`A-44`** ENI SG 단위 · **`A-45`** 파이프라인·pgsync 핀 구조 · **`A-46`** ECR 네이밍 통일 · **`A-47`** IRSA 롤 3종 · **`A-48`** 온프렘 전용 패치 전수 · **`A-49`** platform/pg eks 2건(미결 ㉕·㉖ 집행) · **`0-8e`** 기본 SC 가 여전히 `Delete` | 앱·config 레포 · Terraform |

🔴 **"머지됨" 6건이 `[ ]` 인 이유 = `0-4b`(2026-08-13 신설)** — `AppProject/platform` 이 그것들을 아직 허용하지 않아 **EKS 에서 sync 가 거부된다.**
그 선행이 어디에도 항목으로 없어서 표와 체크박스가 어긋나 보였다. 🟢 **`0-4b` 는 config `bootstrap/argocd/overlays/eks` + 신설 `eks.yml` 로 끝난다**(온프렘 무수정).
...
아래는 그 순서를 강제한 원래 근거다 —
라이브 PVC **21/21 이 `openebs-lvm`(reclaimPolicy `Delete`)** 이고 그 안에 `data/pg-{1,2}`(+`-wal`) ·
`observability/prometheus-…-0` 30GiB · **`app/mp-ranking-model`(`ranker.pkl`)** 이 들어 있다.
소유권 이전은 ArgoCD 가 prune 할 수 있는 유일한 국면이므로, **Retain 이 먼저 걸려 있어야 안전망이 성립한다.**

🔴 **"AWS 먼저 · 온프렘 나중" 칸을 폐기한 근거** (2026-08-13 · 작업 세션 실측) — ⓐ **C-83 이 앱 코드를 동결 대상에서 뺐다**. ⓑ 그리고 분리가 **CD 설계상 불가능**했다: `TARGETS` 가 `git diff` 변경 감지 기반이라 `#564` 가 건드린 5개는 소스 미변경으로 **앞으로도 빌드되지 않고**, `SERVICES=` 강제 빌드하면 `A-43` 적용 후 **양쪽이 동시에** 핀되고, eks 핀만 손으로 최신 sha 로 바꾸면 **그 sha 로 빌드된 이미지가 없다**(현 eks 핀은 낡았지만 *실재하는* 빌드 sha 라는 점이 중요). ⓒ 🔴 **분리를 위한 CI 파라미터(`PIN_SITES`)는 만들지 않는다** — **eks 오버레이 핀의 소비자가 아직 없고**(EKS 클러스터 부재), A0~A2 에서 arm64 로 전 서비스를 다시 빌드해 ECR 에 넣어야 하므로(C-29·`1-6`) **그때 eks 핀이 한꺼번에 최신이 된다**. ⓓ 🟢 **안전은 실증됐다**: `JWT_SECRET` 사용 서비스 = 정확히 6개이고 `JWT_SECRET_MIN_LEN` 가드 보유 서비스도 **같은 6개**(폴백 의존 0) · 라이브 시크릿 11개 **전부 64바이트** · `mp-mealplan` 이 이미 배포돼 **restarts=0 · Running** 으로 게이트가 걸린 채 정상 기동.
...
- [ ] **0-3b 🔴 `externalLabels: {site: onprem|aws}` — 두 번째 사이트가 생기기 전에** (2026-08-09 신설)
      ...
      🔴 **범위 확정(2026-08-13 · C-83) = AWS 쪽에만 `site: aws` 를 붙인다. 온프렘 Prometheus 는 건드리지 않는다.**
      온프렘은 **라벨 부재 = 암묵적 onprem** 이고 쿼리는 `site!="aws"` 로 잡는다.
      🔴 근거 = **양쪽에 붙여도 비대칭이 남는다** — 이미 쌓인 온프렘 시계열엔 라벨이 **소급되지 않으므로**,
      온프렘에도 붙이면 얻는 것 없이 **Prometheus StatefulSet 재시작(메트릭 1~2분 공백)만 추가로 산다**.
      🟢 PV 는 건드리지 않는다(롤아웃만) — 즉 재시작이 위험해서가 아니라 **대가만 있고 이득이 없어서** 안 한다
...
- [ ] 🆕 **0-4b 🔴 EKS `AppProject` 확장 8엔트리 — 머지된 config PR 들이 이것 없이는 안 돈다** (2026-08-13 신설)
      🔴 **발단 = 정본 안의 불일치.** §0-0 버킷표는 `0-3`·`0-3b`·`0-8d`·`1-28`(config **#162** 머지됨)을 *"✅ AWS 전용 신설 — 머지됨"* 으로
      세는데 **항목 체크박스는 `[ ]`** 다. 이유가 있다 — **머지해도 EKS 에서 sync 가 거부된다.** `AppProject/platform` 이
      새로 들어온 것들을 허용하지 않기 때문이다(라이브 실측 2026-08-13). 그런데 그 선행이 **어디에도 항목으로 없었다.**
      🔴 **필요한 것 = 8엔트리** (`AppProject/platform`):
      `sourceRepos` **+2** = metrics-server 차트 레포 · prometheus-community 차트 레포
        (라이브 9개엔 둘 다 없다 — 🔴 **차트 레포가 빠지면 그 Application 이 `InvalidSpecError("repo is not permitted")` 로 조용히 정지**한다.
         descheduler 가 실제로 이 방식으로 죽어 있었다[2026-08-03] — `bootstrap/argocd/base/appprojects.yaml` 주석에 기록돼 있다)
      `destinations` **+3** = `app` · `pipeline` · `mp-ingress`
        (`0-3` 이 옮겨오는 ResourceQuota·LimitRange·PSA 의 목적지다. 라이브 platform 은 관측·오퍼레이터 ns 9개뿐)
      `clusterResourceWhitelist` **+3** = `""/Namespace` · `scheduling.k8s.io/PriorityClass` · **`storage.k8s.io/StorageClass`**
        (라이브 6종 = ClusterRole·ClusterRoleBinding·CRD·Validating·Mutating·APIService.
         🔴 **세 번째는 `0-8e`/config #163 이 늘린 것** — 종전 인계는 "2줄"이라고 봤다)
      ...
      *"이 디렉터리는 ArgoCD 가 읽지 않는다(뿌리 자신이라 순환) · **적용자는 Ansible `k8s_argocd` 롤** ·
      ...
      ① `mealplanning` AppProject 의 `mp-ingress` destination 이 **라이브에만** 있다(`argocd_allowed_namespaces` 는 3개뿐)
      ② `mealplanning-root` Application 의 `project` 가 손 patch 로 바뀌어 있다.
      ⇒ EKS 용 롤의 입력값은 **Ansible defaults 가 아니라 `bootstrap/argocd/overlays/eks` 렌더**여야 한다.
      🕐 **온프렘 쪽 drift 해소는 이관 후**(C-83 · 지금 `--tags argocd` 를 돌리면 `mp-ingress` 가 지워진다)
...
- [x] **0-8b ✅ PV reclaimPolicy 정책 실구현 — 완료(2026-08-13)** (2026-08-09 신설)
      🟢 **실행 결과** — 라이브 PV **21/21** 을 `Delete → Retain` patch. **파드 재시작 0**(전 ns 175 파드의 이름·RESTARTS·생성시각 diff 완전 동일) ·
      PVC/PV 바인딩 **21/21 `Bound` 유지** · claimRef diff 없음 · **레포 파일 변경 0** ⇒ `overlays/onprem` 렌더는 정의상 불변(**C-83 게이트 자동 충족**).
      대상에 `data/pg-{1,2}`·`pg-{1,2}-wal` · `observability/prometheus…-0`(30Gi) · **`app/mp-ranking-model`(`ranker.pkl` · #616)** · kubecost 64+32Gi 포함.
      🟢 **ES 3개도 포함했다** — 근거 = `Retain` 은 PVC 삭제 시점에만 동작해 **평시 비용 0** 이고, *"ES 는 PG 에서 재파생 가능"* 은 **PG 가 살아남았을 때만** 성립한다.
      실제 위험 시나리오(ns 통째 삭제 · ArgoCD prune · 잘못된 `delete -k`)는 **PG·ES 를 같이 가져간다**. 예외 3개를 두면 철거 시 기억 부담이 생기는데 대가는 VG 여유 248GiB 중 **30GiB** 뿐.
      **검증** = `kubectl get pv -o custom-columns=…RECLAIM…` → 21행 전부 `Retain`/`Bound`.
      🔴 **잔여 = `0-8e`**(기본 SC 가 여전히 `Delete` — 신규 볼륨은 되돌아간다). 아래는 원래 진단이다 —
...
- [ ] **0-8e 🔴 기본 StorageClass 가 여전히 `Delete` — `0-8b` 의 잔여 구멍** (2026-08-13 신설)
      `0-8b` 의 `kubectl patch` 는 **기존 PV 만** 덮었다. 기본 SC 는 그대로 `openebs-lvm`(`Delete`)
      (`infra/ansible/roles/k8s_storage/templates/storageclass.yaml.j2:23`) ⇒ **CNPG `pg-3` 증설 · Prometheus 재생성 · Kafka 증설 등
      신규 볼륨은 조용히 `Delete` 로 복귀한다.** `openebs-lvm-retain` SC 는 같은 템플릿 `:28` 에 **이미 있고 주석에 "PG·ES·Kafka" 용도까지
      적혀 있는데 소비자 0** 이다 — `0-8b` 가 지적한 *"방어선만 만들고 배선을 안 했다"* 가 **신규 볼륨 축에서는 아직 그대로다.**
      🔴 **메우는 방법이 둘 다 형상 변경이다** — ⓐ 기본 SC 를 바꾼다(SC 는 immutable ⇒ **재생성** 필요) ⓑ `volumeClaimTemplate` 에 `storageClassName` 명시
      (STS 재생성을 유발할 수 있다). **둘 다 C-83 상 온프렘에서 하면 안 된다.**
      🟢 **그래서 두 갈래로 처리한다** — **AWS** = `overlays/eks` 에 `gp3` Retain SC 를 처음부터 명시(C-83 안전) /
      **온프렘** = 형상을 안 바꾸고 **"신규 볼륨을 만들 때 `storageClassName: openebs-lvm-retain` 을 명시한다"** 는 운영 수칙으로 둔다.
      🔴 **`0-3`(온프렘 스택 철거·소유권) 착수 전에 닫는 게 맞다** — 그 국면이 PV 를 재생성할 수 있는 유일한 시점이다.
...
- [ ] **0-12 ⭐ `jwt_secret` 조용한 폴백 제거** — 커밋된 placeholder(`dev-insecure-change-me`) + pydantic-settings 가 env 누락 시 조용히 폴백 → **토큰 위조 가능 상태로 무증상 기동**. 누락 시 기동 실패로 바꾼다 〔#32〕
      🟢 **코드 머지 #564** · 🔴 **정정(2026-08-13 실측) = 6개 중 1개만 배포됐다.** 종전 *"미배포"* 는 부정확하다.
      🟢 `mealplan` = onprem 핀 `67521674c356`(PR #615 머지) **배포됨** / 🔴 **미배포 5개** = `account`(`03c9334d`) · `pantry`·`notify`(`fbffab83`) · `price`(`e02f0a29`) · `recipebook`(`24fa81a1`) — 전부 08-03~08-06 핀
      🟢 **게이트 실증** — `mp-mealplan` 파드 **restarts=0 · Running**. fail-fast 가 걸린 채 정상 기동한다
...
- [ ] **0-14c 🔴 S4-2 워크로드별 ServiceAccount 신설 — 0-16 의 진짜 선행** (2026-08-09 신설)
      ...
      **AWS 가 아니라 라이브 온프렘에서** 터진다. ⇒ **`overlays/eks` 에만 넣는다. 온프렘은 `default` SA 를 그대로 쓴다.**
      🟢 그래서 이 항목은 **위험 항목에서 무위험 항목으로 내려간다**(§0-0 버킷표 = AWS 전용 신설)
...
- [ ] **0-16 정적 AWS 키 `envFrom` 제거** — pipeline ns 워크로드 22개 전부에 전파. 🔴 **env 가 자격증명 체인에서 IRSA 보다 앞선다** 〔#78〕
      🔴 **순서를 지켜야 한다 — 역순이면 개선이 0이다** (2026-08-09, C-24):
      ① 워크로드별 SA 신설(**0-14c**) → ② `mp-pipeline-secrets` 분리(**0-14d**) → ③ **IRSA 롤 신뢰정책(OIDC) + SA 에 `eks.amazonaws.com/role-arn` 어노테이션**(C-30)
      🔴 **①②는 온프렘에서 미리 / ③④는 AWS 오버레이에서만** — 온프렘은 EKS 밖이라 IRSA 를 못 쓰고 **정적 키가 유일한 경로**다. 온프렘에서 ④를 하면 파이프라인이 죽는다
...
- [x] **0-17 egress 무제한 11 워크로드 닫기** (data·observability) — AWS 에선 **IMDS 를 통한 노드 IAM 롤 탈취 경로** 〔#57〕
      🟢 **완료·라이브 (2026-08-16 · config `628d36b`·`7c97040`)**. 🔵 **탈취 경로는 이미 다른 층에서 막혀 있었다** —
      MNG 인스턴스·Karpenter `EC2NodeClass` 둘 다 `httpPutResponseHopLimit=1`·`httpTokens=required` 라
      파드에서 `169.254.169.254` 가 **timeout** 이다(실증). `mp-deny-imds` 는 그 위의 두 번째 층이다.
- [x] **0-18 netpol 재작성** — LAN `192.168.0.0/24` 전 포트 ipBlock 7건 + 내부 게이트웨이 전면 개방(`ingress: [{}]`)·와일드카드 SNI. 🔴 **VPC CIDR 로 기계적 치환 금지** 〔이슈 #549 · 감사 #53 #58〕
      🟢 **완료·라이브 (2026-08-16)** — 기계적 치환하지 않았다(FQDN·엔티티·서브넷 CIDR 로 재작성).
      실증: `recipe` → 임의 인터넷 **차단** · `account` → `kauth.kakao.com` **허용**·인터넷 차단 ·
      사이드카 없는 파드의 평문 요청 **거부**(mTLS STRICT 발효 — 켜기 전엔 200 이었다).
      🔴 **그 과정에서 공개 사이트를 죽였을 결함을 찾아 같이 고쳤다** — `policies-ingress/overlays/eks` 가
      **C-26(NLB·`instance`) 시절 그대로**였다. C-60 이 ALB(`ip`)로 바꾸면서 ALB ENI 가 파드 IP 에
      직접 붙고 **헬스체크(15021)도 그 ENI 에서** 오는데 base 는 15021 을 `fromEntities: host` 로만
      열어 둔다 ⇒ 타깃 2개가 전부 unhealthy 로 떨어졌을 것이다. 공개 서브넷 2개
      (`10.10.0.0/24`·`10.10.1.0/24`)에서 443·80·15021 을 여는 규칙을 추가했다.
      🔴 **교훈 = 오버레이는 쓰인 시점의 결정을 굳힌다.** 나중 결정이 그것을 무효화해도
      **배포되지 않은 파일은 아무도 다시 읽지 않는다.** 같은 부류가 이번 감사에서 셋 나왔다
      (`monitoring` "골격" 주석 · 정책 4종 "선행 미완" 주석 · 이 NLB 규칙).

### 0-C. 리드타임이 있어 착수만 먼저

- [ ] **0-19 🔴 AWS 서비스 쿼터 증액 신청** (vCPU·EIP·**ALB** — C-60 으로 NLB 에서 정정) — 승인에 며칠. **일정 블로커**
...
- [ ] **0-22 ⏸ Jenkins 백업 — 🔴 유예 확정(2026-08-13 · 사용자 판단)**. ~~GitLab 이관 전 선행~~
      **근거 = 호스트 C 가 로컬 소재라 소실 위험을 감수한다.** ⇒ **A0.5(CI) 의 선행이 아니다.**
      🔴 **감수한 것을 정확히 적는다** — 호스트 C 가 죽으면 Jenkins 형상(잡·자격증명·웹훅)이
      소실되고, **A0.5 가 끝나기 전까지 Jenkins 는 유일한 CI** 다(온프렘 크롤은 C-3 상 상시 프로덕션).
      🟢 **A0.5 완료 시점에 이 항목은 자연 소멸한다**(CI 정본이 GitLab 으로 넘어가므로).
      🔴 **그때 살아남는 것은 이 항목이 아니라 A-28 의 "GitLab·SonarQube 자신의 백업"** 이다 —
      부채를 반복하지 않는다는 원칙은 그쪽으로 이월된다.
      이하는 유예 시점의 실측 기록(삭제하지 않는다) — systemd 유닛 **없음**, `s3://mp-jenkins-backup-ap2` **NoSuchBucket**.
      ...
      🔴 **정정(2026-08-13 실측) — 블로커가 2건이다.** 버킷 부재 외에 **`jenkins_backup_passphrase` 가 `secrets.yml` 에 키 자체로 없다.**
      롤 첫 태스크가 `length >= 24` 로 assert 하므로 **버킷만 만들어도 실행되지 않는다**(`backup_s3_access_key`·`secret_key` 는 설정돼 있다).
...
- [ ] **0-24 🔴 Kafka 프로듀서 전달 실패 미관측 — 유실을 성공으로 마감** 〔이슈 #558〕
      🟢 **코드 머지 #567** · 🔴 **정정(2026-08-13 실측) = 이미 온프렘에 배포됐다.** pipelines 온프렘 핀 `4f03052e` 가 `ffd5960e`(#567 머지, 08-10)를 **포함**한다.
      ⇒ 종전 *"미배포"* 는 틀렸고, 아래 알림 증가는 **가능성이 아니라 현재진행형**이다.
      ⚠️ 배포하면 조용히 성공 처리되던 실패가 종료코드로 드러난다 → **`KubeJobFailed` 알림이 늘 수 있는데 그건 새 고장이 아니라 원래 있던 고장이 보이는 것**(현재 `mp-poller-kurly` Job 2건이 이미 Failed)
...
- [ ] 🕐 **0-26 `lgtm-apps.yaml.j2` 이중 소유 해소 — 🔴 이관 후로 재분류(C-72, MinIO 삭제가 이관 후)** — S3 컷오버(C-18)의 선행** (2026-08-09 신설)
...
- [ ] **0-28 🔴 크롤 CronJob **6건** Kafka 이탈 (config 레포)** — ⟳ **7종 → 6건 정정**(실측: `mp-poller-recipe-review` 는 이미 파일 모드). 🟢 **Python 변경 0**.
      ...
      🔴 ⟳ **절차 정정(2026-08-12, C-72) — 기존 CronJob·크롤러 코드를 고치지 않는다.**
      ~~`--kafka` 를 뺀 채로 온프렘에서 1주기 돌려본다~~ → 그러면 그 주기 동안 Kafka 경로가 빈다.
      ⇒ **사본으로 검증한다**:
      ```
        crawler/<소스>/prototype.py       🔒 불변 (--kafka 그대로)
        CronJob mp-poller-<소스>          🔒 불변
        CronJob mp-poller-<소스>-s3  🆕   suspend: true · --out 파일 모드 · S3 sync
              검증 = kubectl create job --from=cronjob/mp-poller-<소스>-s3   (1회씩)
      ```
      🔴 **상시 병행(dual-write)은 미채택** — 크롤 대상 요청이 2배가 되어 차단 위험이 는다.
      🕐 **기존 CronJob 의 `--kafka` 제거는 이관 후**(C-70·C-72)
- [ ] 🕐 **0-29 `pipelines/stream/` 15개 모듈 — 🔴 삭제 금지 · 이관 완료까지 방치** (⟳ 2026-08-12, C-72)
...
- [ ] 🆕 **0-32 🔴 DB 초기 적재 설계 — "온프렘 PG 를 AWS 로 어떻게 처음 넣는가"** (2026-08-12 신설, C-72)
      ...
      ```
        ① 온프렘 barman → S3            🟢 이미 돌고 있다 (0-30 실측 · ObjectStore mp-pg-backup)
        ② AWS CNPG 를 bootstrap.recovery 로 그 S3 에서 복원      ← 🔴 계획 0건
        ③ 컷오버 : 온프렘 쓰기 중단 → 마지막 WAL 아카이브 → AWS 따라잡기 → promote
        ④ 그 뒤 방향이 뒤집힌다 (C-40 = AWS→S3→온프렘)
      ```

      🔴 **`0-23`(barman 경로 `pg-prod`/`pg-dr` 분리)이 ②의 선행**이다 — 지금 양 사이트 Cluster 이름이 둘 다 `pg` 라
      `pg/pg/wals` 가 겹쳐 **WAL 이 서로를 덮는다.** standby 를 세우기 전에 잡는 게 압도적으로 싸다.

      🔴 **`bootstrap` 은 Cluster 생성 시점 1회만 유효**(0-20) — AWS Cluster 를 만들 때 한 번에 맞춰야 하고,
      틀리면 **Cluster 삭제·재생성**(PVC 파기)이다. 리허설이 없으므로(1-9) 종이 위에서 정확해야 한다.

      **ES·Redis 도 같이 정한다** — 지금 문서에 한 줄도 없다:
      · **ES** = 이전 대상이 아니다. **AWS PG 에서 PGSync 로 재파생**(실측 8,963 docs / 13.9 MB → 분 단위)
      · **Redis** = 캐시라 **비우고 시작**한다. 🔴 단 `mp-redis-pgsync`(385 ops/s)는 PGSync 체크포인트를 들고 있으므로 재색인과 순서를 맞춘다
      · **Kafka** = 이전 대상 없음 (C-39 로 AWS 에 없다)

      🔴 **컷오버 중 쓰기 중단 창(③)의 길이가 실질 다운타임**이다 — `2-10`(앱 읽기 전용 모드)이 이것과 한 세트

## Phase 1 — 리허설·컷오버 준비 (조용히 깨지는 것)

- [x] **1-1 ✅ 해소 — PGSync CDC 는 고장나지 않았다 (2026-08-13 실측)** — 종전 서술 *"논리 복제 슬롯 `lost`"* 는 **#555 당시 사실이고 이후 복구됐다** 〔#17〕
      🔴 **2026-08-13 세션이 이걸 "고장"으로 오진한 채 시작했다.** 오진의 뿌리 = `active=f` 와 `wal_status=reserved` 를 손상 신호로 읽은 것.
      🔴 **판정 기준을 박아둔다: `active`·`wal_status` 가 아니라 `restart_lsn` 전진 여부로 본다.**
      · `active=f` = **정상값** — PGSync 는 walsender 스트리밍이 아니라 **SQL 폴링**이라 24h 평균 `active` 가 **0.001** 이다
      · `wal_status=reserved` = **가장 건강한 값** — 악화 순서는 `reserved → extended → unreserved → lost`
      **실측 근거** = 슬롯 2개 보유 WAL **16MB(1세그먼트) 고정** · 60초에 `restart_lsn`·`confirmed_flush_lsn` 둘 다 ~16MiB 전진 ·
      WAL PVC **12%**(1.1/9.8GiB) · PGSync 파드 Running·재시작 0 · 로그 `Xlog: [0]`(백로그 0)
...
- [ ] **1-6 ⟳ 이미지 아키 — 🔴 "멀티아치" 철회(2026-08-14, C-89)**. 종전 서술 = *"전 이미지 amd64 단일. Graviton 노드면 전면 CrashLoop"* 〔#31 #35〕. 🟢 문제 자체는 유효하나 **해법이 바뀌었다** — manifest list 가 아니라 **사이트별 단일 아키**다(GitLab=arm64→ECR / Jenkins=amd64→Harbor). **A-29 에 흡수**
...
- [ ] **1-29 🔴 `JWT_SECRET`·`PGPASSWORD` 로테이션 절차 신설** (2026-08-09 신설, C-23)
      ...
      🔴 **범위 추가 (2026-08-16 감사 A-7) — `mp-crawl-uploader` 정적 AWS 키가 이 목록에 없었다.**
      온프렘 크롤 릴레이가 S3 에 쓰는 데 쓰는 **장수 액세스 키**이고, 온프렘은 IRSA 를 못 쓰므로
      (클러스터 밖 · C-30) **정적 키가 구조적으로 남는 유일한 자리**다. 그래서 더더욱 로테이션
      대상이어야 한다 — *"정적 키 0" 목표에서 빠진 항목이라 영원히 안 도는 상태*였다.
      ⚠️ 이 키를 돌리면 온프렘 릴레이 CronJob 이 **다음 실행부터** 새 값을 쓴다(파드가 매번 새로 뜨므로
      `rollout restart` 가 필요 없다) — 위 10~11건과 절차가 다르다.
...
- [ ] **1-16 🔄 재분류(2026-08-13) — 버그가 아니라 의도된 선택이다** — `CHECKPOINT_PATH=/app/checkpoint` 가 emptyDir 위 8 B 파일 2개 → 파드 재시작 시 소멸(사실 확인됨).
      🔴 그러나 `platform/pgsync/base/pgsync.yaml` 에 **선택 근거와 비용이 명시**돼 있다 — *"재시작 시 재동기화 … 문서 8.4k 규모라 수 분"*.
      현재 **9,418 문서**라 비용은 여전히 수 분이고, 종전 서술 *"별건 결함"* 은 과했다.
      🟢 **온프렘은 손대지 않는다**(C-83 형상). PVC 화는 **`overlays/eks` 신설 시** 반영한다.
      🔴 **지금 고치면 안 되는 구체적 이유** — 신규 PVC 가 기본 SC(`Delete`)로 생겨 **`0-8e` 의 구멍을 다시 밟는다.**
...
- [x] **1-18 ✅ 완료 — 요구한 알림이 이미 3중으로 라이브다 (2026-08-13 실측)**
      🔴 **종전 수치는 틀린 게 아니라 *다른 것을 재는 값*이었다** — *"WAL 이 **361 MB/일** → 약 27일치"* 의 오류는 숫자가 아니라 **적용 대상**이다.
      · **361 MB/일** = barman **S3 업로드량(압축 후)** — 대역폭·S3 비용 논거에서는 **여전히 맞다**(§ 하이브리드 대역폭 · 실측표)
      · **4.99 GiB/일** = **WAL 세그먼트 생성량(압축 전)** — **PVC 에 쌓이는 것은 이쪽이다**
      **실측 = `pg_stat_archiver` 4,741 세그먼트 / 14일 20시간 = 213.0 MiB/h = 4.99 GiB/일** (15일 평균).
      🟢 **14배 차이가 나는 기전이 설명된다** — `archive_timeout=300s` 가 5분마다 세그먼트를 강제 전환하므로
      **대부분이 패딩인 16MiB 세그먼트**가 만들어진다. 그건 **S3 에선 거의 0 으로 압축되지만 PVC 에서는 16MiB 를 온전히 차지한다.**
      (그래서 활동만 있으면 **192MiB/h 가 바닥**이다.)
      🔴 **그래서 아카이빙 정지 시 여유는 27일이 아니라 약 2일**(9.8GiB ÷ 4.99GiB/일). **13배 낙관이었다.**
      🔴 **짧은 창 샘플링은 무의미** — 같은 시각대에 60초 창 16MiB / 180초 창 36KB 로 널뛴다. **`pg_stat_archiver` 카운터로 재라.**
      🟢 **알림은 3중으로 이미 있다** (라이브 `prometheusrule` 전수 확인):
      · 슬롯 정지 축 = `MpPGReplicationSlotRetainedWALWarning`(256MiB · **~1.1시간**) · `MpPGReplicationSlotWALGrowing`(`delta[2h]>200MiB`) · `MpPGReplicationSlotRetainedWALHigh`(1GiB · critical)
      · 아카이빙 정지 축 = `MpBackupWalArchivingStalled`
      · 🟢 **PVC 사용률 축 = `KubePersistentVolumeFillingUp`**(+`…InodesFillingUp`·`…Errors`) — kube-prometheus-stack 기본 룰이라
        이 항목이 *"PVC 사용률 자체의 알림이 없다"* 고 적은 시점 이후 **스택 도입으로 이미 메워졌다**
      🟢 `rules-data-tier.yaml` 이 **`active == 0` 을 알림 조건으로 쓰지 않는 이유**를 2026-08-04 실측으로 문서화해 뒀다 — 위 `1-1` 오진을 룰은 애초에 피하고 있었다.
      🔴 **남은 것은 문서 상수뿐** — `monitoring/base/rules-data-tier.yaml` 헤더 주석의 *"128MB/h · 하루 3GB"* 를 **213 MiB/h · 4.99 GiB/일** 로.
      임계값 자체는 바꿀 필요 없다(256MiB warning 이 ~1.1시간에 울어 실 대응창 **~3.3시간**). ⚠️ 그 파일은 헤더에 **"태현 소유"** 로 명시돼 있어 소유자 판단에 맡긴다.
      🔴 **주 생성원** = `crawl_raw` 19.3만행 · `retail_price` 19.3만행 — 둘 다 `schema.json` 에 **없는데도 논리 디코딩을 통과**한다(#555 핵심 메커니즘과 동일)
...
- [ ] **1-20 `data/mp-pg-onsite` egress 를 S3(443)로 여는 CNP 추가** 〔목적지 = **`mp-pg-dump-ap2`** · C-69〕 — 현재 DNS·5432·**9000(MinIO)만** 허용.
...
- [ ] **1-21 🔴 랭킹 모델 로드 실패가 조용하다** 〔이슈 #561〕 — `ml/recipe-ranking/serve.py:157-169` 가 파일 부재·pickle
      ...
      🔴 **2026-08-16 감사 — 이 항목이 예고한 일이 EKS 에서 그대로 일어나 있었다.** 실측:
      `/health` → `model_loaded:false` · `model_source:file_missing` 인데 **파드 Ready·헬스 200**.
      실호출 `POST /rank/personalize` 로 `[{10,score 0.1},{20,0.9},{30,0.5}]` 를 넣으면
      **200 + `personalized:false` + `ml_score` 전부 0.0 + 입력 순서 그대로**가 돌아온다
      (score 0.9 인 20번이 1위로 오지 않는다). ⇒ **기능이 죽은 게 아니라 AI 가 빠진 채로 통과**한다.
      🟢 **배선은 전부 이미 있었다** — `.gitlab-ci.yml` 이 `MODEL_S3_URI=s3://mp-ai-model-ap2/models/ranking/ranker.pkl`
      로 받아 `.dockerignore` 의 `!ranker.pkl` 예외를 타고 `COPY . .` 로 굽는다. **빠진 것은 S3 객체 하나**였다.
      🟢 **2026-08-16 조치** — 온프렘 PVC 에서 꺼내 그 키에 적재했다(702,418 B ·
      `sha256 d52e82ca69fe711c8e7485456a00a76c8204d19047ea5f4a871f228279aea8e0` = `MODEL_MANIFEST.txt` 와 일치).
      🔴 **남은 것 둘** = ① `mp-ci-job` 의 `s3:GetObject` 정책(`ci_job_model_read`) — **코드는 main 에 있는데
      `terraform apply` 가 안 됐다** ② 적용 후 `SERVICES=ranking-serving` 으로 재빌드.
      🔴 **알림 규칙 0건은 아직 그대로다** — `model_loaded` 를 노출만 하고 아무도 안 본다. 이번에 켠
      `mp-app-symptom`(F-1)에 그 규칙을 넣는 것이 이 항목의 진짜 완결이다.
- [ ] **1-21b 🟡 리파이너 PG 데드락 재시도 — 지금은 "안 사기로" 확정** (2026-08-16 · 사용자 확정)
      리파이너가 PG 데드락을 만나면 파드가 통째로 죽고, 그래서 `maxReplicaCount: 1` 로 회피 중이라
      **처리량 상한이 그 회피에 묶여 있다.** 🔴 **그럼에도 지금은 조치하지 않는다** —
      ① 병목이 아니다(실측: SQS 3큐 전부 대기 0 · 24h 유입 7,331행이 전부 적재된다)
      ② 근본 해법(쓰기를 `product_id` 로 정렬)은 **Kafka 파티션 친화성을 앱에서 되사는 것**이라 크다
      ③ 남은 기간에 넣으면 **지금 잘 도는 적재 경로를 흔든다.**
      ⇒ **알려진 제약으로 문서화하고 둔다.** 발동 조건 = *SQS 대기가 쌓이기 시작하면* 그때
      `40P01` 감지 + 지수 백오프 3회를 넣고 `maxReplicaCount` 를 푼다.
...
- [ ] 🕐 **중단(2026-08-13 · C-53)** **1-45 🔴 Tailscale ACL 정책 (C-53)** — 기본 거부 + 필요한 것만. 🔴 **핵심 = 데이터 평면 금지를 ACL 로 강제**한다
      (사람 → `pg-pooler:5432`·`es:9200` 만 허용 / 파드 ↔ 파드 · 사이트 ↔ 사이트 서비스 호출은 **불허**).
      문서로만 적으면 유혹을 못 막는다 — **정책 파일이 경계선이다**
- [ ] 🕐 **중단(2026-08-13 · C-53)** **1-46 Tailscale 키 만료·디바이스 승인 정책 (C-53)** — subnet router 는 **auth key 로 뜬다**.
      만료 없는 키를 쓰면 그게 새 장기 자격증명이 된다 · 디바이스 자동 승인 끄기 · tailnet lock 검토
- [ ] 🕐 **중단(2026-08-13 · C-53)** **1-47 Tailscale K8s Operator 배치 (C-53)** — 🔴 **subnet router 만으론 PG 에 못 붙는다**(PG 는 파드).
...
- [ ] **1-44 S3 라이프사이클 규칙 Terraform 신설 (C-49)** — 🔴 **지금 규칙이 0건이다.**
      ...
      **⇒ 만료는 전 버킷 · 전환은 CloudTrail 하나.**
      ...
      | `mp-pg-dump-ap2` | 🟢 **30일 → IA · 90일 → Glacier IR** | **190일** | ⟳ **정정(2026-08-13 · 미결 ㉕ 해소)** — 종전 *"7일 · 현행 `mc rm` 승계"*. 🔴 **`mc rm --older-than 7d` 를 eks 에서 걷어야** 라이프사이클이 발동한다 · 🔵 온프렘은 7일 유지 |
...
- [ ] **2-8 `mp-pg-onsite-dump` 프리픽스 DR 분기** (AWS 목적지 = **`mp-pg-dump-ap2`** · 프리픽스 `onprem/`·`aws/` · C-69) — 같은 경로면 상호 덮어쓰기 + 백업 신선도 판정 오염. **0-23 과 한 작업**
...
- [ ] 🔴 **PGSync 논리 슬롯 2개가 여전히 inactive** — `foodbudget_recipes_live`·`foodbudget_user_recipes_live`,
      각 **16 MB retained** · `wal_status=reserved` · `active=f` (2026-08-09 실측 · ⟳ **2026-08-13 재확인: 값이 그대로 16MB 고정 = 슬롯이 따라가고 있다는 증거다.** 🔴 판정은 `active`·`wal_status` 가 아니라 **`restart_lsn` 전진**으로 한다 — `1-1` 참조). #555 의 잔재이자 **1-1 의 실측 근거**다.
...
- [x] 🆕 **A-24 ✅ 조직 SCP — 사용자가 승인을 받았다 (2026-08-13) · 게이트 해제** (2026-08-11 신설, C-57)
      사용자 확인 = *"오케이 받았어 스펙 걱정하지 말고 그냥 넘어가"* ⇒ **`m7g` · EKS · `ap-northeast-2` 전제를 그대로 유지한다.**
      🟢 **이것으로 "AWS 착수의 실질 게이트" 가 사라졌다** — `0-19`(쿼터, 2026-08-13 해제)와 함께 **A0 착수를 막는 조직 측 조건은 없다.**
      🔴 **다만 승인은 "설계 전제가 유효하다" 까지다** — 실제 `terraform apply` 는 **자격증명 문제**가 남아 있다(아래 `0-19` 옆 · A-7):
      계정 ID 12자리와 **VPC·EKS·IAM 을 만들 수 있는 프로필**이 필요하다. 🔴 기존 `mp-backup` 은 **백업 전용 키**로 보이므로
      그것으로 apply 하면 `AccessDenied` 다 — **A0 전에 별도 프로필/키를 확인할 것**(A-47 의 IRSA 신뢰정책도 계정 ID 가 PLACEHOLDER 다).
      ⚪ 원문(무엇을 왜 물었나) = 확인 대상 3개 ① `m7g` 계열(C-45 · Graviton 전제) ② EKS ③ `ap-northeast-2`.
      막혀 있었다면 `m7g` 하나로 **C-45·C-29·1-6(arm64) 전부 재검토**였다 — 그래서 게이트였다.
...
- [ ] 🆕 **A-28 CI 서버 프로비저닝 — 🔴 "수정"이 아니라 대부분 "신설"이다** (2026-08-11 신설, C-59)
      🆕 **단계 = A0.5**(2026-08-13 배정 · 사용자 확정 — C-78 에 그 단계를 신설했다).
      🔴 **A-29(파이프라인 이식)와 한 단계다** — 서버만 서고 파이프라인이 없으면 ECR 이 비어 있어
         A1 이 못 뜬다. A0.5 의 완료 판정 = **ECR 18리포에 arm64 이미지가 들어간 것**.
      🟢 **선행 미결 ⑲·⑳ 은 둘 다 닫혀 있다** — ⑲ → C-61⑥(Ansible over SSM) · ⑳ → C-61·C-62·C-71.
      ⚠️ **`0-22` 는 선행에서 뺐다**(2026-08-13 사용자 판단 = 로컬 소재라 감수). 아래 백업 주의는
         **GitLab·SonarQube 자신의 백업**에 대해서는 여전히 유효하다.

      실측(2026-08-11): Ansible 롤 **34개 중 `gitlab` 롤은 없다** · Terraform provider 는 **`bpg/proxmox` 단독**(AWS 0건 — A-7 과 같은 뿌리).
      ⟳ **2026-08-13 정정** — Terraform AWS 는 **더 이상 0건이 아니다.** A0 에서 `aws-platform/` 이
        118 리소스로 apply 됐다(hashicorp/aws · S3 backend `tfstate/aws-platform.tfstate`).
        ⇒ A-28 의 Terraform 몫은 "provider 도입"이 아니라 **그 스택에 VPC-B·EC2 파일을 더하는 것**이다.
      | 대상 | 지금 | 할 일 |
      |---|---|---|
...
- [ ] 🆕 **A-29 🔴 파이프라인 이식 — `Jenkinsfile` → `.gitlab-ci.yml`** (2026-08-11 신설, C-2·C-61)
      ...
      3. **ECR 인증 주입** — A-30
- [ ] 🆕 **A-50 🔴 CI 잡 신원 = GitLab OIDC → 잡 전용 롤** (2026-08-13 신설 · 사용자 확정 *"무조건 정석대로"* · **A-30 을 대체**)

      **한 줄** = 빌드 잡이 **자기 OIDC 토큰**으로 AWS 롤을 빌린다. 저장되는 비밀 0개 · 경계는 **프로젝트+브랜치**.
      🟢 **새 패턴이 아니다 — IRSA 가 정확히 이것이다.** 발급자만 EKS → GitLab 이고
      `sub` 가 `system:serviceaccount:…` → `project_path:…` 로 바뀐다.

      **구현** = `infra/terraform/aws-platform/iam_ci_oidc.tf`(작성 완료 · **2단 apply**)
      · `ci_oidc_issuer_url` 이 비면 전부 건너뛴다(A0 `create_node_group` 과 같은 패턴)
      · 잡 롤 `mp-ci-job` = ECR push(`mealplanning/*`) 전용 · 🔴 인스턴스 롤에는 ECR 권한 **없음**

      🔴 **선행 = Cloudflare Access 에 경로 2개 Bypass** (조각 5 cloudflared 와 한 세트)
      ```
        https://gitlab.mealbong.cloud/.well-known/openid-configuration
        https://gitlab.mealbong.cloud/oauth/discovery/keys        ← JWKS(공개키)
      ```
      AWS STS 가 **익명으로** 읽어 서명을 검증한다. 🟢 둘 다 **공개키·메타데이터뿐**이라 노출로 잃는 것이 없다.
      ⚠️ C-61⑤ 가 기각한 Bypass 는 **git 경로**였다(뒤에 소스 전체 ⇒ *검문소가 한 겹으로 줄어든다*).
      이 두 경로는 보호할 비밀이 없어 **그 논거가 전이되지 않는다.**

      🔴 **사고를 막는 두 조건**(둘 다 코드에 있음 · `terraform console` 로 렌더 확인 완료)
      · `sub` = **리터럴 프로젝트 경로 + 브랜치 목록**(`main` 하나). `*` 로 두면 그 GitLab 의
        **아무 프로젝트·아무 브랜치 잡**이 이 롤을 빌린다 — GitHub Actions OIDC 사고가 반복된 지점이다.
      · `aud` 고정(`client_id_list`) — 안 좁히면 다른 신뢰 당사자용 토큰이 우리 롤에 통한다.
      · thumbprint 하드코딩 금지(TLS 갱신 때 조용히 깨진다)

- [ ] 🆕 **A-51 🔴 app 레포 GitLab 이관 — "가져오기"와 "정본 전환"을 나눈다** (2026-08-13 신설 · 사용자 확정)

      정본 = **C-61②**(app 레포 = GitLab 정본 + GitHub push mirror · 미러는 오프사이트 백업).
      🔴 **그 한 줄이 두 작업이라는 것이 이 항목의 요지다.**
      ```
        ①  GitLab 에 프로젝트 생성 + GitHub 에서 전체 히스토리 import
            → 러너가 빌드할 대상이 생긴다(파이프라인 없이는 A0.5 완료 판정 불가)
            🟢 GitHub 이 그대로 정본 — 팀 작업 흐름 무영향
        ②  정본 전환 — 개발자가 GitLab 에 push · GitLab → GitHub push mirror
            🔴 CLAUDE.md 의 작업 규칙이 통째로 이사한다
      ```
      🔴 **①을 먼저 하지 않으면** 파이프라인을 시험할 대상이 없고, **②를 먼저 하면** CI 가 양쪽에서
      다 안 되는 구간이 생긴다. ⇒ **① 지금 · ② 파이프라인 검증 후**(사용자 확정).

      🔴 **이관 시점의 제약** — cloudflared(조각 5) 전에는 **외부에서 GitLab 에 닿을 수 없다**
      (SG 인바운드 0개). ⇒ import 는 **인스턴스 자신이** 한다: 공개 GitHub 레포를 clone 해
      localhost GitLab 으로 push. 🟢 app 레포가 public 이라 자격증명이 필요 없다.

      ✅ **이슈 트래커는 GitHub 유지**(사용자 확정 2026-08-13). 근거는 C-61③ 과 같은 계열 —
      ...
      **GitHub 을 과거로 되돌린다**(force push 성격). 순서를 못박는다:
      ...
      ⚠️ 이걸 빠뜨리면 *"PR 리뷰 필수 · main 직접 머지 금지"* 가 **조용히 사라진다** — 규칙이
      문서에는 남고 강제만 없어지는 형태라 알아채기 어렵다.

- [x] ~~🆕 **A-30 ECR 자격증명을 빌드 컨테이너에 넘기는 방식**~~ → ⟳ 🔴 **A-50 으로 대체 확정**(2026-08-13 · 사용자 확정 *"이제 이게 정본이야"*)

      🔴 **문제 정의는 그대로 유효하다** — 아래 서술(빌드 컨테이너에서 IMDS 를 때리면 권한이 통째로 넘어간다)이
      A-50 의 출발점이다. **바뀐 것은 해법**이다.

      | | A-30 (2026-08-11 · 사용자 지시) | **A-50 (2026-08-13 · 채택)** |
      |---|---|---|
      | IMDS hop limit | **1** | **1** (같음 — 조치 ① 은 유지된다) |
      | 자격증명 출처 | 호스트가 받은 **12h ECR 토큰을 잡에 주입** | 잡이 **자기 OIDC 토큰**으로 롤 assume |
      | 인스턴스 롤에 ECR 권한 | 🔴 **필요**(호스트가 토큰을 받아야 한다) | 🟢 **불필요 — 아예 뺐다** |
      | 브랜치별 경계 | ❌ 토큰이 공유돼 **아무 브랜치가 push 가능** | 🟢 `main` 만(`sub` 조건) |
      | 자격증명 수명 | 12시간 | 분 단위 |
      | 갱신 장치 | 🔴 호스트에 토큰 갱신 cron 이 필요 | 🟢 없음(잡마다 새로 받는다) |

      🔴 **A-50 이 더 강한 결정적 이유** = *"방 안에 가져갈 것이 없다"* 가 *"문을 잠갔다"* 보다 강하다.
      A-30 은 hop 1 로 문을 잠그지만 **호스트에는 여전히 ECR 권한이 있다**(그래서 토큰을 받을 수 있다).
      A-50 은 인스턴스 롤에서 ECR 을 제거하므로 **hop 이 뚫려도 훔칠 것이 없다.**
      🟢 그리고 A-30 의 조치 ①(IMDSv2 + hop limit 1)은 **버려지지 않고 그대로 남는다** —
      C-71 의 상쇄 조건 ⓒ(A-34)가 그것을 요구하고, A-50 이 그 조건을 **공짜로 만족**시킨다.

      이하는 A-30 원문(문제 정의 = 사료로 보존):
...
- [x] ~~🆕 **A-31 ECR → Harbor 이미지 미러링 경로**~~ → ✅ **불필요 확정(2026-08-14, C-89)** — Jenkins 은퇴 철회로 Harbor 가 **자체 공급원**으로 남는다(미러가 아니다). 아래는 신설 당시 기록: (2026-08-11 신설) — §1 이 *"Harbor = ECR 미러(DR 이미지 공급)"* 라고 적어 두었으나 🔴 **누가 어떻게 미러링하는지 계획이 0건**이다.
...
### 🆕 2026-08-13 세션에서 나온 항목 (C-73~C-82 파생)

- [ ] 🆕 **A-35 🔴 Blue-Green 버스트 용량 실측 — C-76 의 전제** — 13종 동시 BG 면 promote 창에 파드가 2배다.
      2노드가 CPU 75.9% / 메모리 71.7%(C-45·C-64)라 **동시 전환이 안 들어간다.** Karpenter NodePool 의
      상한을 이 실측으로 정한다. 🟢 account·recipe 2종(replica 2 → 4)만이면 여유 안이나, 11종 전환 시점엔 필수.
- [ ] 🆕 **A-36 access key 90일 로테이션 — C-80 의 상쇄 조건** — 장기 키는 **만료가 없다.**
      IP allowlist 도 없으므로(C-41) 키만 있으면 어디서든 붙는다. 🔴 `apply`·`delete` 는 소유자 프로필에만 둔다.
- [ ] 🆕 **A-37 ⚠️ WireGuard × ENI 모드 지원 확인 — C-82 의 미검증분** — 온프렘은 Cilium WireGuard 를 쓴다.
      ENI 모드에서도 되는지, 그리고 **AWS 에서 필요한지**(VPC 내부 트래픽) 둘 다 확인 대상.
- [ ] 🆕 **A-38 🔴 YouTube 가 AWS NAT 공인 IP 를 봇으로 보는지 실측** — `video` 서비스가 `www.youtube.com` 을 부른다.
      지금은 **가정용 IP**(온프렘)에서 나가고 이후엔 **데이터센터 대역**이다. 정본은 크롤 CronJob 에 대해 이미
      *"`recipe-review` ← 가정용 IP 필요"* 를 적었는데 **video 도 같은 성질인데 누락돼 있었다.**
      🔴 **AI 파트에서 유일하게 "안 돌아갈 수도 있는" 항목**이다(배선 문제가 아니라 상대가 거부하는 문제).
      🟢 **AWS 착수 전에도 실측 가능** — EC2 하나에서 요청 한 번.
- [ ] 🆕 **A-39 FQDN netpol 9개 EKS 재검증 + S3·SQS 추가** — 라이브 = `app` 4개(account·chat·ocr·video) +
      `pipeline` 5개(bedrock·gemini·kurly·oasis·recipe). `toFQDNs` 는 **Cilium DNS 프록시가 응답을 가로채 IP 를
      학습**하는 방식이라 EKS(CoreDNS + VPC DNS)에서 재검증이 필요하다. 🔴 **안 맞으면 조용히 끊긴다**(타임아웃으로만 보임).
      🔴 **S3·SQS 가 허용목록에 없다** — Interface 엔드포인트(C-56)로 가면 FQDN 이 아니라 VPC 내부 IP 라 규칙 형태도 달라진다.
- [ ] 🆕 **A-40 OAuth redirect URI 에 검증 호스트 추가 — C-78 A2 의 전제** — 카카오·구글 콘솔에
      `aws.mealbong.cloud` 를 등록해야 병행 검증에서 로그인이 된다. 🔴 안 하면 **로그인만 막혀서** 원인 찾느라 시간 쓴다.
...
- [ ] 🆕 **A-43 🔴 Jenkinsfile CD 에 `overlays/eks` 핀 추가 — C-83 이 드러낸 CI 배선 결함** (2026-08-13 신설)
      실측: CD 스테이지가 `kustomize edit set image` 를 거는 경로가 **`services/${name}/overlays/onprem` 하나뿐**이다.
      🔴 **그대로 두면 main 머지마다 온프렘 태그만 오르고 `overlays/eks` 의 이미지는 영원히 낡는다** —
      이관 준비를 해도 AWS 오버레이가 배포 시점에 **몇 주 전 이미지**를 가리키게 된다.
      🟢 이건 C-83 의 동결 대상이 **아니다** — 동결하는 것은 *형상*이고 이미지 태그는 CD 가 하는 일이다.
      ⚠️ 이관 후 CI 는 GitLab(C-2)이므로 **Jenkins 에 넣는 것은 과도기 조치**다. GitLab CI 를 새로 쓸 때
      요구사항으로 넘긴다(`1-6` arm64 와 같은 묶음). 🔴 `crawler-kurly`·`data-pipeline` 은 **오버레이가 아예 없어**
      Jenkins 가 태그 핀을 스킵한다(손으로만 올라간다) — 그 둘은 별건이고 이 항목의 범위 밖이다.
      🟢 **구현 완료 · 머지됨 PR #624**(`c8eeac6`). ⚠️ 커밋 제목은 `A-42` 로 적혀 있으나 정본 번호는 **`A-43`** 이다(#622 가 A-42 를 먼저 씀). 🔴 **재사용할 함정 — `kustomize edit set image` 가 `newName` 을 파괴한다**(v5.4.3 직접 실행 실측).
      eks 오버레이는 매칭 키(`images[].name`)가 **Harbor 주소**이고 `newName` 이 **ECR 주소**라, 오버레이만 늘리면 ECR 매핑이 깨진다:
      `edit set image <name>=<img>:<sha>` → `newName` 을 Harbor 주소로 **덮어씀** / `edit set image <name>:<sha>` → `newName` **통째 삭제** /
...
- [ ] 🆕 **A-44 🔴 Cilium ENI IPAM 의 SG 적용 단위 확인 — C-82 근거 ④의 검증** (2026-08-13 신설)
      ...
      파드별 SG(Security Groups for Pods)는 **vpc-cni 의 branch ENI 기능**이고, **Cilium** ENI IPAM 은 노드 ENI 의 보조 IP 를 파드에 나눠주는 방식이라
      **파드가 노드의 SG 를 물려받을** 가능성이 크다(= 파드 단위 통제가 아니다).
      🟢 **C-82 자체는 안 흔들린다** — 결정을 이끈 건 ②(오버레이면 웹훅 5종을 `hostNetwork` 로 영구 패치)이고 ④는 부수 근거였다.
      🟢 **C-85 의 권고에도 영향 없다** — 파드 SG 가 없다면 *"노드 SG 에 대시보드 EC2 SG 만 허용"* 이 정확히 맞는 수단이고, 파드 단위는 **Cilium netpol** 이 한다.
      🔴 **그래도 근거란을 고쳐야 한다** — 안 고치면 나중에 누가 *"파드별 SG 가 있으니 netpol 을 줄여도 되겠지"* 로 읽는다.
- [ ] 🆕 **A-45 🔴 파이프라인·pgsync 이미지 핀을 `images:` 블록으로 전환 — 지금은 CD 자동화가 C-83 없이는 불가** (2026-08-13 신설)
      실측: 온프렘 핀이 **`base` 인라인**이다 — `data-pipeline`·`crawler-kurly` = `pipelines/base`·`pipelines/jobs` **21곳** · `pgsync` = `platform/pgsync/base/pgsync.yaml` **2곳**(`5b4e66c7…`).
      eks 쪽은 오버레이가 있으나 낡았다(`710b056…` · `1ea4a8be…`).
      🔴 **CD 를 자동화하려면 `base` 를 고쳐야 하고 그건 온프렘 렌더를 바꾼다 = C-83 위반**이다. ⇒ **이관 후** 또는 **eks 한정**으로만 가능.
      그래서 지금은 `A-43` 의 범위 밖이고, 두 이미지는 계속 **손으로** 올라간다.
- [ ] 🆕 **A-46 🔴 ECR 리포 네이밍 통일 = `mealplanning/` 유지** (2026-08-13 신설 · 사용자 확정)
      실측 불일치: `.gitlab-ci.yml` 푸시 대상은 `${ECR_REGISTRY}/mealplanning/mp-account-service` 인데 eks 오버레이 `newName` 은 `<acct>.dkr.ecr…/mp-account-service`(**`mealplanning/` 없음**).
      둘 다 유효한 ECR 이름이라 문법이 아니라 **합의 문제**였고, PR #624 는 포지션을 잡지 않고 기존 `newName` 보존만 했다.
...
- [ ] 🆕 **A-49 🔴 `platform/pg` eks 오버레이 2건 — 미결 ㉕·㉖ 의 집행분** (2026-08-13 신설 · 사용자 확정)
      한 PR 로 묶는다(같은 디렉터리를 만진다). 🟢 **둘 다 `overlays/eks` 한정이라 C-83 안전** — 온프렘 렌더 diff 0 이어야 한다.
      **① 논리 덤프 CronJob 에서 `mc rm --older-than 7d` 제거** (미결 ㉕ 해소 = C-79 190일)
      🔴 **안 걷으면 라이프사이클이 발동조차 못 한다** — 7일에 컨테이너가 먼저 지우므로 Standard-IA 최소 **30일**에 도달하지 못한다.
      🔴 **반대로 라이프사이클만 걸고 `mc rm` 을 남기면 "둘 다 지우는" 상태**가 되고, 둘 다 빼면 **아무도 안 지운다.** 그래서 한 PR 에서 같이 처리한다.
      🔵 **온프렘은 7일 유지**(C-83 동결) ⇒ 두 사이트의 보존이 **의도적으로 다르다**는 주석을 오버레이에 남길 것.
      ⚠️ IAM 연동 — `mp-pg-dump` 롤에서 **`DeleteObject` 가 불필요해진다**(A-47 표의 괄호 조건이 해소). 최소권한으로 빼라.
      **② `mp-pg-onsite-minio` + `onsite-backup.yaml` 을 `suspend: true`** (미결 ㉖ 해소)
      C-18 로 MinIO 가 삭제되므로 eks 렌더에 **존재하지 않는 엔드포인트를 가리키는 CronJob** 이 남아 매일 실패한다.
      🟢 **걷어내지 않는 이유** = C-69 가 2트랙 유지를 확정했고 재작성 시점이 **A4** 다. 걷으면 A4 에 다시 만들어야 하고, 두면 알림이 매일 온다.
      `suspend: true` 는 둘 다 피하면서 **A4 에 한 줄로 켠다.** 🔴 A4 체크리스트(`2-8`·`1-20`)에 *"suspend 해제"* 를 명시적으로 넣을 것 — 안 적으면 **조용히 안 도는 백업**이 된다.
...
| WAL 볼륨 | — | **361 MB/일** (S3 3,031객체/3.25GB) — 🔴 **이 값은 "S3 업로드량(압축 후)"이다. PVC 사이징에 쓰지 말 것** — 압축 전 생성량은 **4.99 GiB/일**(`1-18`) |
...
AWS 착수  (온프렘 선행이 아니다)        43건      0
          ...
          · A-32~A-33(C-65 Lambda 감시) · 🆕 A-34~A-41(C-71·C-73~C-82 파생)
          · 🆕 A-42(C-70 파생) · 🆕 A-43(C-83 파생)
──────────────────────────────────────────────
합계                                 174건     11
```
...
위 숫자는 다시 **본문 `- [ ]`·`- [x]` 개수를 기계적으로 센 값**이다(신설 `1-60`·`A-42`·`A-43` 포함).
...
| 날짜 | 내용 |
|---|---|
| 2026-08-13 | 🟢 **미결 ㉕·㉖ 해소(사용자 확정) — 논리 덤프 보존 = 190일·라이프사이클 / 온사이트 CronJob = `suspend`.** **① ㉕** — 정본 내부 충돌(보존표 **7일**[CronJob `mc rm`] ↔ **C-79 만료 190일**[라이프사이클])을 **C-79 쪽으로** 정한다. 근거 = C-79 의 판단축이 *"라이프사이클을 설계했다는 증명이 산출물"* 이고 **논리 덤프가 그 스토리의 주 전시물**인데, **7일이면 Standard-IA 최소 30일에 도달조차 못 한다**(계층 전환이 영원히 발동 안 함). 🔴 **그래서 `mc rm --older-than 7d` 를 eks 오버레이에서 걷는 것이 결정의 절반이다** — 안 걷으면 컨테이너가 먼저 지워 라이프사이클이 무의미하고, 라이프사이클만 걸고 `mc rm` 을 남기면 **둘 다 지우며**, 둘 다 빼면 **아무도 안 지운다.** ⇒ 한 PR 로 묶었다(**A-49**). 🔵 **온프렘은 7일 유지**(C-83 동결) ⇒ 두 사이트 보존이 **의도적으로 다르다.** 연쇄 = 보존표 행 정정 · **C-69 비용 재산정**(~~318 MiB × 7일 ≈ $0.05~~ → 정상상태 **약 60.4 GiB** · 계층 전환 반영 **월 $1 내외 추정**) · **A-47 의 `mp-pg-dump` 롤에서 `DeleteObject` 불요**(보존을 S3 가 맡으므로 최소권한에서 빼라). **② ㉖** — `mp-pg-onsite-minio`+`onsite-backup.yaml` 을 eks 에서 **`suspend: true`**. 🟢 걷어내지 않는 이유 = C-69 가 2트랙 유지를 확정했고 재작성 시점이 **A4** 라, 걷으면 **A4 에 다시 만들어야** 하고 두면 **없는 MinIO 를 가리켜 매일 실패**한다. suspend 가 둘 다 피하면서 A4 에 한 줄로 켜진다. 🔴 **A4 체크리스트(`2-8`·`1-20`)에 "suspend 해제" 를 명시할 것** — 안 적으면 **조용히 안 도는 백업**이 된다. |
...
| 2026-08-13 | 🎉 **A0 완료 + A0.5(CI) 단계 신설 + 결함 2건.** **① A0 종료** — Terraform **118 리소스 · 드리프트 0** · 노드 `m7g.xlarge`×2 arm64 · 🟢 **C-82(ENI) 실증**(파드 IP 가 노드 서브넷 VPC 주소) · CoreDNS·EBS CSI·cert-manager·ESO·Istio·RBAC·Karpenter·ArgoCD 전부 가동 · `eks.yml` **ok=57 changed=8 failed=0**(멱등) · ⑦ ArgoCD 가 config 를 읽어 **자식 Application 49개** 생성. **② 🔴 결함 #24 — ESO 를 SSM ParameterStore 로 지었다(C-36 위반).** 원인 = **C-23 행의 정정 머리말을 건너뛰고 그 아래 정정 전 본문을 읽음.** 🔴 2차 원인 = **워킹카피 stale**(로컬 `main` 이 `origin/main` 보다 **57 결정** 뒤처짐 — `maxC=29`·2,652줄 vs `maxC=86`·4,789줄)이라 `C-56` 이 "없다"고 나왔다. ⇒ **정정된 결정 행은 머리말이 본문을 이긴다 · 정본 인용 전 `git fetch`.** 고침 = `service: SecretsManager` · `prefix: mp/prod/`(🔴 SM 이름은 슬래시로 시작 안 함) · 스토어 `mp-aws-ssm`→**`mp-aws-secrets`** · IAM `secretsmanager:GetSecretValue`+`DescribeSecret` on `secret:mp/prod/*`(🔴 **와일드카드 필수** — SM 이 ARN 끝에 6자 랜덤 접미사를 붙인다) · ArgoCD ExternalSecret 을 **온프렘과 같은 `dataFrom.extract`**(번들 3키)로 교체. 🟢 **`kms:Decrypt` 불요 실증** — 기본 키 `aws/secretsmanager` 는 IAM KMS 문장 없이 읽힌다(CMK 로 가면 `kms:Decrypt`+**A-26** 필요 · 미결 ⑰ · `update-secret` 으로 가역). 🔴 **#4 는 결함이 아니라 같은 오독이 만든 유령**이었음이 함께 드러났다 — 그 오진이 `variables.tf` 에 *"받아들인 위험"* 으로 **정착**해 있었다(철회). **번들 6 은 유지**(근거가 4KB 가 아니라 remoteRef 70엔트리 무수정 + 온프렘 1:1) · 🔴 **4KB 논거 전량 소멸**(SM 64KB·티어 없음) ⇒ `pg-roles` 분리 근거를 **폭발 반경 분리**로 교체 · 판단축이 **바이트 → 시크릿 개수**($0.40/개/월). **③ 🔴🔴 결함 #25 — istiod 에 `pilot.cni.enabled` 누락.** 주입 파드가 `istio-init`(NET_ADMIN·NET_RAW)을 받아 **PSA `restricted` 가 전량 거부**(`FailedCreate`) ⇒ **사이드카 워크로드가 하나도 못 뜬다.** 🔴 **istio-cni 차트는 istiod 에게 자기 존재를 알리지 않는다** — 두 릴리스가 서로 모른 채 돌고 둘 다 `Ready` 다. 🔴 **`helm list` 로는 못 찾는다**(버전이 같았다 — 둘 다 1.30.3) · **`helm get values` 대조**로 찾았다. 고친 뒤 initContainer 가 `istio-validation` 으로 바뀌고 파드 22개 생성. ⚠️ **A0 에서 안 드러난 이유 = 주입 대상 워크로드가 0개**였다 — ⑦ 이 열리자 나왔다. **④ A0.5 신설** — C-78 에 단계 추가 + **A-28·A-29 배정**. 완료 판정 = **ECR 18리포에 arm64 이미지 적재**. 🔴 **`0-22`(Jenkins 백업) 유예 확정**(사용자 판단 = 호스트 C 로컬 소재라 감수) ⇒ 🟢 **온프렘을 건드릴 잔업이 0건**이 되어 **C-83 동결이 예외 없이 성립**한다. 감수한 것 = 호스트 C 사망 시 Jenkins 형상 소실 · A0.5 완료 전까지 Jenkins 가 유일한 CI. **⑤ 🔴 config `argocd/overlays/eks` 미완이 드러났다** — 자식 23개의 `source.path` 가 아직 `overlays/onprem` 이라 **EKS 가 Harbor `192.168.0.10` 에서 이미지를 당기는 상태**(`ImagePullBackOff` 19). config 자신이 *"아직 안 했다 · 초록 = 이관 준비 완료가 아니다"* 라고 적어둔 Wave B. 🟢 **비용 영향 0 실측**(Karpenter NodeClaim 0 · 노드 2대 · Unschedulable 2건은 CPU 부족이 아니라 **PVC 미바인딩**이라 증설로 안 풀려 Karpenter 가 안 움직인다). ⇒ **처리 순서 = 사용자 결정: CI 를 먼저 올리고 그 뒤에 path 를 뒤집는다**(지금 뒤집어도 ECR 이 비어 있어 무의미). **⑥ 온프렘 비밀 실측이 정본과 1키 어긋난다** — 정본 `6종 37키` ↔ 실측 **6종 36키**(+`repo-food-budget-config` 3키). "키별 34" 가 이 위에 서 있으므로 **A1 적재 직전 재실측**. 결함 이력 = `docs/mp_aws_a0_defect_log.md` **25건 · 분류 8개** |
...
| 2026-08-16 | 🔴 **이관 마무리 전체 감사 — 결정 신설 0 · 라이브 결함 5건 조치 · 사실 정정 4건.** **① 🔴🔴 `monitoring` Application 이 EKS 에 없었다**(F-1) — `$patch: delete` 의 근거 주석이 *"골격"* 이었고 **쓰인 시점엔 맞았다.** 그 뒤 `monitoring/overlays/eks` 가 채워졌는데 스위치를 안 켰다. 결과 = 앱 메트릭 **수집 0** · 우리 PrometheusRule **9종 전부 부재** (`mp-app-symptom`·**`mp-backup`**·`mp-rollouts`…). 🔴 **가정이 아니었다** — 그 사이 백업 실패 알림이 없어 아무도 안 봤다. 조치 후 실측: 스크레이프 타깃 **20개 전부 up=1** · `istio_requests_total` **0→117 시리즈** · mp 규칙 그룹 14개 로드. **② 🔴🔴 netpol·mTLS 가 EKS 에 0건이었다**(F-6 · `0-17`·`0-18` 종결) — 상세는 그 항목. **③ 🔴🔴 보안 알림 스택 30 리소스가 tfstate 에만 있었다**(F-7) — 브랜치 `feat/mp-security-alerts` 가 미머지·MR 없음이라 `origin/main` 에서 `terraform plan` = **`0 add / 3 change / 30 destroy`**. apply 하면 GuardDuty·CloudTrail·Lambda·SNS 가 사라지고 CloudTrail 버킷은 **Object Lock 이라 삭제가 중간에 실패**해 state 와 실물이 갈린다. `3 change` 중 하나는 EKS 로그에 `api`·`authenticator` 재활성(= **C-66 정정도 그 브랜치에만** 있었다). → **MR !37 로 머지**, plan `0 destroy` 확인. **④ 🟡 kubecost 스크레이프 실패**(F-4) — `…-clickhouse` 타깃의 9363 이 **127.0.0.1 바인드**(`/proc/net/tcp` = `0100007F`)라 원리적으로 불가. 라벨이 같아 선택자로 못 거르고 차트에 토글도 없어 **차트 SM 을 끄고 살아 있는 것만 직접 정의**했다. 🟢 비용 데이터는 정상이었다(7일 $21.33). `TargetDown` 발화 **1→0**. **⑤ 🔴 대시보드 접근 경로가 통째로 IaC 밖**(A-1·A-2) — NodePort 3 + netpol 3 이 손으로만 있었고, `mp-policies-observability` 는 EKS 에서 **온프렘 오버레이**를 보고 있었다(죽은 MinIO 정책 2개 포함). eks 오버레이 전환 + `dashboard-access.yaml` 로 편입. **사실 정정 4건** — ⓐ **F-3 취소**: video 는 `VIDEO_GENAI_BACKEND=vertex` 라 `gemini_key:false` 가 정상이다(내 오독) ⓑ **F-5 취소**: ES/PG 정합은 **완전 일치**(9,578 · id diff 0), 앞선 "+160" 은 측정 시점 차이 ⓒ **F-8 축소**: `mp-source-backup-ap2` 는 **월1 정상** · `mp-image-backup-ap2` 는 백업 트랙이 아님(객체 1개). 대신 **`0-22` 가 "미완" 이 아니라 "전무"** 로 드러났다 — `mp-harbor-backup-ap2`·`mp-jenkins-backup-ap2` 가 **NoSuchBucket** 이라 두 백업은 한 번도 성공한 적이 없다. 🔵 **사용자 확정 = 둘 다 안 만든다**(호스트 C 를 껐고 이미지 정본은 ECR) ⓓ **D3 취소**: `gp3`(Delete) 는 의도적 선택이고 근거가 파일에 있다(Loki·Tempo 정본은 S3 · kubecost 는 Prometheus 파생). **관통하는 교훈** = 🔴 **"배포되지 않은 파일은 아무도 다시 읽지 않는다."** ①②와 ALB 규칙 셋 다 *쓰인 시점엔 맞았던 주석*이 **나중 결정에 무효화됐는데 그대로 남아** 스위치를 영원히 꺼 둔 경우다. |
...
| 2026-08-12 | 🔴 **C-69 확정 — PG 백업 2트랙 유지 + 논리 덤프 버킷 개명 `mp-pg-onsite-ap2` → `mp-pg-dump-ap2`.** **발단 = 사용자 질문**(*"오프사이트는 물리(Base+WAL)고 온사이트는 논리라 용도가 다르다는 거지?"*) — 🟢 **그 판단이 맞고 이름이 그걸 가리고 있었다.** 🔴 **AWS 에는 "사이트" 가 없다**(C-57 로 계정 1개 · 두 버킷이 같은 리전) → 이름은 *위치*를 말하는데 실제 근거는 *방법*이라 **왜 나뉘어 있는지 답이 안 나온다.** ⟳ **같은 세션의 내 권고를 철회**한다 — *"893 MB 규모에 온사이트 백업은 불필요하니 버킷을 빼자"* 는 **틀렸다**: ⓐ 이미 도는 트랙이고(`data/mp-pg-onsite-dump` · `0 4 * * *` KST · **`pg_dump -Fc -Z6`** · 7일 · 318 MiB) ⓑ **C-18 이 이미 "다른 버킷" 을 결정**해 두었으며 ⓒ 판단축이 **규모가 아니라 방법**이다. **2트랙이 겹치지 않는 근거 4**: ① 부분 복구(barman 은 클러스터 통째 / 덤프는 테이블 하나) ② 🔴 **PG 메이저 업그레이드** — 물리 백업은 버전 호환이 없어 `16→17` 의 **유일한 탈출구**(C-15 자체운영의 대가이고 그 절차 자체가 미완) ③ 조용한 손상 탐지(`pg_dump` 는 전 페이지를 읽는다 / base backup 은 **손상도 복사**) ④ 도구 다양성(barman 단일 의존 = 그 결함이 전 백업을 무효화 · 선례 = 비밀 묶음 백업 7-29 이후 조용히 정지). 🔴 **오프사이트 역할은 이 버킷이 아니라 온프렘 WAL 복제(C-40)** 다. 🟢 **이관이 이 트랙을 개선한다** — §상시의 *"온사이트 덤프는 DR 이 아니다(b2 디스크 사망 = 7일치 소멸)"* 경고는 **노드 로컬 MinIO** 때문이었고 S3 로 가면 소멸 ⇒ 비로소 2트랙으로 셀 수 있다. ⏱ **개명이 지금 $0 인 이유** = 버킷이 아직 미생성이고 Object Lock 을 걸면 **만료 전까지 삭제 불가**(C-57·A-5). 🔵 **온프렘 실물은 안 건드린다**(§명명 규칙 예외) · 사이트 프리픽스 `onprem/`·`aws/` 유지(2-8). 비용 **월 $0.05** | 
| 2026-08-12 | 🔴 **C-72 확정 — 이관 원칙 = 온프렘 동결 · 이관은 덧셈만** (사용자 지시). **발단** = *"K8s 의 원본 코드를 상실할 수 있는 부분은 건드리면 안 된다. 정본에 Kafka 를 뺀다고 미리 다 빼는 게 아니고, 서비스는 Kafka 를 빼놓고 이전하는 것"*. **원칙 3줄** = ① 온프렘 기존 코드·오브젝트는 **AWS 안정화까지 불변** ② 검증은 고쳐서가 아니라 **사본을 새로 만들어서**(별도 파일·별도 CronJob·`suspend: true`+수동 트리거) ③ AWS 는 **처음부터 Kafka·MinIO 없이** 짓는다(≠ 온프렘에서 먼저 걷어낸다). 🔴 **상시 병행(dual-write)도 미채택** — 크롤 대상 요청이 2배가 되어 차단 위험이 는다. **재분류 4건 → 🕐 이관 후**: `0-29`(stream 15모듈 — 🔴 *"처분 판정"에서 **"삭제 금지·방치"**로 성격 변경*. #585 무응답에서 지우면 Kafka 로 되돌아갈 경로가 사라진다) · `0-31`(Strimzi) · `0-26`+C-18(MinIO) · `0-28`(크롤 `--kafka` 제거 — 절차를 **사본 CronJob `mp-poller-<소스>-s3`** 로 교체). 🔴 **적용 사례 = PR #593 감사** — `infra/terraform/aws/` 9파일·`pipelines/transport/` 7파일은 **전부 신설이라 안전**하나(🟢 **A-7 Terraform 골격이 이미 여기 착수돼 있었다**), `crawler/{kurly,10k_recipe,oasis}` 4파일 + `Jenkinsfile` 이 **Kafka 경로를 삭제**한다(`produce_record()` 함수 통째 · `run(kafka=)` 시그니처 · Dockerfile `confluent-kafka` · Jenkinsfile `pipelines/stream/` 추적) → **PR 을 A(신설)/B(크롤러 수정)로 분리, B 보류** 권고. **신설 `0-32` = DB 초기 적재 설계** — 🔴 **정본에 "온프렘 PG 를 AWS 로 처음 넣는 방향"이 통째로 없었다**(C-40·C-55·0-20 은 전부 AWS→온프렘). 경로 = ①온프렘 barman→S3(🟢 이미 가동) ②**AWS CNPG `bootstrap.recovery`**(계획 0건) ③쓰기 중단→마지막 WAL→promote ④방향 반전. 🔴 `0-23`(barman 경로 `pg-prod`/`pg-dr` 분리)이 선행 — 지금 양쪽 Cluster 이름이 둘 다 `pg` 라 **WAL 이 서로를 덮는다**. 🔴 `bootstrap` 은 **생성 시점 1회만 유효**라 틀리면 Cluster 재생성이고 리허설이 없다(1-9). **ES = 재파생**(8,963 docs·13.9MB) · **Redis = 비우고 시작** · **Kafka = 이전 대상 없음** 까지 함께 명시 | 
