...
- 설계 일관성 트랙 → **별도 이슈 = #557**(2026-08-07 등재). 사용자 선택 = **① 완전 Kafka 화**(리파이너가 "리뷰 필요" 이벤트 발행).
  실측으로 확인된 것 — 미크롤 **0건**(7,444 전량 `ok`)이라 **백필 불요** · 신규 이벤트 볼륨 = 회차당 **164~407건** · 신규 ~230줄/개조 ~130줄 · netpol 추가 0.
  🔴 이관 트랙과 분리하되 **시점은 이관 전**(백필 불요 조건은 나빠지기만 하고, 지금은 크롤러·Kafka·PG 가 같은 클러스터 안이다)
