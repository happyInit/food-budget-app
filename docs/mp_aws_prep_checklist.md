...
> | | |
> |---|---|
> | **확정 결정** | **C-1 ~ C-82** (§0.1). ⚠️ CLAUDE.md 는 아직 "C-1~C-29" 로 적혀 있고 **선행 작업 121건** 도 stale(실측 체크박스 160 · 번호 143) — 🔴 **대조표의 `외부 LB → NLB (ALB 기각)` 은 C-60 이 뒤집었다** |
...
> 반영 범위 = **C-1 ~ C-82** (2026-08-13 — 🔴 **Cilium ENI 모드**(C-82 → 파드 CIDR 소멸·`target-type: ip`) ·
...
> **비대화형 접근**(C-80) · 감사로그 보고서(C-81). 이전 = 2026-08-12 — 🔴 **이관 원칙 = 온프렘 동결·덧셈만**(C-72) · 🔴 **온프렘 목표 = 크롤 단독**(C-70) · **CI 공개 서브넷·NAT 미채택**(C-71) · **PG 백업 2트랙·`mp-pg-dump-ap2` 개명**(C-69) · 2026-08-11 — **CI/CD 형상 확정**(C-61·C-62·C-63) · **NACL 미채택**(C-58) · 🔴 **계정 3개 → 1개**(C-57) · **Interface 엔드포인트 3종 채택**(C-56) · PG 복제 하이브리드(C-55) · DR 전환 2단(C-54)).
...
| ~~④~~ | ✅ **해소(2026-08-13, C-83)** — **안 "가"(PG 직접) 확정.** 종전 기본값 *"무응답 시 SQS 경유(보수적)"* 는 **근거 무효로 폐기**(노출 로그는 Kafka 를 타지 않아 볼륨 논거가 이 두 토픽과 무관). ~~PG 직접 쓰기 vs SQS 경유~~ | 🟢 어느 쪽이든 Kafka 는 제거된다 | ✅ **닫힘** (구 #585·#608) |
...
- [ ] **0-14c 🔴 S4-2 워크로드별 ServiceAccount 신설 — 0-16 의 진짜 선행** (2026-08-09 신설)
      ...
      🟢 **온프렘에서 미리 해도 된다** — SA 신설은 **덧셈**이라 C-72 에 맞고, config `base` 에 넣으면 양 사이트에 다 적용된다 (C-78 A0 전)
