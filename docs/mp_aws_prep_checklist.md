...
> | | |
> |---|---|
> | **확정 결정** | **C-1 ~ C-63** (§0.1). ⚠️ CLAUDE.md 는 아직 "C-1~C-29" 로 적혀 있다 — **stale** |
...
| C-46 | 🔴 **WAF = Cloudflare 계층. AWS WAF 미채택** — NLB(L4)에는 **부착 자체가 불가**하다(AWS WAF 는 CloudFront·ALB·API Gateway·AppSync·Cognito 에만 붙는다). C-26 의 직접 귀결이며 **새 포기가 아니라 이미 포기한 것의 명문화**다. 조건 3건 = `1-20` SG 게이트 **차단 승격** · CF 무료 플랜 활성 룰 실측 · **레이트리밋을 CF 엣지로** | 2026-08-10 | 사용자 확정 |
...
| NLB | 1 | $16.43 | 1.9% |
| **확정 소계** | | **$713.80** | 83.3% |
| GitLab·kubecost EC2 · LCU · AZ간 전송 | 수량 가정 | $143.46 | 16.7% |
| **합계** | | **$857.26** | 100% |
...
**미검증으로 남긴 것**(추정하지 않았다): NAT 데이터 처리 GB · 인터넷 아웃바운드 GB · S3 저장 볼륨 · 스냅샷 · 공인 IPv4 요율 적용 여부($0~$21.90) · NLB LCU 실측 · Reserved/Savings Plan 할인율.
...
```
 ...
 안 죽는 것 🟢 유입 전체 — NLB→노드는 NAT 를 안 거친다 (C-26)
...
→ 대신 **`1-20` 을 차단 게이트로 승격**(C-46 조건 3건에 이미 있다)하고 **`1-33`(NLB 에 SG 부착 가능 여부)** 을 먼저 확인하는 것으로 받는다.
...
> 반영 범위 = **C-1 ~ C-63** (2026-08-11 — **CI/CD 형상 확정**(C-61·C-62·C-63) · **NACL 미채택**(C-58) · 🔴 **계정 3개 → 1개**(C-57) · **Interface 엔드포인트 3종 채택**(C-56) · PG 복제 하이브리드(C-55) · DR 전환 2단(C-54)).
> 🔴 **같은 날 두 번 갈아엎었다.** 1차(C-32~C-43)는 선생님 피드백 반영, 2차(C-44~C-52)는 그 위에서
> **Kafka 를 온프렘까지 마저 지우고 노드를 2대로 내린 것**이다. 1차 그림은 폐기했다.

```
                                    ┌──────────────────────┐
                              유저 ─┤ Cloudflare (C-4)     │  DNS + 프록시(주황)
                                    │ 🛡️ WAF · DDoS · CDN  │  🔴 우리 WAF 는 여기다 (C-46)
                                    │ Access(내부도구 SSO) │     AWS WAF 는 NLB 에 못 붙는다
                                    └──────────┬───────────┘
════ AWS Organizations (남의 조직 o-8gviph817d · mgmt = aws_3@etechcloud.net) ═══════
 ...
 │ ┃         🔴 SG = **Cloudflare 대역 전용** ← CF 우회 차단의 유일 수단 (1-20)   ┃ │
 │ ┃                                                                             ┃ │
 │ ┃  ┌─ AZ-a ────────────────────┐  ┌─ AZ-b ────────────────────┐   (C-32)     ┃ │
...
```
   ...
   │      🔴 NLB SG = Cloudflare 대역 전용 ← CF 우회 차단의 유일 수단   │
   │      Cilium NetworkPolicy — app ns **양방향 default-deny 라이브**  │
   ├──────────────────────────────────────────────────────────────────┤
   │ 3.5단 L7 필터링  "요청 *내용*이 정상인가"           (L7)          │
   │      🛡️ Cloudflare WAF — SQLi·XSS·CVE 패턴 · DDoS · 레이트리밋    │
   │      🔴 AWS WAF 미채택 — **NLB(L4)에는 부착 자체가 불가** (C-46)  │
   │         (AWS WAF 는 CloudFront·ALB·API GW·AppSync·Cognito 에만)   │
   ├──────────────────────────────────────────────────────────────────┤
   │ 4단  탐지      "이상이 일어났는가"                                │
   │      🛡️ GuardDuty (C-48)                                          │
...
1. **"WAF 가 없는 게 아니라 위치가 AWS 밖입니다."**
   NLB(L4)를 고른 순간 AWS WAF 는 붙일 자리가 사라진다 — WAF 는 봉투를 뜯어야 하고 NLB 는 안 뜯는다.
   그래서 그 층을 Cloudflare 가 맡는다. **기각이 아니라 배치다.**
...
| # | 급소 | 지금의 방어 | 상태 |
|---|---|---|---|
| ① | **Cloudflare 우회** — NLB 주소를 직접 때리면 3.5단이 통째로 무효 | NLB SG = CF 대역 전용 | 🔴 `1-20` 미완 · **차단으로 승격**(1-39) |
| ② | **XFF 최좌측 신뢰** — `chat` 레이트리밋을 오늘도 우회할 수 있다 | 없음. SG·`numTrustedProxies` 로도 안 고쳐진다 | 🔴 **CF 엣지 레이트리밋으로 근본 해결**(1-41) |
...
- [ ] **1-40 Cloudflare 무료 플랜에서 실제 활성화된 WAF 룰 실측** — 문서에 *"Cloudflare 가 WAF 를 한다"* 고
      써놨는데 **활성 룰이 뭔지 아무도 안 봤다.** 부족하면 Pro($20/월) 승격 판단
- [ ] **1-41 🔴 레이트리밋을 Cloudflare 엣지로 이관** — `chat` 이 **XFF 최좌측을 신뢰**해
      오늘도 우회 가능하다. SG·`numTrustedProxies` 로는 안 고쳐지고, **엣지는 진짜 소스 IP 를 안다**
- [ ] **1-42 게이트 — NLB 보안그룹 지원·지정 시점 확인** — 🔴 **생성 시점에만 지정 가능**할 수 있다.
      나중에 못 붙이면 1-39 가 통째로 막힌다
