...
- 최초 작성: 2026-08-07 · **최종 갱신: 2026-08-10 (2차 개편)**
...
> 🔴 **팀에게 — 지금 상태 요약 (2026-08-10)**
>
> | | |
> |---|---|
> | **확정 결정** | **C-1 ~ C-52** (§0.1). ⚠️ CLAUDE.md 는 아직 "C-1~C-29" 로 적혀 있다 — **stale** |
> | **목표 아키텍처** | **§1 다이어그램**. 이게 draw.io 의 원본이다 |
> | **보안 설명 뼈대** | **§1.1 보안 계층 모델** — 발표 자료는 여기서 파생시킨다 |
> | **남은 미결** | §1 아래 "열린 항목" 표 — **⑫⑬⑭ 는 신설**(2026-08-10) |
> | **총액** | 🔴 **미확정.** $857.26 은 전제 6개가 바뀌어 무효. 확정 절감 −$238.06/월 |
> | **당장 답이 필요한 것** | **#585**(AI·알림 담당 · 08-12 기한) |
>
...
##### 🔴 C-8⑥ 해소 — **VPC Interface 엔드포인트 미채택**
```
...
> 반영 범위 = **C-1 ~ C-52** (2026-08-10 2차 — Kafka 전면 제거 · 노드 2대 · NAT 1대 · WAF 계층 명문화).
...
════ AWS Organizations · 계정 3개 (C-8②) ══════│═══════════════════════════════════
 ...
 │ 결제 · 예산    │  │ CloudTrail 90일    │     │   🔴 SCP 미채택 · **대체 안 함** (C-50)
 │ (IdC 없음)     │  │  + Object Lock     │     │   → 사람 신원은 **IAM 단독**
 ...
 │                          **SQS**(C-44) · EKS 컨트롤플레인(랜카드만 우리 서브넷)│
 └──────────────────────────────────────────────────────────────────────────────┘
        ▲ 크롤 업로드                    ▼ WAL 아카이브
...
| ⑨ | **총액 재산정** | $857.26 이 무효 — 전제가 **6개** 바뀌었다(AZ 2 · Kafka 0 · 터널 0 · VPC 3 · **노드 2** · **NAT 1**) | 🔴 ②⑧ 가 정해져야 |
...
| 🆕⑬ | **Standard-IA 를 CloudTrail 버킷에 쓸지** | 월 절감 소액 · 객체가 크고 적어 IA 가 성립하는 유일한 버킷 | 🟡 사용자 판단 |
...
- [ ] **A-5 CloudTrail 버킷 = 버전관리 ON + Object Lock **GOVERNANCE** 90일** · 🔴 **SCP 미채택(C-35·C-50)이라 "SCP 로 bypass 차단" 은 성립하지 않는다** → `s3:BypassGovernanceRetention` 을 **IAM 정책에서 아무에게도 부여하지 않는 것**으로 대체
...
- ~~VPC 설계 — CIDR 비충돌 · NAT 개수~~ → **C-8③ · C-28 로 확정**. 남은 것 = **S3/ECR/STS 엔드포인트 손익**(C-8⑥ 미완 · 단가 필요)
