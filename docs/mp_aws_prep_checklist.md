...
> | | |
> |---|---|
> | **확정 결정** | **C-1 ~ C-83** (§0.1). ⚠️ CLAUDE.md 는 아직 "C-1~C-29" 로 적혀 있고 **선행 작업 121건** 도 stale(실측 체크박스 160 · 번호 143) — 🔴 **대조표의 `외부 LB → NLB (ALB 기각)` 은 C-60 이 뒤집었다** |
...
> 반영 범위 = **C-1 ~ C-83** (2026-08-13 — 🔴 **Cilium ENI 모드**(C-82 → 파드 CIDR 소멸·`target-type: ip`) ·
...
| 🟢 **온프렘 필수 · 무해** | **2** | **`0-8b`**(가장 먼저 · `patch pv` = 재시작 0) · `0-22`(호스트 C · ⚠️ 여유 27.9GB) | 라이브 · Ansible 신규 롤 |
| 🔴 **온프렘 필수 · 조심** | **1** | `1-1` + `1-18` + `1-16` — **분리 불가**(WAL 이 지금 차고 있다) | 라이브 `data` ns |
| 🕐 **AWS 먼저 · 온프렘은 나중** | **3** | `0-12` · `0-24` · 이슈 **#617** — 이미지가 하나라 파일 분리 불가 ⇒ **태그 시점으로 분리** | `overlays/eks` 태그만 선행 |
| ⚪ 무관 | **1** | `0-21`(문서) | — |
| 🆕 신규 | **1** | **`A-43`** Jenkinsfile CD 에 `overlays/eks` 핀 추가 | 앱 레포 `Jenkinsfile` |
