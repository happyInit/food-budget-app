(function () {
  ...
  window.loadOperationsLiveData = async function (dashboard) {
    ...
    const [anomalyResponse, incidentResponse, summaryResponse, sourceResponse] = await Promise.all([
      fetch(endpoint(`/internal/anomalies?${query}`)).catch(() => null),
      fetch(endpoint(`/internal/incidents?${query}`)).catch(() => null),
      fetch(endpoint(`/dashboard/summary?${query}`)).catch(() => null),
      fetch(endpoint("/dashboard/sources")).catch(() => null)
    ]);
    // Operations DB(anomalies/incidents/summary)가 꺼져 있어도 M.E.L.T(Prometheus/Loki/Tempo)
    // 패널은 독립적으로 떠야 한다 — DB 하나 실패로 전체를 죽이지 않는다.
    dashboard.anomalies = anomalyResponse?.ok ? (await anomalyResponse.json()).map(anomaly) : [];
    dashboard.incidents = incidentResponse?.ok ? await incidentResponse.json() : [];
    dashboard.summary = summaryResponse?.ok ? await summaryResponse.json() : {};
    ...
    if (dashboard.sources.prometheus?.enabled) {
      ...
      try {
        ...
        // Poller freshness (last successful CronJob run), not lag/rate — these
        // producers are once/day (or twice/week) batch jobs by design, so
        // "time since last success" is the only correct staleness signal.
        dashboard.pollerFreshness = await instant(
          'kube_cronjob_status_last_successful_time{namespace="pipeline", cronjob=~"mp-poller-.*|mp-deal-pruner"}'
        );
        ...
        // These consumers scale to zero between bursts (KEDA), so scrapes are
        // sparse (~10-15/24h) and rate(...[5m]) almost never has two samples
        // inside its window — the records/sink-write/p95 series above are
        // legitimately empty most of the time, not broken. This timestamp is
        // the same "last success" signal pollerFreshness already uses for the
        // CronJob pollers, applied to the event-driven consumers instead.
        // Plain instant() looks only at "right now" — once the consumer pod
        // scale-to-zeroes, its whole scrape target (this gauge included)
        // disappears from Prometheus immediately, not just the rate. Wrap in
        // max_over_time(...[24h]) so the instant query still finds the last
        // reading Prometheus captured before the target went away, the same
        // way `max_over_time` recovers a stale-but-still-relevant gauge.
        dashboard.pipelineLastSuccess = await instant("max by(component) (max_over_time(fb_pipeline_last_success_timestamp_seconds[24h]))");
