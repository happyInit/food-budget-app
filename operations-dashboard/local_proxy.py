...
from (
  ...
  where evaluated_at between :'start_at'::timestamptz and :'end_at'::timestamptz
    -- The database retains historic candidate/noise records for audit. The
    -- operations queue must not re-promote them merely because a browser has
    -- an older JavaScript bundle cached.
    and status = 'anomaly'
    and (
      metric_id not in (
        'service_request_rate', 'pod_cpu_usage',
        'pod_memory_working_set', 'redis_memory_ratio'
      )
      or (
        metric_id = 'service_request_rate'
        and abs(current_value) >= 0.1
        and abs(coalesce((baseline ->> 'mean')::double precision, 0)) > 1e-12
      )
      or (
        metric_id = 'pod_cpu_usage'
        and abs(current_value) >= 0.05
        and abs(current_value - coalesce((baseline ->> 'mean')::double precision, 0)) >= 0.025
      )
      or (
        metric_id = 'pod_memory_working_set'
        and abs(current_value) >= 134217728
        and abs(current_value - coalesce((baseline ->> 'mean')::double precision, 0)) >= 67108864
      )
      or (
        metric_id = 'redis_memory_ratio'
        and abs(current_value) >= 1
        and abs(coalesce((baseline ->> 'mean')::double precision, 0)) > 1e-12
        and abs(current_value - coalesce((baseline ->> 'mean')::double precision, 0)) >= 1
      )
    )
...
class DashboardHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        parsed = urlparse(self.path)
        if parsed.path == "/api/internal/chat":
            try:
                content_length = int(self.headers.get("Content-Length", "0"))
                if content_length <= 0 or content_length > 4096:
                    raise ValueError("chat request body is invalid")
                payload = json.loads(self.rfile.read(content_length))
                question = payload.get("question")
                if not isinstance(question, str) or not question.strip():
                    raise ValueError("chat request must include a non-empty question")
                # Helpdesk mode (Incident detail "물어보기") sends incident_id to
                # scope the answer to one Incident's own Evidence/RCA instead of
                # a cluster-wide snapshot — this proxy used to silently drop it,
                # so every browser-side chat request landed in ungated mode no
                # matter what the frontend sent (confirmed live: identical
                # question via curl straight to Operations API answered
                # correctly scoped; via this proxy it always came back generic).
                incident_id = payload.get("incident_id")
                if incident_id is not None and not isinstance(incident_id, str):
                    raise ValueError("incident_id must be a string when present")
                forward_payload = {"question": question}
                if incident_id:
                    forward_payload["incident_id"] = incident_id
                status_code, body = forward_chat_message(forward_payload)
            ...
            self.wfile.write(body)
            return
        if parsed.path == "/api/internal/anomalies/rca":
            try:
                content_length = int(self.headers.get("Content-Length", "0"))
                if content_length <= 0 or content_length > 4096:
                    raise ValueError("anomaly RCA request body is invalid")
                payload = json.loads(self.rfile.read(content_length))
                if set(payload) not in (
                    {"metric_id", "subject_key", "evaluated_at"},
                    {"metric_id", "subject_key", "evaluated_at", "force_refresh"},
                ):
                    raise ValueError("anomaly RCA request fields are invalid")
                status_code, body = forward_anomaly_rca(payload)
            except (ValueError, json.JSONDecodeError) as error:
                self.send_error(HTTPStatus.BAD_REQUEST, str(error))
                return
            except RuntimeError as error:
...
def forward_chat_message(payload: dict[str, str]) -> tuple[int, bytes]:
    """Forward a chat question. The snapshot is always server-gathered on the
    Operations API side — the browser cannot spoof it by sending its own.
    incident_id (when present) is only a lookup key forwarded as-is; Operations
    API re-resolves it against the DB rather than trusting any of its content."""
    base_url = os.environ.get(OPERATIONS_API_URL_ENV)
    if not base_url:
        raise RuntimeError(f"set {OPERATIONS_API_URL_ENV} first")
    forward_body = {"question": payload["question"]}
    if payload.get("incident_id"):
        forward_body["incident_id"] = payload["incident_id"]
    encoded = json.dumps(forward_body).encode()
    request = urllib.request.Request(
        f"{base_url.rstrip('/')}/internal/chat",
        data=encoded,
        method="POST",
...
def forward_anomaly_rca(payload: dict[str, str]) -> tuple[int, bytes]:
    """Forward a selected anomaly key; Evidence stays server-generated."""
    base_url = os.environ.get(OPERATIONS_API_URL_ENV)
    if not base_url:
        raise RuntimeError(f"set {OPERATIONS_API_URL_ENV} first")
    encoded = json.dumps(payload).encode()
    request = urllib.request.Request(
        f"{base_url.rstrip('/')}/internal/anomalies/rca",
        data=encoded,
        method="POST",
        headers={"Accept": "application/json", "Content-Type": "application/json"},
    )
...
if __name__ == "__main__":
    # Docker에서는 Nginx가 같은 Compose network에서 이 API에 연결한다.
    # Host 포트는 publish하지 않으므로 외부 공개 범위는 Nginx로 한정된다.
    listen_host = os.environ.get("OPERATIONS_DASHBOARD_LISTEN_HOST", "127.0.0.1")
    listen_port = int(os.environ.get("OPERATIONS_DASHBOARD_PORT", "8080"))
    ThreadingHTTPServer((listen_host, listen_port), DashboardHandler).serve_forever()
