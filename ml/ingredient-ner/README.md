...
|---|---|---|---|
| 약지도 라벨(test 246) — *참고용* | 0.976 | 0.979 | 0.978 |
| **사람 검수 gold(50건) — 정본** | 0.900 | **0.803** | **0.849** |

미등재 일반화 스팟체크: `방울토마토 150g, 양파 10g` → `[방울토마토·양파·부추]` ✅

> ⚠️ **0.978 ≠ 실제 성능.** 그건 "약지도 vs 약지도"(자동라벨끼리)라 부풀려진 값이다. **사람 gold 대비 진짜 F1은 0.849**(`score_gold.py`, gold=`gold_test.txt`) — `ai-spec.md §1`의 F1≥0.85 **경계선에 걸쳐 있고**(50건 표본 오차 범위), **아직 명확한 통과 아님.**

**병목 2가지 (실측):**
- **Recall 0.803** — 사전 밖 재료를 놓침(그린빈스·주꾸미·요거트 등).
- **Precision** — 약지도 사전에 든 헤더성 단어(소스·양념)를 CRF가 재료로 오탐.

## 다음 단계 (≥0.85 — gold 확대 없이 자동 개선으로)

> gold는 측정 자일 뿐이라, **기존 50건 gold에 재채점하며 아래 자동 개선을 반복**한다. 사람 재검수 노동 없이 상대 개선 확인 가능(절대 수치는 50건이라 ±수%p 노이즈 감안).

1. **Precision↑**(코드): 약지도 사전에서 헤더성·노이즈 단어(소스·양념·물…) 제거 + 섹션 마스킹 강화 → CRF 오탐↓ → 재학습
2. **Recall↑**(코드): 공공 식품 사전으로 사전 증강 + 자기학습(부트스트래핑)으로 사전 밖 재료 확장 → 놓침↓ → 재학습
3. 위 반복으로 gold 재채점 → 0.85 안정 초과 확인
4. `CrfSpanExtractor`(services/chat `span_extractor/ner.py`) 구현 → `EXTRACTOR_BACKEND=ner` 스왑 후 챗봇 재가동
5. *(선택)* gold 확대는 노동 부담이 커 **후순위** — 필요 시 소량씩만.
4. 형태소 단위(현재 문자 단위) 검토 — nori 등, 정확도 여지 · 자기학습(부트스트래핑)으로 사전 밖 재료 확장
