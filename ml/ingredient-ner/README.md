...
3. ✅ `CrfSpanExtractor`(services/chat `span_extractor/ner.py`) **in-process 구현(A안)** — 단, **채팅 백엔드로는 미스왑**(아래 ⚠️).
   ...
   - ✅ **[해소] 채팅용 대화체 NER — 학습데이터 없이 결론남**(실측 2026-07-29). 세 방식을 같은 대화체 케이스에 나란히 측정:

     | 방식 | 정밀도 | 재현율 | F1 | 평균지연 |
     |---|---|---|---|---|
     | **rule(현행)** | **0.983** | **1.000** | **0.990** | **1ms** |
     | CRF(레시피 분포) | 0.633 | 0.667 | 0.620 | 0ms |
     | 생성형 `nova-micro` | 0.533 | 0.533 | 0.533 | 358ms |

     **rule 이 이미 최선이라 대화체 학습데이터가 필요 없다.** 생성형은 두 방향으로 동시에 틀렸다 —
     "떡볶이 재료 알려줘"에 `['고기','떡','채소','고추장','참기름','양파','마늘']` 처럼 **유저가 말하지 않은
     재료를 창작**하고(추천 입력 오염), 반대로 "삼겹살 500g 사왔는데"·"새우 알러지 있어서"는 **빈 배열**로 놓쳤다.
     지연도 rule 대비 **350배**이고 턴마다 API 비용이 든다. → **대화체는 rule 유지가 결론.**
     ⚠️ 그 과정에서 rule 오탐 1건 발견·수정: "양파로" → 접미매칭 → item 2572 **'파로'**(곡물 farro).
     조사를 뗀 형태가 다른 품목에 매칭되면 그쪽이 진짜라는 규칙으로 걸러 **F1 0.977 → 0.990**.
     (실 대화 로그는 2건뿐이라 학습데이터 확보 경로가 없었는데, **애초에 필요하지 않았다.**)
