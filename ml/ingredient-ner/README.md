...
| | train | test | span F1 |
|---|---|---|---|
| 문자 CRF | 897 | 246 | **P 0.976 · R 0.979 · F1 0.978** |

미등재 일반화 스팟체크: `방울토마토 150g, 양파 10g` → `[방울토마토·양파·부추]` ✅

> ⚠️ **정직한 해석**: 이 F1 0.978은 **약지도 라벨(weak label) 대비** 수치다 — 사람이 검수한 gold가 아니라 사전매칭 자동라벨을 정답으로 잰 것. 즉 "CRF가 약지도 패턴을 잘 재현한다"는 뜻이지 **실제 정확도 0.978이 아니다.** 진짜 성능은 **사람이 라벨링한 held-out 테스트셋(HITL)**으로 재야 하며, 그게 `ai-spec.md §1`의 F1 ≥ 0.85 검증의 정본이다.

## 다음 단계

1. **HITL gold 테스트셋 구축**(수십~수백 건 수기 검수) → 진짜 span F1 측정 (현재 0.978은 약지도 대비치)
2. 약지도 노이즈 보정(육안 검수로 사전 화이트리스트·오탐 패턴 조정) 후 재학습
