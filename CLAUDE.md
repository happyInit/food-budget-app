...
- **Ansible** = `infra/ansible/` — 노드 베이스라인 + (현행) 서비스 배포. **멱등** · remote_user=`ubuntu`·become.
  ...
  **존치 롤**(K8s 이후에도 씀) = `base`·`harbor`·`ca_trust`·`team_ssh_keys`·`node_exporter_host`·`monitoring_agents`(호스트 C 포함) + `jenkins`·`sonarqube`·`cloudflared`(CI 웹훅 터널)·`harbor_backup`·`jenkins_backup`(→ S3).
  🔴 **`k8s_platform_apps` 도 존치다** — 단 **역할이 2026-08-13 에 바뀌었다**(config#151 + app#619, C-18 선행). ~~`observability/lgtm-minio-creds` 를 직접 만든다~~ → 그 Secret 은 이제 **ESO 소유**(`ExternalSecret mp-lgtm-object-store` · `reconcile.external-secrets.io/managed=true`)이고, 이 롤이 공급하는 것은 **ESO 가 읽어가는 백엔드 소스 `fb-secrets/observability-secrets`** 다. 공급 사슬 = `이 롤 → fb-secrets/observability-secrets → ESO → observability/lgtm-minio-creds → Loki·Tempo`. **그래도 지우면 안 된다** — 지우면 ESO 가 읽을 소스가 IaC 밖(수작업)이 되어 재구축 때 소실된다. 🔴 키 이름 2개는 config 레포 `platform/observability/base/externalsecret-lgtm-object-store.yaml` 의 `remoteRef.property` 와 **한 쌍**이라 한쪽만 바꾸면 ESO 가 조용히 SecretSyncError 로 떨어지고 **다음 재시작에 Tempo 가 fail-fast** 한다.
  🔴 **다만 이 롤을 함부로 돌리지 말 것** — `lgtm-apps.yaml` 로 **loki·tempo·alloy Application 을 apply 한다**. 그 셋은 이미 `platform-root` 소유다(2026-08-13 실측: `argocd.argoproj.io/tracking-id=platform-root:...`). 롤이 위에 덮어쓰면 selfHeal 이 되돌려 **플랩**이 나고, **C-72(온프렘 동결·덧셈만)에도 걸린다.** 실행은 클러스터 재구축 시 또는 이 롤에서 Application apply 를 걷어낸 뒤.
