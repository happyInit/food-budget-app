...
- **이미지 태깅 = 3태그** (2026-07-16 확정, PR #97): `:<sha>`(불변 신원) + `:X.Y.Z`(릴리스 핀·불변) + `:latest`(가변 편의). **버전 태그 `:X.Y.Z`는 릴리스 런(`workflow_dispatch`)에서만** 빌드·push — 자동 `main` push 는 `:<sha>`+`:latest`만(불변성 + 부분빌드 landmine 회피). 버전 SoT = `build-push-app.yml`·`build-push-pipeline.yml`의 `APP_VERSION` env(현재 `1.1.1`, 릴리스마다 올림). 내부 semver: **MAJOR**=마이그레이션급·계약파괴 / **MINOR**=하위호환 기능 / **PATCH**=버그픽스·설정. compose 는 `${IMAGE_TAG:-1.1.1}`로 핀(app=`deploy/app/docker-compose.yml`, 파이프라인=루트 `docker-compose.yml`). 배포 핀 = app 자동 push `:latest`·릴리스 `:APP_VERSION` / 파이프라인은 CD 없음 → fb-data operator 가 `IMAGE_TAG` 지정(기본 = 릴리스 핀).
