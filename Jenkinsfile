...
def CATALOG = [
  ...
  //   🔴 crawler-kurly 의 srcs — Dockerfile 이 pipelines/stream/ 에서 3개 파일(_kafka·_topics·_observability)을
  //      COPY 하는데 감지 경로가 crawler/kurly/ 뿐이라, 공유코드를 고쳐도 이 이미지는 리빌드되지 않았다.
  //      파일 3개가 아니라 디렉터리 전체를 넣는다 — 그 의존이 늘 때 또 같은 함정에 빠지지 않게.
  //      대신 pipelines/stream/ 의 아무 파일이나 바뀌면 크롤러도 리빌드되는 비용을 받는다.
