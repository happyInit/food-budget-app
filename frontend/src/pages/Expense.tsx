    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginBottom: 18 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => nav('/performance')} style={{ padding: '10px 15px', border: '1.5px solid #E6E6E6', background: '#fff', color: '#1A1A1A', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>성과 보기</button>
          <button onClick={() => nav('/expense/add')} style={{ padding: '10px 15px', border: 'none', background: '#1FA463', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>지출 기록</button>
        </div>
      </div>

      {/* 다크 예산 히어로 */}
      <div style={{ background: '#1A1A1A', color: '#fff', padding: '24px 26px', marginBottom: 18, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 18 }}>
        <div>
          ...
          <div style={{ fontSize: 12.5, color: '#8FE3B7', marginTop: 6, fontWeight: 600 }}>이 페이스면 월말 +42,000원 절약 예상</div>
        </div>
        <div style={{ flex: 1, minWidth: 190, maxWidth: 300 }}>
          ...
          <div style={{ height: 9, background: 'rgba(255,255,255,.16)', overflow: 'hidden' }}>
            <div style={{ height: '100%', width: '39%', background: '#1FA463' }} />
          </div>
        </div>
      </div>
      ...
      <div style={{ ...card, padding: 26 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          ...
          <span className="num" style={{ fontSize: 13, color: '#1FA463', fontWeight: 700 }}>117,600원</span>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 6, textAlign: 'center' }}>
          ...
          {days.map((d) => {
            ...
            const sp = spend[d]
            return (
              <div key={d} style={{ minHeight: 74, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontSize: today ? 12.5 : 17, background: today ? '#1FA463' : 'transparent', color: today ? '#fff' : sp ? '#1A1A1A' : '#B5B5B5', fontWeight: today ? 700 : 400 }}>
                {d}
                {sp && <span className="num" style={{ fontSize: today ? 9 : 11.5, color: today ? '#fff' : '#1FA463', fontWeight: 700 }}>{sp}</span>}
              </div>
            )
          })}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 16, marginTop: 16 }}>
        <div style={{ ...card, padding: 22 }}>
          ...
          {[['집밥 (재료비)', '68,400원 · 58%', 58, '#1FA463'], ['외식', '35,000원 · 30%', 30, '#1FA463'], ['카페 · 간식', '14,200원 · 12%', 12, '#B5B5B5']].map(([k, v, w, c]) => (
        ...
        <div style={{ ...card, padding: 22 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            ...
            <span style={{ padding: '4px 10px', fontSize: 12, fontWeight: 700, background: '#E6F6EC', color: '#1FA463' }}>11,200원</span>
          </div>
          {[['아침 · 계란말이', '집밥', '#15B76E', '0원', '#15B76E'], ['점심 · 김밥천국', '외식', '#1FA463', '7,000원', '#1A1A1A'], ['저녁 · 김치찌개', '집밥', '#15B76E', '4,200원', '#1A1A1A']].map(([t, tag, tagC, p, pC], i) => (
        ...
        {priceTrends.map((t) => (
          <div key={t.name} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '15px 0', borderTop: '1px solid #EFEFEF' }}>
            ...
            <svg viewBox="0 0 72 24" width="72" height="24" style={{ flexShrink: 0 }}>
              <polyline points={t.pts} fill="none" stroke={t.up ? '#F04452' : '#1FA463'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            <div style={{ textAlign: 'right', minWidth: 92 }}>
              <div className="num" style={{ fontSize: 15, fontWeight: 800 }}>{t.price}</div>
              <div className="num" style={{ fontSize: 12, fontWeight: 700, color: t.up ? '#F04452' : '#15B76E' }}>{t.delta}</div>
            </div>
          </div>
        ))}
      </div>
