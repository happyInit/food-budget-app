...
const list = [
  ...
  { to: '/hotdeal', title: '삼겹살 최저가 알림!', desc: '마켓컬리 100g 1,290원 (평균 대비 30%↓)', time: '3시간 전', color: '#1FA463' },
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginBottom: 18 }}>
        ...
        <span style={{ fontSize: 13, color: '#1FA463', fontWeight: 600, cursor: 'pointer' }}>모두 읽음</span>
      </div>
      <div style={{ maxWidth: 640, background: '#fff', border: '1px solid #E6E6E6', padding: '8px 20px' }}>
        {list.map((n, i) => (
          <div key={i} onClick={() => nav(n.to)} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 0', borderBottom: i < list.length - 1 ? '1px solid #EFEFEF' : 'none', cursor: 'pointer' }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: n.color ?? '#1A1A1A' }}>{n.title}</div>
