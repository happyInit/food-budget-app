  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 18 }}>
        {filters.map((f, i) => (
          <span key={f} style={{ padding: '7px 14px', fontSize: 13, fontWeight: i === 0 ? 700 : 600, border: i === 0 ? '1.5px solid #F26419' : '1.5px solid #E6E6E6', background: i === 0 ? '#F26419' : '#fff', color: i === 0 ? '#fff' : '#5E5E5E', cursor: 'pointer' }}>{f}</span>
        ))}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(130px,1fr))', gap: 10 }}>
        {Array.from({ length: 12 }, (_, i) => (
          <div key={i} onClick={() => nav('/recipes/' + (i + 1))} className="zoom-wrap" style={{ aspectRatio: '1', cursor: 'pointer', background: '#F0F0F0' }}>
            <div className="zoom" style={{ width: '100%', height: '100%', background: `center/cover no-repeat url("${img(i, 300)}")` }} />
          </div>
        ))}
      </div>
