...
// 냉장고 한 칸(냉장/냉동) — 남는 높이를 flex 로 채우고, 재료 많으면 내부 스크롤.
function Compartment({ label, temp, tint, accent, items, onAction }: {
  label: string; temp: string; tint: string; accent: string; items: PantryVM[]; onAction?: (it: PantryVM) => void
}) {
  return (
    <div style={{ background: tint, border: '1px solid rgba(0,0,0,.06)', padding: '10px 12px', flex: 1, minHeight: 92, display: 'flex', flexDirection: 'column' }}>
      ...
      {items.length === 0 ? (
        <div style={{ fontSize: 11.5, color: '#9AA3AF' }}>비어있어요</div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, overflowY: 'auto' }}>
          {items.map((it) => <MiniChip key={it.id} it={it} onAction={onAction} />)}
        </div>
      )}
    </div>
  )
...
export default function FridgeCard({ zones, total, mode = 'home', onItemAction, onManage }: {
  ...
  return (
    <div style={{ border: '1px solid #C6CDD7', borderRadius: 16, overflow: 'hidden', background: 'linear-gradient(180deg,#EEF1F5,#DFE4EB)', boxShadow: '0 20px 44px rgba(23,38,74,.14)', height: '100%', minHeight: 460, display: 'flex', flexDirection: 'column' }}>
      ...
      {/* 실온·팬트리 = 문 밖 선반(항상 보임) */}
      <div style={{ background: 'linear-gradient(180deg,#F0EBE2,#E4DCCE)', borderBottom: '1px solid #D8CDB8', padding: '11px 14px 13px', flexShrink: 0 }}>
        ...
        {zones.room.length === 0 ? (
          <div style={{ fontSize: 11.5, color: '#BBA97F' }}>비어있어요</div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>{zones.room.map((it) => <MiniChip key={it.id} it={it} onAction={onItemAction} />)}</div>
        )}
      </div>
      {/* 선반–본체 사이 나무 단 */}
      ...
      <div style={{ position: 'relative', perspective: 1600, padding: 14, flex: 1, display: 'flex' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, flex: 1, minWidth: 0 }}>
          <Compartment label="냉장실" temp="3℃" tint="rgba(255,255,255,.6)" accent="#17264A" items={zones.fridge} onAction={onItemAction} />
          <Compartment label="냉동실" temp="−18℃" tint="#EAF6FF" accent="#2178AE" items={zones.freezer} onAction={onItemAction} />
        </div>
