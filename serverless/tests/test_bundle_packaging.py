...
# ── ④ 컨테이너 함수는 **없다** ────────────────────────────────────────────────
def test_컨테이너_함수가_없다():
    """🔵 `rank-serve` 를 이관하지 않기로 하면서(`docs/serverless/09`) 컨테이너 경로가 사라졌다.
    ⇒ **10종 전부 zip 이고 `build.sh` 규율이 통일된다.** 누가 다시 Dockerfile 을 넣으면
       그건 «왜 zip 으로 안 되는가» 를 먼저 답해야 하는 변경이라, 여기서 걸리게 둔다."""
    dockerfiles = [p.name for p in SERVERLESS.iterdir() if p.is_dir() and (p / "Dockerfile").exists()]
    assert not dockerfiles, (
        f"컨테이너 함수가 생겼다 — {dockerfiles}. zip 으로 안 되는 이유를 "
        f"`docs/serverless/09` 처럼 문서로 남기고 이 테스트를 고칠 것")
